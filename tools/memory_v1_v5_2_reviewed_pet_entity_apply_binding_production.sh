...
# seebx backend only. Applies three reviewed pet entity resolutions and binds
# three already-staged observations. It stops before claims, projection,
# Qdrant, retrieval, and prompt influence.

if [[ "${MEMORY_V1_V5_2_REVIEWED_PET_ENTITY_APPLY_PRODUCTION:-}" != authorized ]]; then
  echo 'MEMORY_V1_V5_2_REVIEWED_PET_ENTITY_APPLY_PRODUCTION=authorized is required' >&2
  exit 1
fi
...
lock_file=/home/ubuntu/brains/.memory_v1_v5_2_reviewed_pet_entity_apply.lock
phase=initialization
run_tag=
status_file=
units_quiesced=0
unit_state=$(mktemp /tmp/memory-v1-v5-2-reviewed-pet-entity-apply-units.XXXXXX)
table_list=$(mktemp /tmp/memory-v1-v5-2-reviewed-pet-entity-apply-tables.XXXXXX)
...
if sum(expected.values()) != 13:
    raise SystemExit("target row budget is not 13")
PY
}
...
git merge-base --is-ancestor f8870f0d26f46fe031acb32c01c10f6596219d90 "$head"
...
hash_lock "$sql_test" sql_test_sha \
  0af4a1f6f0c74591378adf4a14ac0429ff37b5c89293d46b94fec034787c1f87
hash_lock "$runner" runner_sha \
  ba4b0a80badfa7b41fcec671aa42db1d22a9bd66cf772e0397c8faadc516edad
...
status_file="$snapshot_dir/memory_v1_v5_2_reviewed_pet_entity_apply_${run_tag}.status"
reviews="$review_base/reviewed-pet-entity-apply-$run_tag"
...
phase=backup
backup_partial="$snapshot_dir/.memory_pre_v5_2_reviewed_pet_entity_apply_${run_tag}.dump.partial"
backup="$snapshot_dir/memory_pre_v5_2_reviewed_pet_entity_apply_${run_tag}.dump"
...
[[ -s "$table_list" ]]
target_before="$snapshot_dir/memory_v1_v5_2_reviewed_pet_entity_apply_target_before_${run_tag}.tsv"
target_after="$snapshot_dir/memory_v1_v5_2_reviewed_pet_entity_apply_target_after_${run_tag}.tsv"
non_target_before="$snapshot_dir/memory_v1_v5_2_reviewed_pet_entity_apply_non_target_before_${run_tag}.tsv"
non_target_after="$snapshot_dir/memory_v1_v5_2_reviewed_pet_entity_apply_non_target_after_${run_tag}.tsv"
...
assert_equal report_rows \
  "$(jq -r '.new_rows' "$reviews/apply-report.json")" 13
assert_equal report_entities \
  "$(jq -r '.new_entities' "$reviews/apply-report.json")" 1
assert_equal report_applies \
  "$(jq -r '.new_entity_resolution_applies' "$reviews/apply-report.json")" 3
assert_equal report_aliases \
  "$(jq -r '.new_alias_observations' "$reviews/apply-report.json")" 3
assert_equal report_replay \
  "$(jq -r '.zero_write_replay' "$reviews/apply-report.json")" true
assert_equal report_bindings \
  "$(jq -r '.new_observation_bindings' "$reviews/apply-report.json")" 3
assert_equal report_operations \
  "$(jq -r '.new_operation_requests' "$reviews/apply-report.json")" 3
...
assert_equal exact_applies "$(psql_row "
  ...
  WHERE owner_user_id='$owner'::uuid
    AND resolution_id IN (
      '$keasha_resolution'::uuid,'$dahlia_resolution'::uuid,
      '$helsing_resolution'::uuid
    )")" 3
...
phase=report
report="$snapshot_dir/memory_v1_v5_2_reviewed_pet_entity_apply_${run_tag}.json"
jq -n \
  ...
  '{
    contract_version:"memory_v1_v5_2_reviewed_pet_entity_apply_production_report_v1",
    ...
    rows:{entities:1,resolution_applies:3,aliases:3,bindings:3,
      operation_requests:3,total:13,claims:0,projections:0},
...
phase=complete
printf '%s\n' \
  'MEMORY_V1_V5_2_REVIEWED_PET_ENTITY_APPLY_PRODUCTION=PASS' \
  "head=$head" \
  "report=$report" \
  "backup=$backup" \
  'entities_created=1' \
  'entity_applies_created=3' \
  'aliases_created=3' \
  'observation_bindings_created=3' \
