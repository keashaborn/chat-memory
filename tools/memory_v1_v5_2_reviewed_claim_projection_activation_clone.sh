...
# seebx backend only. Clone-tests copying the 19 validated reviewed-claim
# vectors from the isolated shadow collection into a temporary activation
# collection while updating only a disposable Postgres clone.
...
source_db=memory
clone_db="memory_v5_reviewed_activation_$(date -u +%Y%m%d%H%M%S)_$$"
python_bin=/opt/chat-memory/venv/bin/python
artifact_dir="/home/ubuntu/memory-v1-reviews/reviewed-claim-projection-activation-clone-$(date -u +%Y%m%dT%H%M%SZ)-$(git -C "$repo_root" rev-parse --short=12 HEAD)"
table_list=$(mktemp /tmp/memory-v1-reviewed-activation-tables.XXXXXX)
clone_exists=0
collection_exists=0
...
collection_exists=1
TARGET_COLLECTION="$target_collection" QDRANT_URL="$QDRANT_URL" \
...
jq -n \
  ...
  '{
    contract_version:"memory_v1_v5_2_reviewed_claim_projection_activation_clone_report_v1",
    head_commit:$head,
    ...
    clone_postgres_outbox_rows_applied:19,
    temporary_qdrant_points_applied:19,
...
printf 'memory_v1_v5_2_reviewed_claim_projection_activation_clone: PASS\n'
