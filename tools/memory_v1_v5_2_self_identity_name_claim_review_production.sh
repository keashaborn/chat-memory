...
# seebx backend only. Stages and reviews exactly one bound self identity.name
# candidate, proves zero-write replay and isolation, then stops before claims,
# Qdrant, retrieval, or prompt influence.

if [[ "${MEMORY_V1_V5_2_SELF_IDENTITY_NAME_CLAIM_PRODUCTION_APPLY:-}" \
      != authorized ]]; then
  echo 'MEMORY_V1_V5_2_SELF_IDENTITY_NAME_CLAIM_PRODUCTION_APPLY=authorized is required' >&2
  exit 1
fi
if [[ "$#" -ne 1 ]]; then
  echo 'usage: ..._self_identity_name_claim_review_production.sh ARTIFACT_DIR' >&2
  exit 2
fi
...
stage_runner=scripts/memory_v1_v5_2_self_identity_name_claim_stage.py
review_manifest_runner=scripts/memory_v1_v5_2_self_identity_name_claim_review_manifest.py
review_runner=scripts/memory_v1_v5_2_self_identity_name_claim_review_batch.py
container=brains-postgres-1
database=memory
lock_file=/home/ubuntu/brains/.memory_v1_v5_2_self_identity_name_claim_apply.lock
...
for required in \
  "$stage_runner" "$review_manifest_runner" "$review_runner"; do
...
brains_quiesced=0
brains_state_before=
unit_state=$(mktemp /tmp/memory-v1-v5-2-self-identity-name-units.XXXXXX)
table_list=$(mktemp /tmp/memory-v1-v5-2-self-identity-name-tables.XXXXXX)
...
status_file="$snapshot_dir/memory_v1_v5_2_self_identity_name_claim_${run_tag}.status"
...
target_before="$snapshot_dir/memory_v1_v5_2_self_identity_name_target_before_${run_tag}.tsv"
target_after="$snapshot_dir/memory_v1_v5_2_self_identity_name_target_after_${run_tag}.tsv"
target_replay="$snapshot_dir/memory_v1_v5_2_self_identity_name_target_replay_${run_tag}.tsv"
non_target_before="$snapshot_dir/memory_v1_v5_2_self_identity_name_non_target_before_${run_tag}.tsv"
non_target_after="$snapshot_dir/memory_v1_v5_2_self_identity_name_non_target_after_${run_tag}.tsv"
non_target_replay="$snapshot_dir/memory_v1_v5_2_self_identity_name_non_target_replay_${run_tag}.tsv"
...
phase=backup
backup_partial="$snapshot_dir/.memory_pre_v5_2_self_identity_name_${run_tag}.dump.partial"
backup="$snapshot_dir/memory_pre_v5_2_self_identity_name_${run_tag}.dump"
...
if len(stage["items"]) != 1:
    raise SystemExit("exactly one self-name candidate is required")
item = stage["items"][0]
if (
    item["predicate"] != "identity.name"
    or item["canonical_text"] != "The user's name is Eric."
):
    raise SystemExit("self-name candidate semantics drifted")
value = {
    "contract_version":
        "memory_v1_v5_2_self_identity_name_claim_review_decisions_v1",
    ...
    "decisions": [{
        ...
        "decision": "authorized",
        "reason": (
            "Reviewed direct self-name claim from the owner-authored evidence "
            "and its trusted owner-self entity binding."
        ),
        "reason_codes": [
            "owner_authored_identity",
            "trusted_owner_self_binding",
            "direct_claim_semantics_reviewed",
        ],
    }],
}
...
MEMORY_V1_V5_2_SELF_IDENTITY_NAME_CLAIM_STAGE_APPLY=authorized \
PYTHONPATH="$repo_root" /opt/chat-memory/venv/bin/python \
  "$repo_root/$stage_runner" apply \
  --manifest "$manifest" --authorization "$authorization" \
  --confirm STAGE_EXACT_SELF_IDENTITY_NAME_CLAIM_CANDIDATE_ONLY \
  --output "$stage_apply"
...
MEMORY_V1_REQUIRED_HEAD="$head" \
MEMORY_V1_V5_2_SELF_IDENTITY_NAME_CLAIM_REVIEW_APPLY=authorized \
PYTHONPATH="$repo_root/scripts:$repo_root" \
  /opt/chat-memory/venv/bin/python "$repo_root/$review_runner" \
...
[[ "$(psql_row "
  ...
  WHERE payload.owner_user_id='$target_owner'
    ...
    AND item.predicate='identity.name'
    AND item.review_state='manual_review_required'
    AND payload.claim_class='direct_claim'
    AND payload.canonical_text='The user''s name is Eric.'
...
value = {
    "contract_version":
        "memory_v1_v5_2_self_identity_name_claim_review_production_report_v1",
    "completed_at":
...
printf 'backup=%s\n' "$backup"
printf 'report=%s\n' "$report"
printf 'memory_v1_v5_2_self_identity_name_claim_review_production: PASS\n'
