...
# seebx backend only. Tests the compact semantic prompt, bounded retry
# compatibility, and rollback against a disposable production database clone.

repo_root=$(git rev-parse --show-toplevel)
port=${MEMORY_V1_V5_2_COMPACT_PROMPT_CLONE_PORT:-55476}
export MEMORY_V1_STAGE_BATCH_CLONE_PORT="$port"
compose=(
  docker compose
  -p memoryv1v52compactpromptclone
  -f docker-compose.ci.yml
  -f docker-compose.stage-batch-clone.yml
)
migration=ops/sql/20260722_memory_v1_v5_local_incomplete_prompt_retry.sql
rollback=ops/sql/20260722_memory_v1_v5_local_incomplete_prompt_retry_rollback.sql
...
expected_rollback_sha=92644619ff8337dfe9d8b9607878f5d4a38c20dfa51ccc2bf0ac5d1f048ed104
expected_test_sha=f13549e05910435cb834de7ad8bef28d8a1ee64a3dcce032e47955c57eb5bafc

backup=$(mktemp /tmp/memory-v1-v5-2-compact-prompt.XXXXXX.dump)
tables=$(mktemp /tmp/memory-v1-v5-2-compact-prompt-tables.XXXXXX)
before=$(mktemp /tmp/memory-v1-v5-2-compact-prompt-before.XXXXXX)
after=$(mktemp /tmp/memory-v1-v5-2-compact-prompt-after.XXXXXX)
...
printf '%s\n' 'memory_v1_v5_2_compact_prompt_retry_production_clone: PASS'
