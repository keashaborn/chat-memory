...
# seebx backend only. Clone-tests exact four-claim deferred projection
...
artifact_dir="/home/ubuntu/memory-v1-reviews/compiler-v8-projection-clone-$(date -u +%Y%m%dT%H%M%SZ)-$(git -C "$repo_root" rev-parse --short=12 HEAD)"
...
[[ "$(jq -er '[.outcomes[]|select(.status=="pending")]|length' "$apply")" == 4 ]]
...
[[ "$exact_state" == "4|pending|pending|0|0|t" ]]
...
jq -n \
  ...
  '{
    contract_version:"memory_v1_v5_compiler_v8_four_claim_projection_clone_report_v1",
    head_commit:$head,
    ...
    apply_rows:4,
    replay_rows:0,
    exact_pending_held_outbox_rows:4,
...
printf 'memory_v1_v5_compiler_v8_four_claim_projection_production_clone: PASS\n'
