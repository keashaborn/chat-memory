...
if [[ "${MEMORY_V1_V5_2_EVIDENCE_CONTEXT_SELF_ENTITY_PRODUCTION:-}" != authorized ]]; then
  echo 'MEMORY_V1_V5_2_EVIDENCE_CONTEXT_SELF_ENTITY_PRODUCTION=authorized is required' >&2
  exit 1
fi

repo_root=$(git rev-parse --show-toplevel)
required_ancestor=59d9e484c5758bc24a772acb107604d7d359beb6
...
lock_file=/home/ubuntu/brains/.memory_v1_v5_2_evidence_context_self_entity.lock
...
status_file=
run_id=
timer_state=$(mktemp /tmp/memory-v1-v5-2-evidence-context-entity-timers.XXXXXX)
table_list=$(mktemp /tmp/memory-v1-v5-2-evidence-context-entity-tables.XXXXXX)
...
artifact_dir="$review_root/evidence-context-self-entity-production-$run_id"
status_file="$snapshot_root/memory_v1_v5_2_evidence_context_self_entity_${run_id}.status"
...
phase=backup
backup_partial="$snapshot_root/.memory_pre_v5_2_evidence_context_self_entity_${run_id}.dump.partial"
backup="$snapshot_root/memory_pre_v5_2_evidence_context_self_entity_${run_id}.dump"
...
value = {
    "contract_version": "memory_v1_v5_2_evidence_context_self_entity_production_report_v1",
...
printf 'report=%s\n' "$report"
printf 'backup=%s\n' "$backup"
printf 'memory_v1_v5_2_evidence_context_self_entity_apply_production: PASS\n'
