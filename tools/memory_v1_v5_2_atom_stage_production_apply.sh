...
# seebx backend only. Applies one hash-locked reviewed V5.2 atom stage plan,
...
if [[ "${MEMORY_V1_V5_2_ATOM_STAGE_PRODUCTION_APPLY:-}" != authorized ]]; then
  echo 'MEMORY_V1_V5_2_ATOM_STAGE_PRODUCTION_APPLY=authorized is required' >&2
  exit 1
fi
if [[ "$#" -ne 3 ]]; then
  echo 'usage: memory_v1_v5_2_atom_stage_production_apply.sh PLAN AUTHORIZATION STAGE_REPORT' >&2
  exit 2
fi
...
lock_file=/home/ubuntu/brains/.memory_v1_v5_2_atom_stage_apply.lock
phase=initialization
run_tag=
status_file=
units_quiesced=0
unit_state=$(mktemp /tmp/memory-v1-v5-2-atom-stage-apply-units.XXXXXX)
table_list=$(mktemp /tmp/memory-v1-v5-2-atom-stage-apply-tables.XXXXXX)
...
[[ "$(jq -er '.contract_version' "$plan")" == memory_v1_v5_2_stage_batch_plan_v1 ]]
...
status_file="$snapshot_dir/memory_v1_v5_2_atom_stage_apply_${run_tag}.status"
target_before="$snapshot_dir/memory_v1_v5_2_atom_stage_target_before_${run_tag}.tsv"
target_after="$snapshot_dir/memory_v1_v5_2_atom_stage_target_after_${run_tag}.tsv"
non_target_before="$snapshot_dir/memory_v1_v5_2_atom_stage_non_target_before_${run_tag}.tsv"
non_target_after="$snapshot_dir/memory_v1_v5_2_atom_stage_non_target_after_${run_tag}.tsv"
...
phase=backup
backup_partial="$snapshot_dir/.memory_pre_v5_2_atom_stage_${run_tag}.dump.partial"
backup="$snapshot_dir/memory_pre_v5_2_atom_stage_${run_tag}.dump"
...
[[ "$(psql_row "
  ...
  WHERE owner_user_id='$target_owner'::uuid
    AND evidence_id='22bd0732-3539-4180-8f89-8f84114131c0'::uuid
")" == 0 ]]
...
phase=report
report="$snapshot_dir/memory_v1_v5_2_atom_stage_apply_${run_tag}.json"
...
jq -n \
  ...
  '{contract_version:"memory_v1_v5_2_atom_stage_production_apply_report_v1",
    ...
    hard_stop:"before_entity_resolution_apply_or_claims_or_projection_or_live_retrieval"}' \
  >"$report"
chmod 0600 "$report"
...
phase=complete
printf '%s\n' 'memory_v1_v5_2_atom_stage_production_apply: PASS'
