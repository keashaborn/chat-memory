...
compose=(docker compose -p memoryv1deferredscanaudit -f docker-compose.ci.yml)
reconciliation_migration=ops/sql/20260716_memory_v1_deferred_entailment_reconciliation_v5.sql
scanner_migration=ops/sql/20260716_memory_v1_deferred_reconciliation_scanner_v5.sql
...
backup=$(mktemp /tmp/memory-v1-deferred-scan-audit.XXXXXX.dump)
...
[[ $("${compose[@]}" exec -T postgres psql -X -A -t -U sage -d memory \
  -c "SELECT (
    ...
    AND NOT has_table_privilege(
      'brains_app','memory.deferred_reconciliation_scan_run_v5','SELECT'
    )
...
echo 'memory_v1_deferred_scan_audit_v5_production_clone: PASS'
