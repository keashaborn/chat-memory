...
# relationship projection plan. It creates four manual-review staging rows and
# no claim, vector, retrieval, or prompt-influence state.

if [[ "${MEMORY_V1_RELATIONSHIP_CLAIM_V5_1_STAGE_APPLY:-}" != authorized ]]; then
  echo 'MEMORY_V1_RELATIONSHIP_CLAIM_V5_1_STAGE_APPLY=authorized is required' >&2
  exit 1
fi
if [[ "$#" -ne 4 ]]; then
  echo 'usage: relationship_claim_stage_v5_1_production_apply.sh BUNDLE PREFLIGHT APPLY REPLAY' >&2
  exit 2
fi

repo_root=$(git rev-parse --show-toplevel)
required_ancestor=7843b206142eb0e2a7d9a048a49425bebdce7183
runner=scripts/memory_v1_relationship_claim_stage_v5_1.py
projector=scripts/memory_v1_relationship_claim_projection_v5_1.py
expected_runner_sha=d9fd78fa9b6f85b0c00dedb594ccd1f8d7b2ce41009c3720568b0b7a55a0982e
expected_projector_sha=176ac995b0d93622a434c99f3a0ed7188769e2e526163c6f00547dc91ef77833
bundle=$(realpath "$1")
...
lock_file=/home/ubuntu/brains/.memory_v1_relationship_claim_v5_1_stage.lock
...
brains_quiesced=0
brains_state_before=
unit_state=$(mktemp /tmp/memory-v1-relationship-v5-1-stage-units.XXXXXX)
table_list=$(mktemp /tmp/memory-v1-relationship-v5-1-stage-tables.XXXXXX)
...
[[ "$(jq -er '.predicate' "$bundle")" == relationship.parent_of ]]
...
status_file="$snapshot_dir/memory_v1_relationship_claim_v5_1_stage_${run_id}.status"
...
phase=backup
backup_partial="$snapshot_dir/.memory_pre_relationship_claim_v5_1_stage_${run_id}.dump.partial"
...
cross_owner_error="$snapshot_dir/memory_v1_relationship_claim_v5_1_stage_cross_owner_${run_id}.log"
...
SELECT * FROM memory.preflight_relationship_claim_packet_v5_1(
  :'plan'::uuid, :'packet'
);
ROLLBACK;
SQL
then
  echo 'cross-owner relationship packet was accepted' >&2
  exit 1
fi
...
phase=report
report="$snapshot_dir/memory_v1_relationship_claim_v5_1_stage_${run_id}.json"
...
report = {
    "contract_version": "memory_v1_relationship_claim_v5_1_stage_report_v1",
...
phase=complete
printf 'backup=%s\nreport=%s\n' "$backup" "$report"
printf 'memory_v1_relationship_claim_stage_v5_1_production_apply: PASS\n'
