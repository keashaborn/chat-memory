...
# exact compiler-failure retry. No table data is intentionally changed; the
# security suite runs inside rollback transactions.

if [[ "${MEMORY_V1_V5_LOCAL_COMPILER_RETRY_V2_INSTALL:-}" != authorized ]]; then
  echo 'MEMORY_V1_V5_LOCAL_COMPILER_RETRY_V2_INSTALL=authorized is required' >&2
  exit 1
fi
...
lock_file=/home/ubuntu/brains/.memory_v1_v5_local_compiler_retry_v2.lock
phase=initialization
run_tag=
status_file=
units_quiesced=0
unit_state=$(mktemp /tmp/memory-v1-v5-local-compiler-retry-v2-units.XXXXXX)
table_list=$(mktemp /tmp/memory-v1-v5-local-compiler-retry-v2-tables.XXXXXX)
...
status_file="$snapshot_dir/memory_v1_v5_local_compiler_retry_v2_${run_tag}.status"
before="$snapshot_dir/memory_v1_v5_local_compiler_retry_v2_before_${run_tag}.tsv"
after="$snapshot_dir/memory_v1_v5_local_compiler_retry_v2_after_${run_tag}.tsv"
log="$snapshot_dir/memory_v1_v5_local_compiler_retry_v2_${run_tag}.log"
...
phase=backup
backup_partial="$snapshot_dir/.memory_pre_v5_local_compiler_retry_v2_${run_tag}.dump.partial"
backup="$snapshot_dir/memory_pre_v5_local_compiler_retry_v2_${run_tag}.dump"
...
phase=compiler_retry_v2_install
...
[[ "$(psql_scalar "SELECT (
  ...
  AND (SELECT prosecdef
       FROM pg_proc
       WHERE oid='memory.requeue_owner_local_compiler_failure_v2(uuid,uuid,text,uuid,uuid,integer,text,text,text)'::regprocedure)
...
phase=report
report="$snapshot_dir/memory_v1_v5_local_compiler_retry_v2_${run_tag}.json"
jq -n \
  ...
  '{contract_version:"memory_v1_v5_local_compiler_retry_v2_install_report_v1",
    ...
    checks:{fresh_backup:true,additive_function_only:true,
      owner_scoped_compiler_retry:true,cross_owner_rejection:true,
    ...
    hard_stop:"before_exact_compiler_retry_or_any_promotion_or_live_retrieval"}' >"$report"
chmod 0600 "$report"
sha256sum "$report" >"$report.sha256"
chmod 0600 "$report.sha256"

phase=complete
printf 'memory_v1_v5_local_compiler_retry_v2_production_install: PASS\n'
printf 'report=%s\nbackup=%s\n' "$report" "$backup"
