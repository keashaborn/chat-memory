...
"${compose[@]}" run --rm --no-deps \
  -e PYTHONPATH=/app \
  brains python scripts/memory_v1_intent_test.py

"${compose[@]}" run --rm --no-deps \
  -e PYTHONPATH=/app \
  brains python scripts/memory_v1_preference_project_shadow_test.py
...
"${compose[@]}" run --rm --no-deps \
  -e PYTHONPATH=/app \
  brains python scripts/memory_v1_preference_project_extraction_test.py

"${compose[@]}" run --rm --no-deps \
  -e POSTGRES_DSN=postgresql://sage:ci_only_postgres_password@postgres:5432/memory \
  -e PYTHONPATH=/app \
  brains python scripts/memory_v1_preference_project_apply_integration.py
...
"${compose[@]}" run --rm --no-deps \
  -e POSTGRES_DSN=postgresql://sage:ci_only_postgres_password@postgres:5432/memory \
  -e PYTHONPATH=/app \
  brains python scripts/memory_v1_preference_project_durable_apply_integration.py

"${compose[@]}" run --rm --no-deps \
  -e PYTHONPATH=/app \
  brains python scripts/memory_v1_preference_project_retrieval_test.py
