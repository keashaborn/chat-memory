...
# seebx backend only. Copies exactly 19 validated reviewed-claim vectors from
...
if [[ "${MEMORY_V1_V5_2_REVIEWED_CLAIM_LIVE_ACTIVATION:-}" != authorized ]]; then
  echo 'MEMORY_V1_V5_2_REVIEWED_CLAIM_LIVE_ACTIVATION=authorized is required' >&2
  exit 1
fi
...
runner="$repo_root/scripts/memory_v1_v5_2_reviewed_claim_projection_activate.py"
...
lock_file=/home/ubuntu/brains/.memory_v1_v5_2_reviewed_claim_live_activation.lock
...
brains_quiesced=0
brains_state_before=
timer_state=$(mktemp /tmp/memory-v1-reviewed-live-timers.XXXXXX)
table_list=$(mktemp /tmp/memory-v1-reviewed-live-tables.XXXXXX)
...
artifact_dir="$review_root/reviewed-claim-projection-live-$run_id"
status_file="$snapshot_root/memory_v1_v5_2_reviewed_claim_projection_live_${run_id}.status"
...
phase=backup
backup_partial="$snapshot_root/.memory_pre_reviewed_claim_live_${run_id}.dump.partial"
backup="$snapshot_root/memory_pre_reviewed_claim_live_${run_id}.dump"
...
MEMORY_V1_V5_2_REVIEWED_CLAIM_ACTIVATION=authorized \
MEMORY_V1_V5_2_REVIEWED_CLAIM_LIVE_ACTIVATION=authorized \
...
[[ "$(jq -er '.postgres_rows_mutated' "$apply")" == 57 ]]
...
phase=replay
MEMORY_V1_REQUIRED_ANCESTOR="$required_ancestor" \
MEMORY_V1_V5_2_REVIEWED_CLAIM_ACTIVATION_REPLAY=authorized \
MEMORY_V1_V5_2_REVIEWED_CLAIM_LIVE_ACTIVATION=authorized \
PYTHONPATH="$repo_root/scripts:$repo_root" \
...
jq -n \
  ...
  '{
    contract_version:"memory_v1_v5_2_reviewed_claim_projection_live_report_v1",
    head_commit:$head,
    ...
    claims_activated:19,
    new_live_points:19,
    replaced_live_points:0,
    embedding_requests:0,
    external_model_calls:0,
    qdrant_writes:19,
    exact_outbox_rows:19,
...
printf 'memory_v1_v5_2_reviewed_claim_projection_production: PASS\n'
