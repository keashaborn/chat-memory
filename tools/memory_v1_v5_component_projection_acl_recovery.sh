...
# seebx backend only. Repairs one missing function EXECUTE grant after the
# component/projection schema install, proves zero row/vector changes, and
# stops before component registration, runtime deployment, or retrieval.

if [[ $# -ne 2 ]]; then
  echo "usage: $0 RECOVERY_PLAN.json AUTHORIZATION.json" >&2
  exit 2
fi
if [[ "${MEMORY_V1_COMPONENT_PROJECTION_ACL_RECOVERY:-}" != "authorized" ]]; then
  echo "MEMORY_V1_COMPONENT_PROJECTION_ACL_RECOVERY=authorized is required" >&2
  exit 1
fi
...
expected_plan="$repo_root/ops/manifests/memory_v1_v5_component_projection_acl_recovery_plan_20260717.json"
verifier="$repo_root/scripts/memory_v1_v5_component_projection_acl_recovery_plan.py"
container=brains-postgres-1
database=memory
database_role=sage
snapshot_dir=/home/ubuntu/brains/snapshots
lock_file=/home/ubuntu/brains/.memory_v1_v5_component_projection_acl_recovery.lock
...
[[ "$plan" == "$expected_plan" ]] || {
  echo "recovery plan must be the committed canonical plan" >&2
  exit 1
}
...
if [[ -n "$(git -C "$repo_root" status --porcelain)" ]]; then
  echo "ACL recovery requires a clean Git worktree" >&2
  exit 1
fi
...
exec 9>"$lock_file"
flock -n 9 || {
  echo "another ACL recovery holds $lock_file" >&2
  exit 1
}
umask 077
...
status_file="$snapshot_dir/memory_v1_v5_component_projection_acl_recovery_${run_id}.status"
unit_state_before="$snapshot_dir/memory_v1_v5_component_projection_acl_units_before_${run_id}.tsv"
...
phase=preflight
preflight="$snapshot_dir/memory_v1_v5_component_projection_acl_preflight_${run_id}.json"
python3 "$verifier" --manifest "$plan" --repo-root "$repo_root" \
...
baseline_tables="$snapshot_dir/memory_v1_v5_component_projection_acl_tables_${run_id}.txt"
baseline_state="$snapshot_dir/memory_v1_v5_component_projection_acl_before_${run_id}.tsv"
post_state="$snapshot_dir/memory_v1_v5_component_projection_acl_after_${run_id}.tsv"
psql_scalar "
  SELECT table_name
...
phase=backup
backup_partial="$snapshot_dir/.memory_pre_v5_component_projection_acl_${run_id}.dump.partial"
backup="$snapshot_dir/memory_pre_v5_component_projection_acl_${run_id}.dump"
...
# Revalidate the short-lived authorization immediately before the sole write.
python3 "$verifier" --manifest "$plan" --repo-root "$repo_root" \
  --authorization "$authorization" >/dev/null

phase=acl_recovery
recovery_log="$snapshot_dir/memory_v1_v5_component_projection_acl_${run_id}.log"
: >"$recovery_log"
printf 'INSTALL %s\n' "${migrations[0]}" >>"$recovery_log"
run_sql_file "${migrations[0]}" >>"$recovery_log" 2>&1
chmod 0600 "$recovery_log"

phase=rolled_back_security_tests
for test_file in "${tests[@]}"; do
  printf 'TEST %s\n' "$test_file" >>"$recovery_log"
  run_sql_file "$test_file" >>"$recovery_log" 2>&1
done
...
cmp -s "$baseline_state" "$post_state" || {
  ...
  echo "memory rows changed during ACL recovery" >&2
  exit 1
}
qdrant_after=$(qdrant_signature)
[[ "$qdrant_before" == "$qdrant_after" ]] || {
  echo "Qdrant memory_claim_v1 changed during ACL recovery" >&2
  exit 1
}
...
")
[[ "$postflight_ok" == "1" ]] || { echo "ACL recovery postflight failed" >&2; exit 1; }

phase=restore_timer_state
restore_units
unit_state_after="$snapshot_dir/memory_v1_v5_component_projection_acl_units_after_${run_id}.tsv"
: >"$unit_state_after"
while IFS=$'\t' read -r unit _enabled _active; do
  printf '%s\t%s\t%s\n' "$unit" \
    "$(systemctl is-enabled "$unit")" "$(systemctl is-active "$unit")" \
...
phase=report
report="$snapshot_dir/memory_v1_v5_component_projection_acl_${run_id}.json"
PLAN="$plan" AUTHORIZATION="$authorization" PREFLIGHT="$preflight" \
  BACKUP="$backup" BACKUP_SHA="$checksum" CATALOG="$catalog" \
  RECOVERY_LOG="$recovery_log" BASELINE="$baseline_state" POST="$post_state" \
...
report = {
    "contract_version": "memory_v1_v5_component_projection_acl_recovery_report_v1",
    ...
    "backup_catalog": os.environ["CATALOG"],
    "preflight_sha256": sha(os.environ["PREFLIGHT"]),
    "recovery_log_sha256": sha(os.environ["RECOVERY_LOG"]),
    "memory_state_before_sha256": sha(os.environ["BASELINE"]),
    "memory_state_after_sha256": sha(os.environ["POST"]),
    "timer_state_before_sha256": sha(os.environ["UNIT_BEFORE"]),
    "timer_state_after_sha256": sha(os.environ["UNIT_AFTER"]),
    "qdrant_before_sha256": os.environ["QDRANT_BEFORE"],
    "qdrant_after_sha256": os.environ["QDRANT_AFTER"],
    "grant_added": "EXECUTE memory.v5_project_scope_valid(jsonb) to memory_v5_extraction_maintainer",
...
phase=complete
printf 'component_projection_acl_recovery=PASS\n'
printf 'report=%s\n' "$report"
printf 'backup=%s\n' "$backup"
