...
# seebx backend only. Applies a mixed V5.2 review batch to a disposable clone.
# It never writes production, claims, Qdrant, retrieval, or prompts.

if [[ "$#" -ne 3 ]]; then
  echo 'usage: production_clone.sh MANIFEST APPLY_RESULT REPLAY_RESULT' >&2
  exit 2
fi
...
source_db=memory
clone_db="memory_v5_2_projection_review_$(date -u +%Y%m%d%H%M%S)_$$"
...
before_review=$(docker exec "$container" psql -X -A -t -U sage -d "$clone_db" -c \
  "SELECT count(*) FROM memory.projection_review")
before_apply=$(docker exec "$container" psql -X -A -t -U sage -d "$clone_db" -c \
  "SELECT count(*) FROM memory.projection_apply_event")
before_claim=$(docker exec "$container" psql -X -A -t -U sage -d "$clone_db" -c \
  "SELECT count(*) FROM memory.claim")

POSTGRES_DSN="$clone_dsn" MEMORY_V1_REQUIRED_HEAD="$head" \
MEMORY_V1_V5_2_PROJECTION_REVIEW_APPLY=authorized \
PYTHONPATH="$repo_root/scripts" "$python_bin" "$repo_root/$runner" \
...
after_review=$(docker exec "$container" psql -X -A -t -U sage -d "$clone_db" -c \
  "SELECT count(*) FROM memory.projection_review")
after_apply=$(docker exec "$container" psql -X -A -t -U sage -d "$clone_db" -c \
  "SELECT count(*) FROM memory.projection_apply_event")
after_claim=$(docker exec "$container" psql -X -A -t -U sage -d "$clone_db" -c \
  "SELECT count(*) FROM memory.claim")
...
printf 'memory_v1_v5_2_projection_review_production_clone: PASS\n'
