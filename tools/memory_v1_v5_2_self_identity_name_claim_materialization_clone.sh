...
# seebx backend only. Verifies the single reviewed self-name claim on a
# disposable production clone. It does not write production or Qdrant.

if [[ "$#" -ne 4 ]]; then
  echo 'usage: self_identity_name_claim_materialization_clone.sh MANIFEST PREFLIGHT APPLY REPLAY' >&2
  exit 2
fi
...
exec env \
  ...
  "$repo_root/tools/memory_v1_v5_2_compiler_v8_claim_materialization_clone.sh" \
  "$@"
