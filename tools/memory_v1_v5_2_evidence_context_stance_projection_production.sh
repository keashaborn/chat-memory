...
# seebx backend only. Admits and projects the exact reviewed evidence-context
# reported-stance claim, then
...
if [[ "${MEMORY_V1_V5_2_EVIDENCE_CONTEXT_STANCE_PROJECTION:-}" != authorized ]]; then
  echo 'MEMORY_V1_V5_2_EVIDENCE_CONTEXT_STANCE_PROJECTION=authorized is required' >&2
  exit 1
fi
...
project_runner=scripts/memory_v1_v5_2_evidence_context_stance_controlled_project.py
python_bin="$repo_root/venv/bin/python"
review_root=/home/ubuntu/memory-v1-reviews
snapshot_root=/home/ubuntu/brains/snapshots
lock_file=/home/ubuntu/brains/.memory_v1_v5_2_evidence_context_stance_projection.lock
...
[[ "$(jq -er '.projection.maximum_embedding_requests' "$plan")" == 1 ]]
...
artifact_dir="$review_root/evidence-context-stance-projection-$run_id"
status_file="$snapshot_root/memory_v1_v5_2_evidence_context_stance_projection_${run_id}.status"
...
phase=backup
backup_partial="$snapshot_root/.memory_pre_evidence_context_stance_projection_${run_id}.dump.partial"
backup="$snapshot_root/memory_pre_evidence_context_stance_projection_${run_id}.dump"
...
[[ "$(jq -er '.rows_written' "$admission")" == 1 ]]
...
[[ "$(jq -er '.embedding_requests' "$project")" == 1 ]]
[[ "$(jq -er '.automatic_http_retries' "$project")" == 0 ]]
[[ "$(jq -er '.qdrant_writes' "$project")" == 1 ]]
[[ "$(jq -er '.shadow_tests|length' "$project")" == 1 ]]
[[ "$(jq -er '.shadow_tests[0].predicate' "$project")" == stance.reported ]]
[[ "$(jq -er '[.shadow_tests[]|select(
  ...
  .prompt_influence==false)]|length' "$project")" == 1 ]]
...
[[ "$(jq -er '[.completed[]|select(.replay_verified==true)]|length' \
  "$project_replay")" == 1 ]]
[[ "$(jq -er '.shadow_tests|length' "$project_replay")" == 1 ]]
[[ "$(jq -er '.shadow_tests[0].predicate' "$project_replay")" == stance.reported ]]
[[ "$(jq -er '[.shadow_tests[]|select(
  .other_owner_target_present==false and
  .other_owner_database_record_count==0 and .selected_count>=1 and
  .prompt_influence==false)]|length' "$project_replay")" == 1 ]]
...
jq -n \
  ...
  '{
    contract_version:"memory_v1_v5_2_evidence_context_stance_projection_report_v1",
    head_commit:$head,
...
printf 'memory_v1_v5_2_evidence_context_stance_projection_production: PASS\n'
