...
# seebx backend only. Applies one exact trusted-self resolution and binds one
# observation. No entity, review, claim, projection, Qdrant, retrieval, or
# prompt-influence changes are permitted.

if [[ "${MEMORY_V1_V5_2_SELF_IDENTITY_NAME_ENTITY_PRODUCTION:-}" != authorized ]]; then
  echo 'MEMORY_V1_V5_2_SELF_IDENTITY_NAME_ENTITY_PRODUCTION=authorized is required' >&2
  exit 1
fi

repo_root=$(git rev-parse --show-toplevel)
required_ancestor=564bc1225cf604573ad4e5cdb7e70ecc6d8a1495
...
lock_file=/home/ubuntu/brains/.memory_v1_v5_2_self_identity_name_entity.lock
...
status_file=
run_id=
timer_state=$(mktemp /tmp/memory-v1-v5-2-self-name-entity-timers.XXXXXX)
table_list=$(mktemp /tmp/memory-v1-v5-2-self-name-entity-tables.XXXXXX)
...
artifact_dir="$review_root/self-identity-name-entity-production-$run_id"
status_file="$snapshot_root/memory_v1_v5_2_self_identity_name_entity_${run_id}.status"
...
phase=backup
backup_partial="$snapshot_root/.memory_pre_v5_2_self_identity_name_entity_${run_id}.dump.partial"
backup="$snapshot_root/memory_pre_v5_2_self_identity_name_entity_${run_id}.dump"
...
if POSTGRES_DSN="$POSTGRES_DSN" PYTHONPATH="$repo_root" \
  ...
  echo 'cross-owner self identity name entity plan unexpectedly passed' >&2
  exit 1
fi
...
[[ "$(jq -er '.database_rows_created' "$apply_result")" == 3 ]]
[[ "$(jq -er '.bindings_created' "$apply_result")" == 1 ]]
[[ "$(jq -er '.item_count' "$apply_result")" == 1 ]]
[[ "$(jq -er '.applied[0].operation' "$apply_result")" == auto_apply ]]
...
phase=postflight
[[ "$(psql_row "SELECT count(*) FROM memory.entity_resolution_apply
...
[[ "$(psql_row "SELECT count(*) FROM memory.entity_resolution_plan
  WHERE owner_user_id='$owner'::uuid
...
value = {
    "contract_version": "memory_v1_v5_2_self_identity_name_entity_production_report_v1",
...
printf 'report=%s\n' "$report"
printf 'backup=%s\n' "$backup"
printf 'memory_v1_v5_2_self_identity_name_entity_apply_production: PASS\n'
