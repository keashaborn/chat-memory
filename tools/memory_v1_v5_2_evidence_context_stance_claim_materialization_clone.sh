...
# seebx backend only. Verifies the exact reviewed reported-stance claim on a
# disposable production clone. It does not write production or Qdrant.

if [[ "$#" -ne 4 ]]; then
  echo 'usage: stance_claim_materialization_clone.sh MANIFEST PREFLIGHT APPLY REPLAY' >&2
  exit 2
fi
