...
# seebx backend only. Installs the reviewed-stage novelty guard while the
# production source remains at the authorized base. It does not admit records,
# deploy runtime code, or enable the recurring timer.

[[ "$EUID" -eq 0 ]]
[[ "${MEMORY_V1_V5_2_REVIEWED_STAGE_NOVELTY_INSTALL:-}" == authorized ]]

repo_root=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
cd "$repo_root"
[[ -z "$(GIT_OPTIONAL_LOCKS=0 git status --porcelain)" ]]
production_root=/opt/chat-memory
authorized_runtime_head=7a34b20403948fa2c23c2220b56808762c19a0da
...
lock_file=/home/ubuntu/brains/.memory_v1_v5_2_reviewed_stage_novelty_install.lock
...
compiler=scripts/memory_v1_relational_extraction_v5_local_provider.py
compiler_test=tests/test_memory_v1_semantic_compiler_v10.py
clone_test=tools/memory_v1_v5_2_reviewed_stage_novelty_guard_clone.sh
...
declare -A expected_sha256=(
  ["$migration"]="931ee4487ff4e9b9568e5cc829b13e5bf5eb55729cc46d879d96218b3937f274"
  ["$rollback"]="18bd918b52d6e398f5fb91bb3ea3f1b6833b8a96ce1284e7143627b4fb08382b"
  ...
  ["$compiler"]="889aeac64303887b5d29632836e5fb5294435ff1735594b5846884ba982e0c09"
  ["$compiler_test"]="93615593578ce462f4c071b5a4d9557714e1c11d1a20a8eed375fae8e5a3c936"
  ["$clone_test"]="5bd33729a1dadadb2f764e817f56074aa2c8c88aeac42e84f71711b905cf2f2a"
...
PYTHONPATH="$repo_root" /opt/chat-memory/venv/bin/python -m unittest \
  ...
  tests.test_memory_v1_v5_2_reviewed_observation_stage
bash -n "$clone_test"
...
[[ -f "$clone_report" ]]
jq -e '.clone_passed==true and .selected_records==2 and
  .database_rows_created==4 and .entailment_ready_observations==3 and
  .zero_write_replay==true and .cross_owner_rejected==true and
  .already_entailed_batches_excluded==true and
...
status_file="$snapshot_root/memory_v1_v5_2_reviewed_stage_novelty_install_${run_tag}.status"
report_file="$snapshot_root/memory_v1_v5_2_reviewed_stage_novelty_install_${run_tag}.json"
...
phase=fresh_backup
partial="$snapshot_root/.memory_pre_reviewed_stage_novelty_install_${run_tag}.dump.partial"
backup="$snapshot_root/memory_pre_reviewed_stage_novelty_install_${run_tag}.dump"
...
jq -e '.apply==false and .selected_records==2 and
...
async def main() -> None:
    ...
    try:
        ...
        if len(target) != 2 or target & other:
...
jq -n \
  --arg contract_version memory_v1_v5_2_reviewed_stage_novelty_install_report_v1 \
  ...
  '{contract_version:$contract_version,installed:true,
    ...
    live_selected_records:2,database_rows_created:0,model_calls:0,
