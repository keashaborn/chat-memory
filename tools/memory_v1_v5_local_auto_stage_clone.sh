...
# proves policy-only auto-stage admission, atomic staging, replay, isolation,
# append-only enforcement, tamper rejection, and zero claim/vector influence.

repo_root=$(git rev-parse --show-toplevel)
port=${MEMORY_V1_V5_LOCAL_AUTO_STAGE_CLONE_PORT:-55467}
export MEMORY_V1_STAGE_BATCH_CLONE_PORT="$port"
compose=(docker compose -p memoryv1v5localautostageclone \
...
backup=$(mktemp /tmp/memory-v1-v5-local-auto-stage.XXXXXX.dump)
work=$(mktemp -d /tmp/memory-v1-v5-local-auto-stage.XXXXXX)
...
printf '%s\n' 'memory_v1_v5_local_auto_stage_clone: PASS'
