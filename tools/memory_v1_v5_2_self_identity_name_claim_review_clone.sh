...
# seebx backend only. Restores production into a disposable clone, stages
# exactly one bound self identity.name claim candidate, records one review,
# proves replay/isolation, then removes the clone. Production data and Qdrant
# remain read-only.
...
port=${MEMORY_V1_V5_2_SELF_IDENTITY_NAME_CLAIM_CLONE_PORT:-55497}
export MEMORY_V1_STAGE_BATCH_CLONE_PORT="$port"
compose=(
  docker compose
  -p memoryv1v52selfidentitynameclaimclone
  -f docker-compose.ci.yml
  -f docker-compose.stage-batch-clone.yml
)
backup=$(mktemp /tmp/memory-v1-v5-2-self-identity-name-claim.XXXXXX.dump)
role_sql=$(mktemp /tmp/memory-v1-v5-2-self-identity-name-claim-roles.XXXXXX.sql)
...
value = {
    "contract_version": "memory_v1_v5_2_self_identity_name_claim_review_decisions_v1",
...
value = {
    "contract_version": "memory_v1_v5_2_self_identity_name_claim_review_clone_report_v1",
...
printf 'report=%s\n' "$report"
printf 'memory_v1_v5_2_self_identity_name_claim_review_clone: PASS\n'
