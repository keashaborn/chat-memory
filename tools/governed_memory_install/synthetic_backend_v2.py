...
class SyntheticBackendV2Error(RuntimeError):
...
class SyntheticScenarioV2:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise SyntheticBackendV2Error("synthetic_scenario_invalid")
        ...
        if self.target_step_id not in _PLAN_STEP_IDS:
            raise SyntheticBackendV2Error("synthetic_target_step_invalid")
...
class SyntheticBackendV2:
    ...
    def __init__(
        ...
        *,
        ...
        if _seal is not _CONSTRUCTION_SEAL or type(scenario) is not SyntheticScenarioV2:
            raise SyntheticBackendV2Error(
    ...
    @property
    def scenario(self) -> SyntheticScenarioV2:
    ...
    def append_journal(self, record: JournalRecord) -> None:
        if type(record) is not JournalRecord:
            raise SyntheticBackendV2Error("synthetic_journal_record_invalid")
    ...
    def _validate_step(step: PlanStep) -> None:
        if type(step) is not PlanStep or step.step_id not in _PLAN_STEP_IDS:
            raise SyntheticBackendV2Error("synthetic_step_invalid")


def _construct_exact_synthetic_backend_v2(
    scenario: SyntheticScenarioV2,
) -> SyntheticBackendV2:
    """Construct the sole backend accepted by the disposable harness."""

    return SyntheticBackendV2(scenario, _seal=_CONSTRUCTION_SEAL)
