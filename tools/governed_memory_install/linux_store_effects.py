...
POSTGRES_BIND: Final = "127.0.0.1:55437"
...
def _expected_rollback_resource_names(
    ...
    return {
        ...
        "network": ("network", "governed-memory-net-9a54cf123493-000006"),
        "qdrant_store_secret": (
            "secret_file",
...
class LinuxStoreHostOperations:
    ...
    def _pre_effect_substrate_snapshots(
        self,
    ) -> tuple[BoundResourceSnapshot, ...]:
        """Observe only resources that exist before the stores are started.

        PostgreSQL catalog and Qdrant API state are deliberately excluded:
        their fixed secret files, containers, volumes, and endpoints must be
        absent at I03, so those logical transports are not yet callable.  The
        exact host substrate plus fixed-port preflight is the closed proof that
        no managed logical store is reachable before the first effect.
        """
    ...
    def _observe_invariant(
        ...
        if request.profile is HostOperationProfile.VERIFY_LIVE_PREFLIGHT:
            try:
                ...
                resource_snapshots = self._pre_effect_substrate_snapshots()
