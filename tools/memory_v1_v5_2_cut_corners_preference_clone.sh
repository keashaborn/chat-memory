...
# seebx backend only. Exercises one exact work-style preference through
...
source_db=memory
clone_db="memory_v5_2_cut_corners_preference_$(date -u +%Y%m%d%H%M%S)_$$"
...
artifact_dir="/home/ubuntu/memory-v1-reviews/cut-corners-preference-clone-$(date -u +%Y%m%dT%H%M%SZ)-${head:0:12}"
    ...
    -U sage -d "$clone_db" -c "
      SELECT
        ...
        (SELECT count(*) FROM memory.projection_preference_payload
         WHERE owner_user_id='$owner'::uuid
           AND plan_id='$plan_id'::uuid
           AND preference_domain='work_style'
...
value = {
    "contract_version": "memory_v1_v5_2_cut_corners_preference_clone_report_v1",
...
printf 'report=%s\n' "$report"
printf 'memory_v1_v5_2_cut_corners_preference_clone: PASS\n'
