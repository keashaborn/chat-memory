...
# seebx backend only. Installs the restricted prompt-compiler retry
# compatibility and requeues exactly one owner-scoped attempt-2 job. This tool
# performs no model, packet, claim, Qdrant, retrieval, or prompt-influence write.

if [[ "${MEMORY_V1_V5_2_COMPACT_PROMPT_RETRY_APPLY:-}" != authorized ]]; then
  echo 'MEMORY_V1_V5_2_COMPACT_PROMPT_RETRY_APPLY=authorized is required' >&2
  exit 1
fi

repo_root=$(git rev-parse --show-toplevel)
migration=ops/sql/20260722_memory_v1_v5_local_incomplete_prompt_retry.sql
test_sql=tests/memory_v1_v5_local_incomplete_prompt_retry.sql
manifest=ops/manifests/memory_v1_v5_2_compact_prompt_retry_20260722.json
expected_migration_sha=3da909cb62e3bf5f545322bdc618c30077ba74b904fcc631cd614e89efab726e
expected_test_sha=f13549e05910435cb834de7ad8bef28d8a1ee64a3dcce032e47955c57eb5bafc
expected_manifest_sha=dc5010d1d582f103c65109ffe2ca19bb2d794469f9c17683e405d0abef917c44
required_ancestor=c8830aba4bdc02ac821e1634a8b7f748a6ea3e1f
...
prior_failure=67b31872-47a4-50da-9f2e-bfc0bf132dc0
local_completion=a3ed450a-0e9d-4199-89f0-968e070e52b4
operation=3bb56c63-7731-46bc-b5c4-b66a59c86a6a
expected_attempts=2
reason=bounded_prompt_compiler_upgrade
container=brains-postgres-1
database=memory
snapshot_dir=/home/ubuntu/brains/snapshots
lock_file=/home/ubuntu/brains/.memory_v1_v5_2_compact_prompt_retry.lock
phase=initialization
run_tag=
status_file=
units_quiesced=0
unit_state=$(mktemp /tmp/memory-v1-v5-2-compact-prompt-units.XXXXXX)
table_list=$(mktemp /tmp/memory-v1-v5-2-compact-prompt-tables.XXXXXX)
...
SELECT * FROM memory.requeue_owner_local_incomplete_failure_v1(
  '$operation','$job','$content_sha','$prior_failure','$local_completion',
  $expected_attempts,'$reason');
COMMIT;
SQL
  chmod 0600 "$output"
}
...
[[ "$(jq -r '.prior_failure_operation_id' "$repo_root/$manifest")" == "$prior_failure" ]]
[[ "$(jq -r '.local_completion_event_id' "$repo_root/$manifest")" == "$local_completion" ]]
[[ "$(jq -r '.expected_attempts' "$repo_root/$manifest")" == "$expected_attempts" ]]
[[ "$(jq -r '.reason_code' "$repo_root/$manifest")" == "$reason" ]]
[[ "$(jq -r '.requeue_operation_id' "$repo_root/$manifest")" == "$operation" ]]
[[ "$(jq -r '.canary_max_output_tokens' "$repo_root/$manifest")" == 4096 ]]
...
status_file="$snapshot_dir/memory_v1_v5_2_compact_prompt_retry_${run_tag}.status"
protected_before="$snapshot_dir/memory_v1_v5_2_compact_prompt_protected_before_${run_tag}.tsv"
protected_after="$snapshot_dir/memory_v1_v5_2_compact_prompt_protected_after_${run_tag}.tsv"
apply_output="$snapshot_dir/memory_v1_v5_2_compact_prompt_apply_${run_tag}.tsv"
replay_output="$snapshot_dir/memory_v1_v5_2_compact_prompt_replay_${run_tag}.tsv"

phase=preflight
[[ "$(psql_scalar "SELECT count(*) FROM memory.evidence_extraction_job
  WHERE owner_user_id='$owner'::uuid AND job_id='$job'::uuid
    AND evidence_content_sha256='$content_sha' AND status='skipped'
    AND attempts=$expected_attempts AND lease_token IS NULL
...
phase=backup
backup_partial="$snapshot_dir/.memory_pre_v5_2_compact_prompt_${run_tag}.dump.partial"
backup="$snapshot_dir/memory_pre_v5_2_compact_prompt_${run_tag}.dump"
...
[[ "$(grep -c "|pending|$expected_attempts|applied$" "$apply_output")" -eq 1 ]]
[[ "$(psql_scalar "SELECT count(*) FROM memory.evidence_extraction_job
  WHERE owner_user_id='$owner'::uuid AND job_id='$job'::uuid
    AND status='pending' AND attempts=$expected_attempts AND last_error IS NULL
...
[[ "$(grep -c "|pending|$expected_attempts|replayed$" "$replay_output")" -eq 1 ]]
[[ "$(psql_scalar 'SELECT count(*) FROM memory.evidence_extraction_event')" == "$((events_before+1))" ]]
...
phase=report
report="$snapshot_dir/memory_v1_v5_2_compact_prompt_retry_${run_tag}.json"
jq -n \
  ...
  '{contract_version:"memory_v1_v5_2_compact_prompt_retry_apply_report_v1",
...
phase=complete
printf 'memory_v1_v5_2_compact_prompt_retry_production_apply: PASS\n'
printf 'report=%s\n' "$report"
