...
if [[ "${MEMORY_V1_V5_2_PET_SPECIES_STAGE_PRODUCTION:-}" != authorized ]]; then
  echo 'MEMORY_V1_V5_2_PET_SPECIES_STAGE_PRODUCTION=authorized is required' >&2
  exit 1
fi
...
manifest_source=manifests/memory_v1_v5_2_pet_species_relational_stage_20260729.json
stage_runner=scripts/memory_v1_v5_2_stage_batch.py
authorizer=tests/memory_v1_v5_2_stage_batch_fixture.py
evidence_ids=(
  5e938057-f012-5e54-8b37-148f3cc3ab9c
  ca637d00-7ff6-5147-8f6b-a82386dbc1c1
  3eb237da-4eec-5fa4-83fb-51360356dba6
)
lock_file=/home/ubuntu/brains/.memory_v1_v5_2_pet_species_stage.lock
phase=initialization
run_tag=
status_file=
units_quiesced=0
unit_state=$(mktemp /tmp/memory-v1-v5-2-pet-species-stage-units.XXXXXX)
table_list=$(mktemp /tmp/memory-v1-v5-2-pet-species-stage-tables.XXXXXX)
...
expected = {
    ...
    "entity_mention": 6,
    "entity_resolution_plan": 6,
    "entity_resolution_candidate": 4,
    "observation": 11,
    "observation_temporal": 11,
...
if sum(expected.values()) != 44:
    raise SystemExit("target row budget is not 44")
PY
}
...
git merge-base --is-ancestor a62f82a5d19c4af2ed2a6bb1f9a6511aff5d0353 "$head"
hash_lock "$manifest_source" manifest_sha \
  649f54a3ee53e24adcd8ce457a800ac88d4d2ab525a37097fd8ec774fea875df
...
status_file="$snapshot_dir/memory_v1_v5_2_pet_species_stage_${run_tag}.status"
work="$review_root/pet-species-stage-production-$run_tag"
mkdir -m 0700 "$work"
...
phase=backup
backup_partial="$snapshot_dir/.memory_pre_v5_2_pet_species_stage_${run_tag}.dump.partial"
backup="$snapshot_dir/memory_pre_v5_2_pet_species_stage_${run_tag}.dump"
...
assert_equal stage_rows \
  "$(jq -r '.database_rows_created' "$work/stage-apply.json")" 44
...
jq -n \
  ...
  '{
    contract_version:"memory_v1_v5_2_pet_species_relational_stage_production_report_v1",
    completed_at:$completed_at,head_commit:$head_commit,
    owner_user_id:$owner_user_id,
    backup:{path:$backup,sha256:$backup_sha256},work_root:$work_root,
    rows:{stage_total:44,batches:3,mentions:6,resolutions:6,
      candidates:4,observations:11,temporals:11,operation_requests:3,
      entities:0,claims:0},
...
phase=complete
printf '%s\n' \
  'MEMORY_V1_V5_2_PET_SPECIES_RELATIONAL_STAGE_PRODUCTION=PASS' \
  "head=$head" \
  "report=$report" \
  "backup=$backup" \
