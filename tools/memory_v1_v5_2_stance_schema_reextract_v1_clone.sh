...
# owner-scoped stance-schema re-extraction path without production writes.
...
production=memory
clone="memory_v5_v5_2_stance_schema_reextract_v1_${$}"
...
[[ "$(docker exec "$container" psql -U sage -d "$clone" -X -Atqc \
  "SELECT to_regprocedure(
    'memory.plan_owner_v5_2_stance_schema_reextract_v1(uuid)') IS NULL
    AND to_regprocedure(
    'memory.enqueue_owner_v5_2_stance_schema_reextract_v1(uuid,uuid,uuid,uuid,text,text,text,text)') IS NULL")" == t ]]
[[ "$(docker exec "$container" psql -U sage -d "$clone" -X -Atqc \
