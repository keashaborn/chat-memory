...
# proves the generic reviewed-observation bridge. Production Postgres and
...
artifact_dir="/home/ubuntu/memory-v1-reviews/reviewed-stage-successor-clone-$(date -u +%Y%m%dT%H%M%SZ)"
...
jq -e '.selected_records == 3 and .database_rows_created == 0 and
...
jq -e '.selected_records == 3 and
  .outcome == "reviewed_observations_admitted" and
  .database_rows_created == 6 and .zero_write_replay_proved == true and
  .write_counts.packet_stage_admission == 3 and
  .write_counts.reviewed_stage_admission == 3 and
  .claims == 0 and .qdrant_writes == 0 and .prompt_influence == 0' \
  "$apply_json" >/dev/null
...
[[ $((packet_after-packet_before)) -eq 3 ]]
[[ $((reviewed_after-reviewed_before)) -eq 3 ]]
...
[[ "$(jq -r '.count' <<<"$observation_gate_report")" == 4 ]]
...
helper_absence=$(scalar "$clone_db" "SELECT (to_regprocedure('memory.v5_2_resolution_successor_source_v1(uuid)') IS NULL AND to_regprocedure('memory.owner_packet_stage_eligible_v1(uuid)') IS NULL)::text")
...
jq -n \
  --arg contract_version memory_v1_v5_2_reviewed_stage_successor_clone_report_v1 \
