...
# seebx backend only. Applies one hash-locked four-item owner-scoped claim
# projection preparation batch. It writes accepted entailment decisions and
# manual-review projection staging only; it does not materialize claims.

if [[ "${MEMORY_V1_CLAIM_PROJECTION_STAGE_BATCH_PRODUCTION_APPLY:-}" != authorized ]]; then
  echo 'MEMORY_V1_CLAIM_PROJECTION_STAGE_BATCH_PRODUCTION_APPLY=authorized is required' >&2
  exit 1
fi
if [[ "$#" -ne 4 ]]; then
  echo 'usage: stage_batch_production_apply.sh MANIFEST PREFLIGHT APPLY REPLAY' >&2
  exit 2
fi
...
runner=scripts/memory_v1_v5_claim_projection_stage_batch.py
lock_file=/home/ubuntu/brains/.memory_v1_claim_projection_stage_batch_apply.lock
...
if sum(expected.values()) != manifest["expected_new_rows"]:
    raise SystemExit("manifest row budget mismatch")
PY
}
...
[[ "$(jq -er '.contract_version' "$manifest")" == \
  memory_v1_claim_projection_stage_batch_manifest_v1 ]]
...
status_file="$snapshot_dir/memory_v1_claim_projection_stage_batch_${run_tag}.status"
target_before="$snapshot_dir/memory_v1_claim_projection_stage_target_before_${run_tag}.tsv"
target_preflight="$snapshot_dir/memory_v1_claim_projection_stage_target_preflight_${run_tag}.tsv"
target_after="$snapshot_dir/memory_v1_claim_projection_stage_target_after_${run_tag}.tsv"
target_replay="$snapshot_dir/memory_v1_claim_projection_stage_target_replay_${run_tag}.tsv"
non_target_before="$snapshot_dir/memory_v1_claim_projection_stage_non_target_before_${run_tag}.tsv"
non_target_preflight="$snapshot_dir/memory_v1_claim_projection_stage_non_target_preflight_${run_tag}.tsv"
non_target_after="$snapshot_dir/memory_v1_claim_projection_stage_non_target_after_${run_tag}.tsv"
non_target_replay="$snapshot_dir/memory_v1_claim_projection_stage_non_target_replay_${run_tag}.tsv"
...
phase=backup
backup_partial="$snapshot_dir/.memory_pre_claim_projection_stage_${run_tag}.dump.partial"
backup="$snapshot_dir/memory_pre_claim_projection_stage_${run_tag}.dump"
...
PYTHONPATH="$repo_root/scripts" /opt/chat-memory/venv/bin/python \
  "$repo_root/$runner" --mode apply --manifest "$manifest" \
  --output "$apply_result"
...
qdrant_replay=$(qdrant_signature)
...
manifest = json.loads(Path(os.environ["MANIFEST"]).read_text())
report = {
    "contract_version": "memory_v1_claim_projection_stage_batch_report_v1",
...
phase=complete
printf 'backup=%s\nreport=%s\n' "$backup" "$report"
printf 'memory_v1_v5_claim_projection_stage_batch_production_apply: PASS\n'
