...
if [[ "${MEMORY_V1_V5_2_PROJECTION_REVIEW_PRODUCTION_APPLY:-}" != authorized ]]; then
  echo 'MEMORY_V1_V5_2_PROJECTION_REVIEW_PRODUCTION_APPLY=authorized is required' >&2
  exit 1
fi
if [[ "$#" -ne 4 ]]; then
  echo 'usage: v5_2_projection_review_production_apply.sh MANIFEST PREFLIGHT APPLY REPLAY' >&2
  exit 2
fi
...
runner=scripts/memory_v1_v5_2_projection_review_batch.py
lock_file=/home/ubuntu/brains/.memory_v1_v5_2_projection_review_apply.lock
...
brains_quiesced=0
brains_state_before=
unit_state=$(mktemp /tmp/memory-v1-v5-2-projection-review-units.XXXXXX)
table_list=$(mktemp /tmp/memory-v1-v5-2-projection-review-tables.XXXXXX)
psql_row() {
...
[[ "$(jq -er '.contract_version' "$manifest")" == memory_v1_v5_2_projection_review_manifest_v1 ]]
...
status_file="$snapshot_dir/memory_v1_v5_2_projection_review_${run_tag}.status"
target_before="$snapshot_dir/memory_v1_v5_2_projection_review_target_before_${run_tag}.tsv"
target_preflight="$snapshot_dir/memory_v1_v5_2_projection_review_target_preflight_${run_tag}.tsv"
target_after="$snapshot_dir/memory_v1_v5_2_projection_review_target_after_${run_tag}.tsv"
target_replay="$snapshot_dir/memory_v1_v5_2_projection_review_target_replay_${run_tag}.tsv"
non_target_before="$snapshot_dir/memory_v1_v5_2_projection_review_non_target_before_${run_tag}.tsv"
non_target_preflight="$snapshot_dir/memory_v1_v5_2_projection_review_non_target_preflight_${run_tag}.tsv"
non_target_after="$snapshot_dir/memory_v1_v5_2_projection_review_non_target_after_${run_tag}.tsv"
non_target_replay="$snapshot_dir/memory_v1_v5_2_projection_review_non_target_replay_${run_tag}.tsv"
...
phase=backup
backup_partial="$snapshot_dir/.memory_pre_v5_2_projection_review_${run_tag}.dump.partial"
backup="$snapshot_dir/memory_pre_v5_2_projection_review_${run_tag}.dump"
...
psql "$POSTGRES_DSN" -X -q -v ON_ERROR_STOP=1 \
  -v probe_plan="$probe_plan" -v other_owner="$other_owner" <<'SQL'
BEGIN READ ONLY;
...
BEGIN
  BEGIN
    PERFORM * FROM memory.preflight_projection_review_v5(
      ...
      'deferred',
      'system',
      'memory_v1_v5_2_stance_review_20260724',
      'cross-owner isolation probe',
      '["cross_owner_isolation_probe"]'::jsonb
    );
...
MEMORY_V1_REQUIRED_HEAD="$head" MEMORY_V1_V5_2_PROJECTION_REVIEW_APPLY=authorized PYTHONPATH="$repo_root/scripts" /opt/chat-memory/venv/bin/python "$repo_root/$runner" --mode apply --manifest "$manifest" --output "$apply_result"
[[ "$(jq -er '.rows_written' "$apply_result")" == "$item_count" ]]
[[ "$(jq -er '.decision_counts.authorized' "$apply_result")" == 0 ]]
[[ "$(jq -er '.decision_counts.deferred' "$apply_result")" == 2 ]]
[[ "$(jq -er '.decision_counts.rejected' "$apply_result")" == 2 ]]
...
phase=report
report="$snapshot_dir/memory_v1_v5_2_projection_review_${run_tag}.json"
...
report={
 'contract_version':'memory_v1_v5_2_projection_review_report_v1',
 ...
 'verification':{'review_rows_written':int(os.environ['ITEM_COUNT']),'decision_counts':{'authorized':0,'deferred':2,'rejected':2},
   'zero_write_replay':True,'non_target_memory_unchanged':True,
   'claims_written':0,'projection_apply_events_written':0,'qdrant_sha256':os.environ['QDRANT'],'qdrant_unchanged':True,
   'account_isolation_verified':True,'postgres_container_healthy':True,'brains_healthy':True,
...
phase=complete
printf 'backup=%s\nreport=%s\n' "$backup" "$report"
printf 'memory_v1_v5_2_projection_review_production_apply: PASS\n'
