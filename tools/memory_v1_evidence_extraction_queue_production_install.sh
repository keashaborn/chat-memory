...
if [[ "${MEMORY_V1_EVIDENCE_EXTRACTION_QUEUE_INSTALL:-}" != "authorized" ]]; then
  echo "MEMORY_V1_EVIDENCE_EXTRACTION_QUEUE_INSTALL=authorized is required" >&2
  exit 1
fi
...
required_ancestor=25159c027abe05c468880066185b13141a558456
expected_migration_sha=bd0604cd1d3084dc41811cdcde4da466e99bfa6816dadc273fb729de26489e55
expected_rollback_sha=f04794897dddc6ef57f96dfdc75337c738d7861f0402394f30cc70da0f33dfce
expected_test_sha=5d053776b5568ea2a5e95ba7c53190246f93a5203c40fd88eace17a677707b12
expected_dispatcher_sha=fb77f1f6bc1eeea80629d2e28a4162cd28109b250422f56a6ef43eecbab35253
container=brains-postgres-1
database=memory
snapshot_dir=/home/ubuntu/brains/snapshots
lock_file=/home/ubuntu/brains/.memory_v1_evidence_extraction_queue_install.lock
...
exec 9>"$lock_file"
flock -n 9 || {
  echo "another extraction queue install holds the lock" >&2
  exit 1
}
umask 077
run_id="$(date -u +%Y%m%dT%H%M%SZ)_$(git -C "$repo_root" rev-parse --short=12 HEAD)"
status_file="$snapshot_dir/memory_v1_evidence_extraction_queue_${run_id}.status"
...
record_exit() {
  ...
  if [[ "$code" -ne 0 && "$migration_applied" -eq 1 ]]; then
    ...
    run_sql_file "$rollback" \
      >"$snapshot_dir/memory_v1_evidence_extraction_queue_rollback_${run_id}.log" \
      2>&1 || true
  fi
  printf 'run_id=%s\nphase=%s\nexit_code=%s\ncompleted_at=%s\n' \
...
phase=backup
partial="$snapshot_dir/.memory_pre_evidence_extraction_queue_${run_id}.dump.partial"
backup="$snapshot_dir/memory_pre_evidence_extraction_queue_${run_id}.dump"
...
baseline="$snapshot_dir/memory_v1_evidence_extraction_queue_baseline_${run_id}.tsv"
post="$snapshot_dir/memory_v1_evidence_extraction_queue_post_${run_id}.tsv"
capture_existing_state "$baseline"
qdrant_before=$(qdrant_signature)

phase=schema_install
log="$snapshot_dir/memory_v1_evidence_extraction_queue_install_${run_id}.log"
...
phase=production_dry_dispatch
dry_report="$snapshot_dir/memory_v1_evidence_extraction_queue_dry_${run_id}.json"
set -a
...
assert report["contract_version"] == "memory_v1_evidence_intake_dispatch_report_v1"
...
phase=report
report="$snapshot_dir/memory_v1_evidence_extraction_queue_${run_id}.json"
...
value = {
    "contract_version": "memory_v1_evidence_extraction_queue_install_report_v1",
    "completed_at": dt.datetime.now(dt.timezone.utc).isoformat().replace("+00:00", "Z"),
    "head_commit": os.environ["HEAD"],
    "backup": {"path": os.environ["BACKUP"], "catalog": os.environ["CATALOG"]},
    "dry_dispatch_report": os.environ["DRY_REPORT"],
    "evidence": {
    ...
    "checks": {
        "restricted_queue_role": True,
        "forced_rls": True,
        "atomic_dispatch_receipt_job_event": True,
        "direct_table_writes_denied": True,
        "cross_owner_access_denied": True,
        "production_dry_dispatch_rows": 0,
        "queue_rows_written": 0,
        "event_rows_written": 0,
        "dispatch_receipts_written": 0,
...
migration_applied=0
phase=complete
printf 'memory_v1_evidence_extraction_queue_production_install: PASS\n'
printf 'report=%s\nbackup=%s\ndry_dispatch_report=%s\n' \
