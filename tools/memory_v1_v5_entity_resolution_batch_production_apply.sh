...
# seebx backend only. Applies one hash-locked owner-scoped V5 entity-resolution
...
if [[ "${MEMORY_V1_V5_ENTITY_BATCH_PRODUCTION_APPLY:-}" != authorized ]]; then
  echo 'MEMORY_V1_V5_ENTITY_BATCH_PRODUCTION_APPLY=authorized is required' >&2
  exit 1
fi
if [[ "$#" -ne 3 ]]; then
  echo 'usage: entity_resolution_batch_production_apply.sh PLAN AUTHORIZATION REPORT' >&2
  exit 2
fi
...
lock_file=/home/ubuntu/brains/.memory_v1_v5_entity_batch_apply.lock
phase=initialization
run_tag=
status_file=
units_quiesced=0
unit_state=$(mktemp /tmp/memory-v1-v5-entity-apply-units.XXXXXX)
table_list=$(mktemp /tmp/memory-v1-v5-entity-apply-tables.XXXXXX)
...
created = sum(item["operation"] == "approve_and_apply" for item in items)
expected = {
    "entity": created,
    "entity_alias_observation": created,
...
[[ "$(jq -er '.contract_version' "$plan")" == memory_v1_v5_entity_resolution_batch_plan_v1 ]]
...
status_file="$snapshot_dir/memory_v1_v5_entity_batch_apply_${run_tag}.status"
target_before="$snapshot_dir/memory_v1_v5_entity_batch_target_before_${run_tag}.tsv"
target_after="$snapshot_dir/memory_v1_v5_entity_batch_target_after_${run_tag}.tsv"
non_target_before="$snapshot_dir/memory_v1_v5_entity_batch_non_target_before_${run_tag}.tsv"
non_target_after="$snapshot_dir/memory_v1_v5_entity_batch_non_target_after_${run_tag}.tsv"
...
phase=backup
backup_partial="$snapshot_dir/.memory_pre_v5_entity_batch_${run_tag}.dump.partial"
backup="$snapshot_dir/memory_pre_v5_entity_batch_${run_tag}.dump"
...
phase=apply
MEMORY_V1_V5_ENTITY_RESOLUTION_BATCH_APPLY=authorized \
...
phase=report
report="$snapshot_dir/memory_v1_v5_entity_batch_apply_${run_tag}.json"
...
jq -n \
  ...
  '{contract_version:"memory_v1_v5_entity_batch_production_apply_report_v1",
...
phase=complete
printf '%s\n' 'memory_v1_v5_entity_resolution_batch_production_apply: PASS'
