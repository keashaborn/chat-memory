...
# one exact, reviewed work-style preference. It never writes claims or Qdrant
# and does not activate retrieval or prompt influence.

if [[ "${MEMORY_V1_V5_2_CUT_CORNERS_PREFERENCE_PRODUCTION:-}" != authorized ]]; then
  echo 'MEMORY_V1_V5_2_CUT_CORNERS_PREFERENCE_PRODUCTION=authorized is required' >&2
  exit 1
fi

repo_root=$(git rev-parse --show-toplevel)
required_ancestor=ee288d0f04a01b634f1a674c3c957f73a29d89a4
container=brains-postgres-1
database=memory
owner=1240822d-ac9a-4096-95aa-e2b24d36ef50
other_owner=557ea042-cb82-48f8-9429-472e96c957ef
evidence=51b03105-4acc-5ded-b703-e139a892db9d
observation=3f0bc967-211b-4d5b-9759-22fe7cc2525f
runner=scripts/memory_v1_v5_2_cut_corners_preference_pipeline.py
...
lock_file=/home/ubuntu/brains/.memory_v1_v5_2_cut_corners_preference.lock
...
artifact_dir="$review_root/cut-corners-preference-production-$run_id"
status_file="$snapshot_root/memory_v1_v5_2_cut_corners_preference_${run_id}.status"
...
phase=backup
backup_partial="$snapshot_root/.memory_pre_v5_2_cut_corners_preference_${run_id}.dump.partial"
backup="$snapshot_root/memory_pre_v5_2_cut_corners_preference_${run_id}.dump"
...
env "${common[@]}" "$python_bin" "$repo_root/$runner" apply \
  ...
  --confirm APPLY_ONE_OWNER_V5_2_CUT_CORNERS_PREFERENCE_ONLY \
  --output "$apply_result"
  ...
  SELECT concat_ws('|',
    ...
    (SELECT count(*) FROM memory.preference_revision_v5
     WHERE owner_user_id='$owner'::uuid
       ...
       AND value='{\"target\":\"cut corners\",\"context\":null}'::jsonb
       AND preference_polarity='dislikes'
...
value = {
    "contract_version": "memory_v1_v5_2_cut_corners_preference_production_report_v1",
...
printf 'report=%s\n' "$report"
printf 'backup=%s\n' "$backup"
printf 'memory_v1_v5_2_cut_corners_preference_production: PASS\n'
