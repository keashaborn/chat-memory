...
# seebx backend only. Exercises a bounded review apply and replay against a
...
[[ "$after_review" == "$((before_review+item_count))" ]]
...
BEGIN
  BEGIN
    PERFORM * FROM memory.preflight_projection_review_v5(
      current_setting('test.probe_plan')::uuid,'p01','authorized','system',
