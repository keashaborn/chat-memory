#!/usr/bin/env python3
"""Validate the separate governed function-creation registry and packages."""
...
from governed_migration import MigrationError, canonical_bytes, sha256
from validate_repository_migrations import ledger_inputs


REGISTRY_RELATIVE = "governed-migration-ci/registry/governed-function-creations-v1.json"
...
def _git(repository: pathlib.Path, arguments: list[str]) -> bytes:
    ...
    if result.returncode != 0 or len(result.stdout) > 8 * 1024 * 1024 or len(result.stderr) > 1024 * 1024:
        raise MigrationError("read-only function-creation registry Git operation failed")
...
def previous_registry(repository: pathlib.Path, commit: str) -> dict[str, dict[str, object]]:
    ...
    if len(parent) != 40:
        raise MigrationError("function-creation registry parent commit is malformed")
    ...
    if len(records) != 1 or not records[0].endswith(b"\t" + REGISTRY_RELATIVE.encode("ascii")):
        raise MigrationError("previous function-creation registry tree identity is malformed")
    return parse_function_creation_registry(
...
def package_directories(repository: pathlib.Path) -> list[pathlib.Path]:
    root = repository / "governed-function-creations"
    if not root.exists():
        return []
...
def main() -> int:
    ...
    packages = [load_function_creation_package(path, ledger) for path in package_directories(repository)]
    registry_path = root / "registry/governed-function-creations-v1.json"
    registry = parse_function_creation_registry(registry_path.read_bytes())
    validate_function_creation_registry_append_only(previous_registry(repository, args.repository_commit), registry)
    validate_function_creation_registry(packages, registry)
    report = {
        "schema_version": "governed-function-creation-repository-validation-v1",
        ...
        "table_migration_contract_modified": False,
        "function_replacement_contract_modified": False,
        "status": "valid",
    }
...
if __name__ == "__main__":
    try:
        ...
        print(
            "governed function-creation validation failed: "
            + type(error).__name__
            + ": "
            + str(error),
            file=sys.stderr,
