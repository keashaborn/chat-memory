...
"""Provider-neutral response-memory provenance contracts.

The serialized contract and hashing domain intentionally retain their deployed
legacy values. Renaming Python ownership must not invalidate stored receipts.
"""
...
MEMORY_PROVENANCE_DOMAIN = "governed_memory.response_provenance.v1"
_SHA256_HEX_LENGTH = 64


class MemoryResponseConfigurationError(RuntimeError):
    """The response-memory lane is missing or inconsistent."""


class MemoryContractError(ValueError):
    """A response-memory contract failed exact validation."""


def _require_uuid(value: object, code: str) -> UUID:
    if not isinstance(value, UUID):
        raise MemoryContractError(code)
    return value
...
def _canonical_value(value: Any) -> Any:
    ...
    if isinstance(value, str):
        if unicodedata.normalize("NFC", value) != value:
            raise MemoryContractError("invalid_memory_contract_unicode")
        return value
    if type(value) is int:
        return value
    if isinstance(value, float):
        raise MemoryContractError("float_not_allowed_in_memory_contract")
    ...
    if isinstance(value, Mapping):
        ...
        for key, item in value.items():
            if not isinstance(key, str):
                raise MemoryContractError("non_string_memory_contract_key")
            normalized[key] = _canonical_value(item)
        return normalized
    if isinstance(value, (tuple, list)):
        return [_canonical_value(item) for item in value]
    raise MemoryContractError("unsupported_memory_contract_value")


def _canonical_sha256(domain: str, material: Mapping[str, object]) -> str:
    payload = json.dumps(
        _canonical_value({"domain": domain, "material": material}),
        ensure_ascii=False,
        allow_nan=False,
        separators=(",", ":"),
        sort_keys=True,
...
class MemoryNotApplicableReason(str, Enum):
    NO_STORE = "no_store"
...
class MemoryAnswerReferenceV1(BaseModel):
...
class MemoryAnswerProvenanceV1(BaseModel):
    ...
    contract_version: Literal[MEMORY_PROVENANCE_CONTRACT_VERSION] = (
        MEMORY_PROVENANCE_CONTRACT_VERSION
    )
    ...
    not_applicable_reason: MemoryNotApplicableReason | None
    answer_id: UUID
    prompt_sha256: str
    ...
    references: tuple[MemoryAnswerReferenceV1, ...] = Field(max_length=8)
    provenance_sha256: str

    @field_validator("prompt_sha256", "outbound_request_sha256", "provenance_sha256")
    @classmethod
    def required_hash(cls, value: str) -> str:
        return _require_hash(value, "invalid_memory_provenance_sha256")
    ...
    def exact_receipt(self) -> "MemoryAnswerProvenanceV1":
        references = tuple(self.references)
        if tuple(item.rank for item in references) != tuple(
            range(1, len(references) + 1)
        ):
            raise ValueError("memory provenance ranks are not contiguous")
        if len({item.claim_id for item in references}) != len(references):
            raise ValueError("memory provenance references are duplicated")
        if len({item.revision_id for item in references}) != len(references):
            raise ValueError("memory provenance revisions are duplicated")
        if self.binding_outcome in {"not_applicable", "no_memory_selected"}:
            if (
                ...
                raise ValueError("no-selection memory provenance is not empty")
            if (
                ...
                raise ValueError("memory provenance reason is inconsistent")
        elif (
            ...
            raise ValueError("exposed memory provenance is inconsistent")
        expected = _canonical_sha256(
            MEMORY_PROVENANCE_DOMAIN,
            self._hash_material(),
        )
...
def _create_provenance(material: dict[str, object]) -> MemoryAnswerProvenanceV1:
    hash_material = {
        "contract_version": MEMORY_PROVENANCE_CONTRACT_VERSION,
        "provenance_basis": (
            "successor_dispatch_receipt_model_exposure_not_semantic_use"
        ),
        **material,
        ...
        "references": [
            (
                ...
                if isinstance(item, MemoryAnswerReferenceV1)
                else item
            )
            for item in material["references"]  # type: ignore[union-attr]
        ],
    }
    return MemoryAnswerProvenanceV1(
        **material,
...
def build_memory_no_selection_provenance_v1(
    *,
    answer_id: UUID,
    prompt_sha256: str,
    outbound_request_bytes: bytes,
) -> MemoryAnswerProvenanceV1:
    answer = _require_uuid(answer_id, "invalid_memory_provenance_answer")
    prompt = _require_hash(prompt_sha256, "invalid_memory_provenance_prompt_sha256")
    if (
        not isinstance(outbound_request_bytes, bytes)
        or not 1 <= len(outbound_request_bytes) <= 1_048_576
    ):
        raise MemoryContractError("invalid_memory_provenance_outbound_request")
    return _create_provenance(
        {
            ...
            "outbound_request_sha256": _sha256_bytes(outbound_request_bytes),
...
def build_memory_not_applicable_provenance_v1(
    *,
    reason: MemoryNotApplicableReason,
    answer_id: UUID,
    prompt_sha256: str,
    outbound_request_bytes: bytes,
) -> MemoryAnswerProvenanceV1:
    if not isinstance(reason, MemoryNotApplicableReason):
        raise MemoryContractError("invalid_memory_not_applicable_reason")
    value = build_memory_no_selection_provenance_v1(
        answer_id=answer_id,
...
def build_memory_exposed_provenance_v1(
    dispatched_binding: Mapping[str, object],
) -> MemoryAnswerProvenanceV1:
    if not isinstance(dispatched_binding, Mapping):
        raise MemoryContractError("invalid_memory_dispatched_binding")
    if (
    ...
    try:
        ...
        raise MemoryContractError("invalid_memory_provenance_answer") from exc
    selected_count = _require_exact_int(
        dispatched_binding.get("selected_count"),
        code="invalid_memory_provenance_selected_count",
        minimum=1,
        maximum=8,
    )
    injected_count = _require_exact_int(
        dispatched_binding.get("injected_count"),
        code="invalid_memory_provenance_injected_count",
        minimum=1,
        maximum=8,
    )
    exposed_count = _require_exact_int(
        dispatched_binding.get("model_exposed_count"),
        code="invalid_memory_provenance_exposed_count",
        minimum=1,
        maximum=8,
    )
    ...
    if (
        ...
        raise MemoryContractError("invalid_memory_provenance_lineage")
    ...
    try:
        ...
        raise MemoryContractError("invalid_memory_provenance_lineage") from exc
    binding_manifest = _require_hash(
        dispatched_binding.get("binding_sha256"),
        "invalid_memory_provenance_binding_sha256",
    )
    injection_manifest = _require_hash(
        dispatched_binding.get("injection_manifest_sha256"),
        "invalid_memory_provenance_injection_sha256",
    )
    if binding_manifest != injection_manifest:
        raise MemoryContractError("memory_provenance_binding_hash_mismatch")
    return _create_provenance(
        {
            "binding_outcome": "exposed",
            "not_applicable_reason": None,
            "answer_id": answer,
            "prompt_sha256": _require_hash(
                dispatched_binding.get("prompt_sha256"),
                "invalid_memory_provenance_prompt_sha256",
            ),
            "outbound_request_sha256": _require_hash(
                dispatched_binding.get("outbound_request_sha256"),
                "invalid_memory_provenance_outbound_sha256",
            ),
            ...
            "selection_manifest_sha256": _require_hash(
                ...
                "invalid_memory_provenance_selection_sha256",
            ),
