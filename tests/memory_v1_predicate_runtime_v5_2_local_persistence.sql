...
SELECT set_config('test.v5_2_job_id', :'claim_job_id', true);
SELECT set_config('test.v5_2_lease_token', :'claim_lease_token', true);
SELECT set_config(
  'test.v5_2_evidence_content_sha256',
  :'claim_evidence_content_sha256',
  true
);
SELECT set_config('test.v5_2_normalized_packet', :'normalized_packet', true);
...
BEGIN
  PERFORM * FROM memory.persist_owner_v5_local_packet_v1(
    ...
    current_setting('test.v5_2_job_id')::uuid,
    current_setting('test.v5_2_lease_token')::uuid,
    'local-inference-test',
    current_setting('test.v5_2_evidence_content_sha256'),
    ...
    current_setting('test.v5_2_normalized_packet')::jsonb,false,0
  );
  RAISE EXCEPTION 'V5 persistence accepted a V5.2 packet';
EXCEPTION WHEN check_violation THEN NULL;
END
$cross_profile_v5$;

DO $cross_profile_v5_2$
DECLARE
  legacy_packet jsonb;
BEGIN
  legacy_packet := jsonb_set(
    jsonb_set(
      current_setting('test.v5_2_normalized_packet')::jsonb,
      '{contract_version}',
  ...
  RAISE EXCEPTION 'V5.2 persistence accepted a V5 packet';
EXCEPTION WHEN check_violation THEN NULL;
END
$cross_profile_v5_2$;
...
FROM memory.persist_owner_v5_2_local_packet_v1(
  'fa600000-0000-4000-8000-000000000001',
  'fa600000-0000-4000-8000-000000000002',
  :'claim_job_id',:'claim_lease_token','local-inference-test',
  :'claim_evidence_content_sha256','v1',repeat('3',64),repeat('4',64),
...
SELECT 'memory_v1_predicate_runtime_v5_2_local_persistence: PASS' AS result;
