...
BEGIN
  IF to_regrole('memory_v5_local_reextract_maintainer') IS NULL
     OR to_regprocedure(
       'memory.plan_owner_v5_2_evidence_context_reextract_v1(uuid)'
     ) IS NULL
     OR to_regprocedure(
       'memory.enqueue_owner_v5_2_evidence_context_reextract_v1(uuid,uuid,uuid,uuid,text,text,text,text)'
     ) IS NULL THEN
    RAISE EXCEPTION 'local V5.2 evidence context reextract objects are absent';
  END IF;
  IF (SELECT rolcanlogin OR rolsuper OR rolcreatedb OR rolcreaterole
     ...
     OR NOT has_function_privilege(
       'brains_app',
       'memory.plan_owner_v5_2_evidence_context_reextract_v1(uuid)','EXECUTE'
     )
     OR NOT has_function_privilege(
       'brains_app',
       'memory.enqueue_owner_v5_2_evidence_context_reextract_v1(uuid,uuid,uuid,uuid,text,text,text,text)',
    ...
    RAISE EXCEPTION 'local V5.2 evidence context reextract ACL is unsafe';
  END IF;
END
$catalog$;
...
FROM memory.plan_owner_v5_2_evidence_context_reextract_v1(:'prior_packet'::uuid)
WHERE evidence_content_sha256=:'content_sha256'
  AND prior_packet_storage_sha256=:'packet_storage_sha256'
  AND reason_code='evidence_context_coreference_available'
  AND next_selector_version='20260726_v5_2_evidence_context_reextract_v1'
  AND next_policy_compiler_version='memory_v1_semantic_policy_compiler_v8';
...
WHERE owner_user_id=:'target_owner'::uuid
  ...
  AND actor_ref='local_v5_2_evidence_context_reextract_v1'
...
DO $cross_owner$
BEGIN
  PERFORM * FROM memory.enqueue_owner_v5_2_evidence_context_reextract_v1(
  ...
  RAISE EXCEPTION 'cross-owner V5.2 evidence context reextract unexpectedly succeeded';
...
SELECT 'memory_v1_v5_2_evidence_context_reextract_v1: PASS' AS result;
