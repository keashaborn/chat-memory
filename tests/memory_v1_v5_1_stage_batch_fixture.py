#!/usr/bin/env python3
"""Generate deterministic synthetic fixtures for the V5.1 stage-batch clone test."""
...
def secure_write(path: Path, value: dict[str, Any]) -> str:
    if value.get("contract_version") == "memory_v1_v5_1_stage_preflight_v1":
        value = dict(value)
        report_path = path.with_name(f"{path.stem}-source-review.json")
        report_sha = secure_write(
            report_path,
            {
                "contract_version": "memory_v1_v5_1_stage_fixture_review_v1",
        ...
        value["schemas"] = {
            "extraction_sha256": digest(
                (ROOT / "specs/memory_v1_relational_extraction_v5_1.schema.json").read_bytes()
            ),
            "resolution_sha256": digest(
                (ROOT / "specs/memory_v1_entity_resolution_review_v5_1.schema.json").read_bytes()
            ),
        }
...
def authorize(plan_path: Path, output: Path, head: str) -> None:
    ...
    value = {
        "contract_version": "memory_v1_v5_1_stage_batch_authorization_v1",
        ...
        "target_server": "seebx",
        "scope": "stage_reviewed_owner_v5_1_packets_only",
