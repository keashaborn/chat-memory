...
BEGIN
  IF has_table_privilege(
    ...
    RAISE EXCEPTION
      'semantic compiler-v9 re-extraction ACL changed';
  END IF;
END
$acl$;
...
BEGIN
  ...
  FROM memory.enqueue_owner_v5_2_semantic_compiler_v9_reextract_v1(
  ...
  IF first_result.apply_outcome<>'applied'
    ...
    RAISE EXCEPTION
      'semantic compiler-v9 re-extraction replay failed';
  END IF;
END
$apply_and_replay$;
...
BEGIN
  ...
  RAISE EXCEPTION
    'out-of-set semantic compiler-v9 re-extraction unexpectedly passed';
EXCEPTION
...
BEGIN
  ...
  RAISE EXCEPTION
    'cross-owner semantic compiler-v9 re-extraction unexpectedly passed';
EXCEPTION
  WHEN SQLSTATE '23514' THEN NULL;
END
$cross_owner$;
...
SELECT
  'memory_v1_v5_2_semantic_compiler_v9_reextract: PASS' AS result;
