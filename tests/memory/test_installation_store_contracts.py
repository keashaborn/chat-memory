...
STORE_SPEC = INSTALLATION / "store_spec-v5.json"
STORE_UNIT = INSTALLATION / "systemd" / "governed-memory-stores-v5.service.in"
BINDING = "a" * 64
NONCE_SHA256 = "b" * 64
EXECUTION_ID = "c" * 64
...
NETWORK_ID = "network-id-9a54cf123493-000005"
...
class DormantStoreInstallStorePackageTests(unittest.TestCase):
    ...
    def test_static_store_spec_and_pure_plan_are_exact_and_stores_only(self) -> None:
        ...
        self.assertEqual(
            spec["candidate_id"], "governed_memory_9a54cf123493_000005"
        )
        self.assertEqual(
            spec["docker_resource_slug"], "governed-memory-9a54cf123493-000005"
        )
        for step in plan:
    ...
    def test_store_spec_refuses_identity_environment_or_hardening_drift(self) -> None:
        ...
        wrong_candidate["candidate_id"] = "governed-memory-9a54cf123493-000005"
    ...
    def test_resolved_spec_and_ledger_bind_supervisor_identity(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            root.chmod(0o700)
            spec_path = root / "store_spec-v5.json"
    ...
    def test_store_unit_is_root_oneshot_unix_only_without_app_or_secret_loading(self) -> None:
        ...
        for required in (
            ...
            "executions-v5/@EXECUTION_ID@/resources.jsonl",
