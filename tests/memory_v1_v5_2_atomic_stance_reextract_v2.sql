...
BEGIN
  IF to_regrole('memory_v5_local_reextract_maintainer') IS NULL
     OR to_regprocedure(
       'memory.plan_owner_v5_2_atomic_stance_reextract_v2(uuid)'
     ) IS NULL
     OR to_regprocedure(
       'memory.enqueue_owner_v5_2_atomic_stance_reextract_v2(uuid,uuid,uuid,uuid,text,text,text,text)'
     ) IS NULL THEN
    RAISE EXCEPTION 'local V5.2 atomic stance reextract objects are absent';
  END IF;
  IF (SELECT rolcanlogin OR rolsuper OR rolcreatedb OR rolcreaterole
     ...
     OR NOT has_function_privilege(
       'brains_app',
       'memory.plan_owner_v5_2_atomic_stance_reextract_v2(uuid)','EXECUTE'
     )
     OR NOT has_function_privilege(
       'brains_app',
       'memory.enqueue_owner_v5_2_atomic_stance_reextract_v2(uuid,uuid,uuid,uuid,text,text,text,text)',
     ...
     OR strpos(
          ...
          'memory_v1_semantic_policy_compiler_v5'
        )=0
     OR NOT has_function_privilege(
       'brains_app',persistence,'EXECUTE'
     ) THEN
    RAISE EXCEPTION 'local V5.2 atomic stance reextract ACL is unsafe';
  END IF;
END
$catalog$;
...
FROM memory.plan_owner_v5_2_atomic_stance_reextract_v2(:'prior_packet'::uuid)
WHERE evidence_content_sha256=:'content_sha256'
  AND prior_packet_storage_sha256=:'packet_storage_sha256'
  AND reason_code='semantic_stance_atomic_split_required'
  AND next_selector_version='20260722_v5_2_atomic_stance_reextract_v2'
  AND next_policy_compiler_version='memory_v1_semantic_policy_compiler_v5';
...
WHERE owner_user_id=:'target_owner'::uuid
  ...
  AND actor_ref='local_v5_2_atomic_stance_reextract_v2'
...
DO $cross_owner$
BEGIN
  PERFORM * FROM memory.enqueue_owner_v5_2_atomic_stance_reextract_v2(
  ...
  RAISE EXCEPTION 'cross-owner V5.2 atomic stance reextract unexpectedly succeeded';
...
SELECT 'memory_v1_v5_2_atomic_stance_reextract_v2: PASS' AS result;
