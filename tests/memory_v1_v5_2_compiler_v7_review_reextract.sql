...
BEGIN
  IF has_table_privilege(
    ...
    RAISE EXCEPTION 'compiler-v7 review re-extraction ACL changed';
  END IF;
END
$acl$;
...
BEGIN
  ...
  FROM memory.enqueue_owner_v5_2_compiler_v7_review_reextract_v1(
  ...
  IF first_result.apply_outcome<>'applied'
    ...
    RAISE EXCEPTION 'compiler-v7 review re-extraction replay failed';
  END IF;
END
$apply_and_replay$;
...
BEGIN
  ...
  RAISE EXCEPTION 'cross-owner compiler-v7 re-extraction unexpectedly passed';
EXCEPTION
