...
from scripts.memory_v1_v5_1_stage_batch import (
    StageBatchError,
    apply_once,
...
from scripts.memory_v1_v5_1_stage_preflight import (
    _validate_extraction_packet as validate_v5_1_extraction,
)
from scripts.memory_v1_v5_stage_preflight import (
    _validate_extraction_packet as validate_v5_extraction,
)
...
def empty_bundle(owner: uuid.UUID = OWNER) -> dict:
    ...
    return {
        "contract_version": "memory_v1_v5_1_stage_preflight_v1",
...
class StageBatchTest(unittest.TestCase):
    ...
    def manifest(self, *, owner=OWNER, bundle_owner=OWNER, counts=None):
        ...
        manifest = {
            "contract_version": "memory_v1_v5_1_stage_batch_manifest_v1",
            "target_server": "seebx",
            "owner_user_id": str(owner),
            "bundles": [
                {
    ...
    def test_v5_and_v5_1_packet_lanes_are_mutually_exclusive(self):
        v5_1 = json.loads(empty_bundle()["extraction_packet_text"])
        validate_v5_1_extraction(v5_1)
        with self.assertRaisesRegex(RuntimeError, "contract version mismatch"):
            validate_v5_extraction(v5_1)

        v5 = dict(v5_1)
        v5["contract_version"] = "memory_v1_relational_extraction_v5"
        v5["predicate_registry_version"] = "memory_predicate_registry_v5"
        validate_v5_extraction(v5)
        with self.assertRaisesRegex(RuntimeError, "contract version mismatch"):
            validate_v5_1_extraction(v5)
    ...
    def test_apply_sets_actor_and_owner_lock(self):
        ...
        self.assertEqual(connection.calls[1][2], (f"{OWNER}|relational_stage_v5_1",))
