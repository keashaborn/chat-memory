...
SELECT * FROM memory.complete_owner_v5_local_inference_v1(
  ...
  'rejected',1,'local_validation_rejected',NULL,NULL,NULL
) \gset rejected_
...
SELECT * FROM memory.fail_owner_evidence_extraction_job_v1(
  ...
  'local_inference_rejected','local_validation_rejected',1
) \gset failed_b_
...
BEGIN
  PERFORM * FROM memory.requeue_owner_local_validation_failure_v1(
    ...
    1,'local_validation_rejected',
    'registry_enum_compiler_repair'
  );
  RAISE EXCEPTION 'cross-owner local validation retry unexpectedly succeeded';
EXCEPTION WHEN check_violation THEN NULL;
END
$cross_owner_retry$;
...
SELECT * FROM memory.requeue_owner_local_validation_failure_v1(
  ...
  :'rejected_event_id',1,'local_validation_rejected',
  'registry_enum_compiler_repair'
) \gset retried_b_
SELECT
...
SELECT 1/((apply_outcome='replayed')::integer)
FROM memory.requeue_owner_local_validation_failure_v1(
  'fb900000-0000-4000-8000-000000000001',:'claim_b_job_id',
  repeat('b',64),'fb800000-0000-4000-8000-000000000001',
  :'rejected_event_id',1,'local_validation_rejected',
  'registry_enum_compiler_repair'
...
SELECT 'memory_v1_v5_local_validation_retry: PASS' AS result;
