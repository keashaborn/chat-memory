...
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    ...
    EXECUTE format(
      'CREATE POLICY final_state_clone_owner_isolation ON memory.%I '
      'TO memory_v5_clone_writer '
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE schemaname = 'memory'
      AND policyname = 'final_state_clone_owner_isolation'
  ) THEN
    RAISE EXCEPTION 'final-state clone policy survived rollback';
  END IF;
END
$block$;

SELECT 'memory_v1_relational_staging_final_state_v5: PASS' AS result;
