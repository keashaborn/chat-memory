        ...
        async with conn.transaction():
            ...
            if supersedes_revision_id is not None:
                ...
                if prior["state"] != RevisionState.NEEDS_CHANGES.value:
                    raise PlanDomainError("state_conflict", "only a needs-changes revision can be superseded")
                if prior["base_plan_version_id"] != base_plan_version_id:
                    raise PlanDomainError("state_conflict", "superseding draft must preserve the plan base")
            ...
            await self._insert_event(
                ...
                detail={"base_plan_version_id": str(base_plan_version_id) if base_plan_version_id else None},
        ...
        *,
        ...
        if snapshot["state"] == RevisionState.PROPOSED.value:
            count = await conn.fetchval(
                f"select count(*) from {SCHEMA}.plan_revision_changes where plan_revision_id = $1",
                revision_id,
            )
            stored = await conn.fetchval(
                f"select validation_result from {SCHEMA}.plan_revisions where owner_user_id = $1 and id = $2",
                owner_user_id,
                revision_id,
            )
            status = _json_object(stored, field="validation_result")["status"]
            return ProposalResult(revision_id, RevisionState.PROPOSED, status, int(count))
        if snapshot["state"] != RevisionState.DRAFT.value:
            raise PlanDomainError("state_conflict", "only a draft revision can be proposed")
        ...
        if snapshot["base_plan_version_id"] is not None:
            ...
            base_document = PlanDocumentV1.from_mapping(_json_object(base_value, field="base document"))
    ...
    async def approve_and_activate(
        ...
        *,
        ...
        if snapshot["state"] == RevisionState.ACTIVATED.value:
            return await self._activation_result(conn, owner_user_id, revision_id)
        if snapshot["state"] != RevisionState.PROPOSED.value:
        ...
        async with conn.transaction():
            ...
            if row["validation_version"] != PLAN_VALIDATION_VERSION:
                raise PlanDomainError("validation_version_stale", "proposal requires current validation")
            stored_validation = _json_object(row["validation_result"], field="validation_result")
            if stored_validation.get("status") != current_validation["status"]:
                raise PlanDomainError("validation_version_stale", "proposal validation result changed")
            ...
            if (
                ...
                await self._insert_event(
                    ...
                    detail={"current_active_plan_version_id": str(current_active) if current_active else None},
                    request_id=request_id,
                )
                conflict = True
            else:
                decision = validate_activation(
                    ...
                    validation_status=current_validation["status"],
        ...
        if conflict:
            raise PlanDomainError("state_conflict", "active plan changed before approval")
        if result is None:
