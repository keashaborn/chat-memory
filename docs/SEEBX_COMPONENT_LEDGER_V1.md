...
| Conversation composition | Candidate `5ed33124` moves both live implementations from `rag_engine/response_composition_root_v0_2.py` and `v0_4.py` to `seebx.capabilities.conversation.composition` and `.lifeswitch_composition`; the generic pipeline is `ConversationResponseComposer`, the domain stage is `LifeSwitchResponseStage`, and the full wrapper is `LifeSwitchConversationComposer`; no legacy wrapper remains | conversation capability | CANONICAL PLACEMENT + ROLE NAMES COMPLETE; DEPENDENCY EXTRACTION CONTINUES | Compilation passes; 126 bounded tests add one no-wrapper guard and retain exactly the one pre-existing stale provenance failure; all 53 Zep tests and exact 149-route/OpenAPI parity pass; next extract the still-live `rag_engine` provider, policy, request, finalization, and LifeSwitch context dependencies without changing response behavior |
