...
| Search executor | Candidate owns plan, budget, authorization, and execution under `seebx.capabilities.search`; shared language, OpenAI, and transcript persistence use their canonical contract, adapter, and conversation owners | search capability | KEEP + FINISH CORE AUTHORITY | Move only authorization dependencies into core; preserve server-owned budgets and owner authority |
