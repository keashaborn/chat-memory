...
| Component | Verified current state | Target owner | Disposition | Gate |
|---|---|---|---|---|
| `resse_response_router.py` | Mounted as the single response route; current static closure is 58 modules/23,963 lines; candidate `25be0d0a` removes local Zep singleton construction and a dead export | `capabilities.conversation` | CONSOLIDATE + RENAME; SHARED RUNTIME EXTRACTED | Move the router without a permanent compatibility wrapper, preserve the exact route/OpenAPI contract, and extract attachment SQL behind a conversation adapter |
