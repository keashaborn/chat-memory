...
| Component | Verified current state | Target owner | Disposition | Gate |
|---|---|---|---|---|
| `resse_response_router.py` | Mounted as the single response route; current static closure is 60 modules/24,025 lines; candidate `25be0d0a` removes local Zep singleton construction and a dead export; candidate `988596bd` removes attachment SQL and the legacy attachment-contract import | `capabilities.conversation` | CONSOLIDATE + RENAME; SHARED RUNTIME AND ATTACHMENT READ EXTRACTED | Move the router without a permanent compatibility wrapper and preserve the exact route/OpenAPI contract; the remaining root still owns provider/composition selection and must be split by declared role |
