...
| Search executor | Candidate owns plan, budget, authorization, and execution under `seebx.capabilities.search`; shared language, OpenAI, transcript persistence, and request identity now use their canonical contract, adapter, conversation, and core owners | search capability | KEEP; CORE AUTHORITY EXTRACTED | Preserve server-owned budgets and exact verified owner behavior; the remaining voice-session lease import belongs to the voice cleanup boundary, not search |
...
| Component | Verified current state | Target owner | Disposition | Gate |
|---|---|---|---|---|
| Voice session lease | Live PostgreSQL-backed authority; still combines route, request authority, and database lease behavior in `rag_engine.voice_session_router` | voice capability | KEEP + EXTRACT | Preserve authenticated lease semantics while separating the database adapter and voice authority from the router in the next bounded batch |
