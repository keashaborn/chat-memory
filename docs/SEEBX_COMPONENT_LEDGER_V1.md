...
| Conversation composition | Candidate `5ed33124` moves both live implementations from `rag_engine/response_composition_root_v0_2.py` and `v0_4.py` to `seebx.capabilities.conversation.composition` and `.lifeswitch_composition`; the generic pipeline is `ConversationResponseComposer`, the domain stage is `LifeSwitchResponseStage`, and the full wrapper is `LifeSwitchConversationComposer`; candidate `7f68089f` places both OpenAI chat adapters under `seebx.adapters`; candidates `b9069fa7`, `6f6ef721`, and `f7a5f7e2` place prompt assembly, orchestration, and policy under the conversation capability; no legacy wrapper remains | conversation capability | CANONICAL PLACEMENT + ROLE NAMES COMPLETE; DEPENDENCY EXTRACTION CONTINUES | Compilation passes; broad conversation/voice/usage tests and all 53 Zep tests retain baseline behavior with exact 149-route/OpenAPI parity; next extract finalization, inspection, usage, and the remaining LifeSwitch context dependencies without changing response behavior |
