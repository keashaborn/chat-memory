...
| LifeSwitch domain | `capabilities.nutrition`, `training`, `measurements`, `plans` | Measurements and every retained Nutrition and Training aggregate place PostgreSQL effects behind named adapters. Nutrition is split into Log, Meals, Foods/Servings/Overrides, and Meal Plans modules behind one route-order composition root. Training is split into Exercises, Conditioning, Sharing, Templates, and Sessions modules behind a second route-order composition root. Plans use `adapters.lifeswitch_plan_postgres`. All eleven domain route modules now consume canonical verified Supabase/voice identity; raw actor-header-only requests fail before repository access. The paired frontend restores Plan-profile loading and four-macro scoring and forwards the bearer. The disposable verifier proves exact current-database restore, self-owner RLS, denied cross-owner writes, fail-closed disabled delegation, and cleanup | isolated LifeSwitch PostgreSQL | no external product selected; Supabase remains an infrastructure option | KEEP; DATABASE EFFECTS, LOGICAL AGGREGATES, PLAN-TO-NUTRITION REPAIR, CURRENT-DATABASE RESTORE/RLS, AND VERIFIED IDENTITY CONSUMPTION COMPLETE; PRODUCTION MOVE PENDING | prove clean-install migrations and one real signed-session application path through paired frontend/backend and the restored database; test enabled delegation only if approved before deployment |
