...
| LifeSwitch domain | `capabilities.nutrition`, `training`, `measurements`, `plans` | Measurements retains three owner-bound routes with all SQL and connection effects in `adapters.lifeswitch_measurements_postgres`. The paired frontend candidate restores Plan-profile loading and calorie, protein, carbohydrate, and fat scoring for Nutrition Log/Analyze without restoring retired page-level AI helpers. `adapters.lifeswitch_nutrition_log_postgres` now owns the complete Nutrition Log database boundary: delegated permission, read projections, create/update/delete/completion statements, exact transaction scopes, validated schemas, and connection lifetime. Foods/meals and Training remain later batches. Plans already use `adapters.lifeswitch_plan_postgres` | isolated LifeSwitch PostgreSQL | no external product selected; Supabase remains an infrastructure option | KEEP + SPLIT; MEASUREMENTS EFFECTS SEPARATED; PLAN-TO-NUTRITION CANDIDATE REPAIRED; NUTRITION LOG DATABASE EFFECTS SEPARATED; PRODUCTION MOVE PENDING | verify authenticated owner behavior against the isolated database and paired frontend candidate before deployment; then extract foods/meals and Training one coherent aggregate at a time |
