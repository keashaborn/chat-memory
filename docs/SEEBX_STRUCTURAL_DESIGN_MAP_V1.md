...
| LifeSwitch domain | `capabilities.nutrition`, `training`, `measurements`, `plans` | Measurements and all retained Nutrition aggregates now place PostgreSQL effects behind named adapters. The paired frontend candidate restores Plan-profile loading and four-macro scoring without retired page-level AI helpers. Training is being separated by real domain boundary: My Exercises now has one canonical repository with exact SQL/transaction parity, while conditioning, sharing/templates, and sessions/logs remain distinct pending aggregates. Plans already use `adapters.lifeswitch_plan_postgres` | isolated LifeSwitch PostgreSQL | no external product selected; Supabase remains an infrastructure option | KEEP + SPLIT; MEASUREMENTS AND ALL NUTRITION EFFECTS SEPARATED; PLAN-TO-NUTRITION REPAIRED; TRAINING MY EXERCISES SEPARATED; PRODUCTION MOVE PENDING | split the effect-free Nutrition module without route drift; then extract the remaining Training aggregates separately and verify authenticated owner behavior against the isolated database and paired frontend candidate before deployment |
...
| 5 | split `app.py`, domain services, and SQL adapters | composition root complete; conversation/search/catalog/Forms/Measurements/AI Operations/telemetry and all Nutrition handlers SQL-free; Plan-to-Nutrition repaired; Training My Exercises SQL-free; split the effect-free Nutrition module and extract conditioning, sharing/templates, and session/log Training owners separately |
