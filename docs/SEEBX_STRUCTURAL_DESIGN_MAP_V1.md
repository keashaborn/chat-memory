...
| legacy `rag_engine/resse_response_router.py` and current 60-module/24,025-line closure | `seebx.capabilities.conversation` | candidate `b56d0e84` moves the mounted router and its tests to the conversation capability with no legacy wrapper; the next seam names the generic base composer and LifeSwitch wrapper by role rather than historical version |
...
| Conversation and transcripts | `capabilities.conversation` | candidate `b56d0e84` mounts `seebx.capabilities.conversation.router` directly with no legacy wrapper; `25be0d0a` centralizes the Zep runtime/prompt settings; `988596bd` owns attachment validation/rendering and its owner-scoped PostgreSQL read; finalized responses and search exchanges share `capabilities.conversation.persistence` | platform PostgreSQL for threads/messages/receipts | Zep **KEEP for memory only**; OpenAI behind model adapter | CANONICAL ROUTER PLACEMENT; ORCHESTRATOR CONSOLIDATION CONTINUES | name the generic base and LifeSwitch wrapper by role, consolidate attachment CRUD/thread reads still in `app.py`, and prove Zep deletion/outage behavior before deployment; keep external `ResseResponseRequestV1` and operation identifiers until a separately versioned API change |
