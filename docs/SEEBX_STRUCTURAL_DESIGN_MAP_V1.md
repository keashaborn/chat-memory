...
| Voice | `capabilities.voice` with transcription, realtime, synthesis, and lease sub-capabilities | candidate `381704fb` separates session HTTP contract, core authority, and PostgreSQL adapter; shared language is in `seebx.contracts.voice_language`; candidate `92adf9e3` places shared request correlation in `seebx.core.voice_observability` and provider-free realtime state in `seebx.capabilities.voice.realtime_session`; candidates `f3889883` and `03a4657f` mount transcription and synthesis from `seebx.capabilities.voice` and isolate their OpenAI effects in `seebx.adapters.openai_transcription` and `.openai_tts` without wrappers; preview and sideband network effects remain under `rag_engine` | platform PostgreSQL session/lease/audit state | OpenAI speech/realtime behind voice adapters | KEEP + CONSOLIDATE; TRANSCRIPTION AND SYNTHESIS COMPLETE; REALTIME PROVIDER EXTRACTION NEXT | extract realtime HTTP/WebSocket/config effects without changing routes, owner/lease authority, cursor semantics, cleanup, or failure mapping; then run bounded canary deployment and authenticated owner/lease tests |
