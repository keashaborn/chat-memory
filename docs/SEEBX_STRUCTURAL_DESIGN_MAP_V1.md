...
| canonical search package plus shared legacy adapters | one search capability with trusted-health and current-news policy profiles | search owns routing, planning, source policy, provider, evidence, and audit; shared voice language lives in `seebx.contracts` and the SDK client lives in `seebx.adapters.openai`; move transcript and authority boundaries only with their owning capabilities |
...
| Search and current information | `capabilities.search` | canonical package owns routing, planning, authorization, registry, policy, provider, evidence, admission, audit, NCBI, and ODS; legacy search wrappers are retired; shared language and OpenAI SDK boundaries are extracted | platform PostgreSQL search audit/cache | `seebx.adapters.openai` | CANONICALIZED; TRANSCRIPT/AUTHORITY EXTRACTION PENDING | isolate transcript and core-authority boundaries; then prove disposable-database audit parity and authenticated frontend behavior before deployment |
