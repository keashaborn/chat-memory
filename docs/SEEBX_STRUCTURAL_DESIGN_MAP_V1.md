...
| canonical search package plus shared legacy adapters | one search capability with trusted-health and current-news policy profiles | search now owns routing, planning, source policy, provider, evidence, and audit; shared voice language now lives in `seebx.contracts`; move shared OpenAI, transcript, and authority boundaries only with their owning capabilities |
...
| Search and current information | `capabilities.search` | canonical package owns routing, planning, authorization, registry, policy, provider, evidence, admission, audit, NCBI, and ODS; legacy search wrappers are retired; language policy imports `seebx.contracts.voice_language` | platform PostgreSQL search audit/cache | provider clients behind shared adapters | CANONICALIZED; MODEL/TRANSCRIPT/AUTHORITY EXTRACTION PENDING | isolate shared OpenAI, transcript, and core-authority boundaries; then prove disposable-database audit parity and authenticated frontend behavior before deployment |
