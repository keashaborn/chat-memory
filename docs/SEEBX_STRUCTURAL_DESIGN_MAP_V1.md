...
| `resse_response_router.py` and current 60-module/24,025-line closure | `capabilities.conversation` | candidate `25be0d0a` gives the application and response route one conversation-owned Zep runtime; candidate `988596bd` moves the attachment contract and exact owner-scoped PostgreSQL read behind declared conversation/adapter owners; next move the router, then name the base composer and LifeSwitch wrapper by role rather than historical version |
| canonical search package plus shared adapters | one search capability with trusted-health and current-news policy profiles | search owns routing, planning, source policy, provider, evidence, and audit; shared language, SDK, conversation persistence, request identity, and voice-session authority now use their declared owners |
...
| Conversation and transcripts | `capabilities.conversation` | candidate `25be0d0a` centralizes the process-wide Zep runtime/prompt settings; candidate `988596bd` owns attachment validation/rendering in `capabilities.conversation.attachments` and its exact owner-scoped read in `adapters.conversation_attachments`; `resse_response_router.py` remains the mounted orchestrator; finalized responses and search exchanges share `capabilities.conversation.persistence` | platform PostgreSQL for threads/messages/receipts | Zep **KEEP for memory only**; OpenAI behind model adapter | CONSOLIDATE; SHARED ZEP RUNTIME AND ATTACHMENT READ EXTRACTED | move/rename the mounted router without a permanent wrapper, name the v0.2 base and v0.4 LifeSwitch wrapper by role, consolidate attachment CRUD/thread reads still in `app.py`, and prove Zep deletion/outage behavior before deployment |
| Search and current information | `capabilities.search` | canonical package owns routing, planning, authorization, registry, policy, provider, evidence, admission, audit, NCBI, and ODS; shared language, OpenAI SDK, transcript persistence, request identity, and voice-session boundaries are extracted | platform PostgreSQL search audit/cache | `seebx.adapters.openai` | CANONICALIZED; PRODUCTION MOVE PENDING | prove disposable-database audit parity and authenticated frontend behavior before deployment |
