...
| LifeSwitch domain | `capabilities.nutrition`, `training`, `measurements`, `plans` | Measurements and all retained Nutrition aggregates now place PostgreSQL effects behind named adapters. The paired frontend candidate restores Plan-profile loading and four-macro scoring without retired page-level AI helpers. Training now has canonical My Exercises, protected stored-writer, delegated-access query, Conditioning, workout-sharing, and templates/exercises/segments PostgreSQL adapters; only strength sessions/logs remain pending. Plans already use `adapters.lifeswitch_plan_postgres` | isolated LifeSwitch PostgreSQL | no external product selected; Supabase remains an infrastructure option | KEEP + SPLIT; MEASUREMENTS AND ALL NUTRITION EFFECTS SEPARATED; PLAN-TO-NUTRITION REPAIRED; TRAINING MY EXERCISES, SHARED WRITER, ACCESS QUERY, CONDITIONING, WORKOUT SHARING, AND TEMPLATE EFFECTS SEPARATED; PRODUCTION MOVE PENDING | split the effect-free Nutrition module without route drift; then extract the Training strength session/log boundary and verify authenticated owner behavior against the isolated database and paired frontend candidate before deployment |
...
| 5 | split `app.py`, domain services, and SQL adapters | composition root complete; conversation/search/catalog/Forms/Measurements/AI Operations/telemetry and all Nutrition handlers SQL-free; Plan-to-Nutrition repaired; Training My Exercises, shared writer, delegated access query, Conditioning, workout sharing, and templates/exercises/segments separated; split the effect-free Nutrition module and extract the strength session/log Training owner |
