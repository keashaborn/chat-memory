...
| Voice | `capabilities.voice` with transcription, realtime, synthesis, and lease sub-capabilities | candidate `381704fb` separates session HTTP contract, core authority, and PostgreSQL adapter; shared language is in `seebx.contracts.voice_language`; candidate `92adf9e3` places shared request correlation in `seebx.core.voice_observability` and the provider-free realtime registry/state machine in `seebx.capabilities.voice.realtime_session` without wrappers; transcription, synthesis, preview, and sideband network effects remain under `rag_engine` | platform PostgreSQL session/lease/audit state | OpenAI speech/realtime behind voice adapters | KEEP + CONSOLIDATE; CORE/STATE PLACEMENT COMPLETE; PROVIDER EXTRACTION NEXT | extract direct OpenAI HTTP/WebSocket/config effects into speech/realtime adapters without changing routes or failure mapping; then run bounded canary deployment and authenticated owner/lease tests |
