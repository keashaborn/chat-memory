...
| LifeSwitch domain | `capabilities.nutrition`, `training`, `measurements`, `plans` | Measurements and every retained Nutrition and Training aggregate place PostgreSQL effects behind named adapters. Nutrition is split into Log, Meals, Foods/Servings/Overrides, and Meal Plans modules behind one route-order composition root. Training is split into Exercises, Conditioning, Sharing, Templates, and Sessions modules behind a second route-order composition root, with shared access and validation helpers explicitly owned. The paired frontend candidate restores Plan-profile loading and four-macro scoring without retired page-level AI helpers. Plans use `adapters.lifeswitch_plan_postgres`; the canonical disposable verifier now proves exact current-database restore, self-owner RLS, denied cross-owner writes, fail-closed disabled delegation, and cleanup | isolated LifeSwitch PostgreSQL | no external product selected; Supabase remains an infrastructure option | KEEP; ALL RETAINED LIFESWITCH DOMAIN DATABASE EFFECTS AND LOGICAL AGGREGATE OWNERSHIP SEPARATED; PLAN-TO-NUTRITION REPAIR AND CURRENT-DATABASE RESTORE/RLS PROOF COMPLETE; PRODUCTION MOVE PENDING | prove clean-install migrations, application routes through the restored database, authenticated paired-frontend owner behavior, and enabled delegation only if approved before deployment |
