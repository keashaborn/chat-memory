...
| Search and current information | `capabilities.search` | canonical package owns routing, planning, authorization, registry, policy, provider, evidence, admission, NCBI, and ODS; candidate `bed831f5` moves the hashed `SearchCapabilityManifestV1` wire contract to `seebx.contracts.search` without changing its bytes; `7ffb6b82` makes search audit/runtime capability code SQL-free and gives connection ownership, shared rate limiting, and metadata-only audit writes to `seebx.adapters.search_audit`; shared language, OpenAI SDK, transcript persistence, request identity, and voice-session boundaries use their canonical owners | platform PostgreSQL search audit/cache | `seebx.adapters.openai` | CANONICALIZED; SHARED CONTRACT AND AUDIT DATABASE BOUNDARIES EXTRACTED; PRODUCTION MOVE PENDING | separate transcript-connection and ODS-cache SQL still in capability code, then prove disposable-database parity and authenticated frontend behavior before deployment |
