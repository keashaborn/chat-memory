...
- five direct SQL/transaction capability modules remain, confined to LifeSwitch
  Nutrition and Training; Catalog, Forms, Measurements, AI Operations, and
  telemetry are behind named adapters.
...
|---|---|---|---|---|---|---|
| Identity and ownership | `core.identity`, `core.ownership`, `core.policy` | candidate `a20e9634` owns bearer extraction, fail-closed request errors, verified actor/owner/session binding, and raw header assertion; `adapters.supabase` owns issuer/JWKS/JWT verification. Candidate `63e2e45a` adds one owner-independent verified request-actor path and routes all ten active app-owned thread operations through Supabase or an active owner-bound voice lease before PostgreSQL lookup; Verbal Sage candidate `c25502b` forwards the original bearer through every active thread proxy/preflight and the inspector active-thread lookup | Supabase Auth identity; PostgreSQL owner/RLS state | Supabase Auth **KEEP** | CANONICALIZED IN PAIRED CANDIDATES; PRODUCTION MOVE PENDING | deploy the backend/frontend pair only after authenticated end-to-end proof and one rollback plan; no request field, cookie, raw actor header, service token, or frontend label is standalone user authority |
...
| LifeSwitch domain | `capabilities.nutrition`, `training`, `measurements`, `plans` | Measurements retains three owner-bound routes with all SQL and connection effects in `adapters.lifeswitch_measurements_postgres`. The paired frontend candidate restores Plan-profile loading and calorie, protein, carbohydrate, and fat scoring for Nutrition Log/Analyze without restoring retired page-level AI helpers. This backend batch moves the exact delegated permission statement and all four `/log/day` and `/log/range` projection statements plus their connection lifetime into `adapters.lifeswitch_nutrition_log_postgres`; Nutrition write paths and Training remain separate later batches. Plans already use `adapters.lifeswitch_plan_postgres` | isolated LifeSwitch PostgreSQL | no external product selected; Supabase remains an infrastructure option | KEEP + SPLIT; MEASUREMENTS EFFECTS SEPARATED; PLAN-TO-NUTRITION CANDIDATE REPAIRED; NUTRITION LOG READ EFFECTS SEPARATED; PRODUCTION MOVE PENDING | verify authenticated owner behavior against the isolated database and paired frontend candidate before deployment; then extract Nutrition writes, foods/meals, and Training one coherent aggregate at a time |
...
| 5 | split `app.py`, domain services, and SQL adapters | composition root complete; conversation/search/catalog/Forms/Measurements/AI Operations/telemetry SQL-free; Plan-to-Nutrition repaired in the paired frontend candidate; extract the five remaining Nutrition/Training direct query owners |
...
- 2026-08-21 candidate checkpoint: the exact delegated People-permission query and all four Nutrition Log read-projection statements for `/log/day` and `/log/range` move into `adapters.lifeswitch_nutrition_log_postgres`, with strict schema identifiers and guaranteed close on success or failure. Normalized SQL hashes match the parent for all five statements. Nineteen focused identity/adapter/route tests and the complete 1,257-test backend suite pass. Parent and candidate retain 138 routes, 124 OpenAPI paths, route-table SHA-256 `dacb3665272ec6720fb477340c9d84d977a6af55fae4f061972526eabe10353d`, and OpenAPI SHA-256 `0c98736818a52d53797460c1f207f014d9882af64400363ae1fd69c21f54b46a`. Production is unchanged.
