...
- all 1,211 locked-runtime tests pass after the Forms adapter extraction;
...
- conversation and search capability packages have zero direct SQL query or
  transaction effects;
- nine direct SQL/transaction capability modules remain, confined to
  LifeSwitch domain, operations, and observability; catalog and Forms are
  now behind named adapters.
...
| 5 | split `app.py`, domain services, and SQL adapters | composition root complete; conversation/search/catalog/Forms SQL-free; extract the nine remaining domain/operations/observability query owners |
