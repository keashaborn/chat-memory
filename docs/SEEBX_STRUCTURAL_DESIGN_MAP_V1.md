...
| LifeSwitch domain | `capabilities.nutrition`, `training`, `measurements`, `plans` | Measurements, Nutrition Log, Meals templates, and Meal Plans now place all PostgreSQL effects behind named adapters. The paired frontend candidate restores Plan-profile loading and four-macro scoring without retired page-level AI helpers. Foods, servings, overrides, and Training remain later batches. Plans already use `adapters.lifeswitch_plan_postgres` | isolated LifeSwitch PostgreSQL | no external product selected; Supabase remains an infrastructure option | KEEP + SPLIT; MEASUREMENTS, NUTRITION LOG, MEALS, AND MEAL-PLAN EFFECTS SEPARATED; PLAN-TO-NUTRITION CANDIDATE REPAIRED; PRODUCTION MOVE PENDING | verify authenticated owner behavior against the isolated database and paired frontend candidate before deployment; then separate foods/servings/overrides and Training one coherent aggregate at a time |
