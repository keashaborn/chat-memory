...
| LifeSwitch domain | `capabilities.nutrition`, `training`, `measurements`, `plans` | Measurements and every retained Nutrition and Training aggregate now place PostgreSQL effects behind named adapters. Nutrition is split into Log, Meals, Foods/Servings/Overrides, and Meal Plans modules behind one route-order composition root. The paired frontend candidate restores Plan-profile loading and four-macro scoring without retired page-level AI helpers. Training has canonical My Exercises, protected writer, delegated-access, Conditioning, workout-sharing, template, and strength-session/set-log adapters. Plans use `adapters.lifeswitch_plan_postgres` | isolated LifeSwitch PostgreSQL | no external product selected; Supabase remains an infrastructure option | KEEP + SPLIT; ALL RETAINED LIFESWITCH DOMAIN DATABASE EFFECTS SEPARATED; NUTRITION LOGICAL SPLIT AND PLAN-TO-NUTRITION REPAIR COMPLETE; PRODUCTION MOVE PENDING | split the effect-free Training module without route drift; then verify disposable-database parity and authenticated owner/delegated behavior against the paired frontend candidate before deployment |
...
| 5 | split `app.py`, domain services, and SQL adapters | composition root complete; retained capability handlers have no raw PostgreSQL effects; all Nutrition and Training database boundaries use named adapters; split the effect-free Nutrition and Training modules by aggregate without route drift |
