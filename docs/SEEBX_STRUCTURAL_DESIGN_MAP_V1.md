...
| `resse_response_router.py` and current 58-module/23,963-line closure | `capabilities.conversation` | candidate `25be0d0a` gives the application and response route one conversation-owned Zep runtime; next move the router and attachment data access, then name the base composer and LifeSwitch wrapper by role rather than historical version |
...
| Conversation and transcripts | `capabilities.conversation` | candidate `25be0d0a` centralizes the process-wide Zep runtime/prompt settings under the conversation capability; `resse_response_router.py` remains the mounted orchestrator; finalized responses and search exchanges share `capabilities.conversation.persistence` | platform PostgreSQL for threads/messages/receipts | Zep **KEEP for memory only**; OpenAI behind model adapter | CONSOLIDATE; SHARED ZEP RUNTIME EXTRACTED | move/rename the mounted router without a permanent wrapper, extract attachment SQL, name the v0.2 base and v0.4 LifeSwitch wrapper by role, and prove Zep deletion/outage behavior before deployment |
