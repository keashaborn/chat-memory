...
| Voice | `capabilities.voice` with transcription, realtime, synthesis, and lease sub-capabilities | candidate `381704fb` separates session HTTP contract, core authority, and PostgreSQL adapter; shared language is in `seebx.contracts.voice_language`; candidate `92adf9e3` places shared request correlation in `seebx.core.voice_observability` and provider-free realtime state in `seebx.capabilities.voice.realtime_session`; candidates `f3889883` and `03a4657f` mount transcription and synthesis from `seebx.capabilities.voice` and isolate their OpenAI effects in `seebx.adapters.openai_transcription` and `.openai_tts`; candidate `c9a552df` mounts the preview HTTP capability directly and isolates negotiation plus secret loading in `seebx.adapters.openai_realtime` and `.voice_realtime_config`, all without wrappers; only the sideband controller still mixes provider WebSocket I/O with governed loopback HTTP | platform PostgreSQL session/lease/audit state | OpenAI speech/realtime behind voice adapters | KEEP + CONSOLIDATE; TRANSCRIPTION, SYNTHESIS, AND PREVIEW HTTP COMPLETE; SIDEBAND CONSOLIDATION NEXT | split sideband WebSocket transport from governed turn orchestration, then replace or formally contain same-process loopback HTTP without changing owner/lease authority, cursor/order semantics, cleanup, or failure mapping; run bounded canary and authenticated owner/lease tests before production movement |
