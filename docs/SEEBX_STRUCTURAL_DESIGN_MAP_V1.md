...
| Voice | `capabilities.voice` with transcription, realtime, synthesis, and lease sub-capabilities | candidate `381704fb` separates session HTTP contract, core authority, and PostgreSQL adapter; shared language is in `seebx.contracts.voice_language`; candidate `92adf9e3` places shared request correlation in `seebx.core.voice_observability` and provider-free realtime state in `seebx.capabilities.voice.realtime_session`; candidate `f3889883` mounts transcription from `seebx.capabilities.voice.transcription` and isolates all OpenAI transcription effects in `seebx.adapters.openai_transcription` without wrappers; synthesis, preview, and sideband network effects remain under `rag_engine` | platform PostgreSQL session/lease/audit state | OpenAI speech/realtime behind voice adapters | KEEP + CONSOLIDATE; TRANSCRIPTION COMPLETE; SYNTHESIS/REALTIME PROVIDER EXTRACTION NEXT | extract TTS and realtime HTTP/WebSocket/config effects without changing routes or failure mapping; then run bounded canary deployment and authenticated owner/lease tests |
