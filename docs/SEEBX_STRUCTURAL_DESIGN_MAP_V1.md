...
| LifeSwitch domain | `capabilities.nutrition`, `training`, `measurements`, `plans` | Measurements retains three owner-bound routes with all SQL and connection effects in `adapters.lifeswitch_measurements_postgres`. The paired frontend candidate restores Plan-profile loading and calorie, protein, carbohydrate, and fat scoring for Nutrition Log/Analyze without restoring retired page-level AI helpers. Named adapters now own the complete Nutrition Log database boundary and all Meals-template SQL/transaction/connection effects. Foods, servings, overrides, meal plans, and Training remain later batches. Plans already use `adapters.lifeswitch_plan_postgres` | isolated LifeSwitch PostgreSQL | no external product selected; Supabase remains an infrastructure option | KEEP + SPLIT; MEASUREMENTS, NUTRITION LOG, AND MEALS EFFECTS SEPARATED; PLAN-TO-NUTRITION CANDIDATE REPAIRED; PRODUCTION MOVE PENDING | verify authenticated owner behavior against the isolated database and paired frontend candidate before deployment; then separate foods/servings/overrides, meal plans, and Training one coherent aggregate at a time |
