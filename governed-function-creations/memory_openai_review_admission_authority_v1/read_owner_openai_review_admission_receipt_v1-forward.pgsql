...
BEGIN
  ...
  IF stage_request.operation<>'stage_packet'
     ...
     OR (stage_request.result#>>'{observation_ids,o01}') IS NULL
     OR (stage_request.result#>>'{resolution_ids,e01}') IS NULL THEN
    RAISE EXCEPTION 'OpenAI review admission stage receipt conflicts'
      USING ERRCODE='23514';
  END IF;
  batch_id_value:=(stage_request.result->>'batch_id')::uuid;
  new_observation_id:=(stage_request.result#>>'{observation_ids,o01}')::uuid;
  new_resolution_id:=(stage_request.result#>>'{resolution_ids,e01}')::uuid;
