...
DECLARE
  ...
  disposition_replay memory.evidence_extraction_event%ROWTYPE;
  claimed record;
  ...
  disposition_target_status memory.evidence_extraction_job_status;
  disposition_event_type text;
  expected_lineage_sha text;
BEGIN
  ...
  IF prefilter_mode='memory_v1_openai_prefilter_skip_v1' THEN
    ...
    IF jsonb_typeof(prefilter_receipt)<>'object'
       ...
       OR prefilter_receipt->>'contract_version' NOT IN (
      ...
      RAISE EXCEPTION 'prefilter skip receipt is invalid'
        USING ERRCODE='22023';
    END IF;
    IF prefilter_receipt->>'contract_version'
       ='memory_v1_prefilter_skip_receipt_v2' THEN
      request_receipt := prefilter_receipt->'eligibility_disposition';
      IF jsonb_typeof(request_receipt)<>'object'
         ...
         OR request_receipt->>'decision'<>'send_external'
         OR request_receipt->>'owner_user_id'<>actor::text
         OR request_receipt->>'job_id'<>exact_job::text
         OR request_receipt->>'evidence_content_sha256'<>exact_content_sha
         OR (request_receipt->>'external_model_calls')::integer<>0
         OR request_receipt->>'gate_result_sha256'
            <>prefilter_receipt->>'gate_sha256'
         OR request_receipt->>'lineage_sha256' !~ '^[0-9a-f]{64}$'
         OR jsonb_typeof(request_receipt->'reason_codes')<>'array'
         OR jsonb_array_length(request_receipt->'reason_codes') NOT BETWEEN 1 AND 16
         OR EXISTS (
           SELECT 1
           FROM jsonb_array_elements_text(
             request_receipt->'reason_codes'
           ) AS reason(value)
           WHERE reason.value !~ '^[a-z][a-z0-9_.-]{1,127}$'
         )
         OR EXISTS (
           SELECT 1 FROM jsonb_object_keys(request_receipt) AS keys(key_name)
           WHERE key_name<>ALL(ARRAY[
             'contract_version','context_envelope_sha256','context_turn_count',
             'decision','evidence_content_sha256','evidence_id','exchange_id',
             'external_model_calls','gate_contract_version','gate_result_sha256',
             'job_id','lineage_sha256','owner_user_id','policy_sha256',
             'policy_version','reason_codes','selected_span_count',
             'selected_spans_sha256','source_gate_sha256'
           ]::text[])
         )
         OR (SELECT count(*) FROM jsonb_object_keys(request_receipt))<>19
         OR EXISTS (
           SELECT 1 FROM jsonb_object_keys(prefilter_receipt) AS keys(key_name)
           WHERE key_name<>ALL(ARRAY[
             'contract_version','eligibility_disposition',
             'external_model_calls','gate_sha256','reason_code'
           ]::text[])
         )
         OR (SELECT count(*) FROM jsonb_object_keys(prefilter_receipt))<>5 THEN
        RAISE EXCEPTION 'prefilter budget skip eligibility is invalid'
          USING ERRCODE='22023';
      END IF;
      expected_lineage_sha := encode(public.digest(convert_to(concat_ws('|',
        request_receipt->>'contract_version',actor::text,
        request_receipt->>'evidence_id',exact_job::text,exact_content_sha,
        request_receipt->>'exchange_id',request_receipt->>'decision',
        request_receipt->>'gate_result_sha256',
        request_receipt->>'policy_sha256',
        coalesce(request_receipt->>'context_envelope_sha256',''),
        coalesce(request_receipt->>'source_gate_sha256',''),
        (
          SELECT string_agg(reason.value,',' ORDER BY reason.ordinality)
          FROM jsonb_array_elements_text(request_receipt->'reason_codes')
            WITH ORDINALITY AS reason(value,ordinality)
        )
      ),'UTF8'),'sha256'),'hex');
      IF request_receipt->>'lineage_sha256'<>expected_lineage_sha THEN
        RAISE EXCEPTION 'prefilter budget skip lineage binding is invalid'
          USING ERRCODE='23514';
      END IF;
    ELSIF EXISTS (
      SELECT 1 FROM jsonb_object_keys(prefilter_receipt) AS keys(key_name)
      WHERE key_name<>ALL(ARRAY[
        'contract_version','external_model_calls','gate_sha256','reason_code'
      ]::text[])
    ) OR (SELECT count(*) FROM jsonb_object_keys(prefilter_receipt))<>4 THEN
      RAISE EXCEPTION 'prefilter skip receipt contains unknown fields'
        USING ERRCODE='22023';
    END IF;
  ...
  IF prefilter_mode='memory_v1_openai_eligibility_disposition_v2' THEN
    IF prefilter_receipt_text IS NULL THEN
      RAISE EXCEPTION 'eligibility disposition receipt is required'
        USING ERRCODE='22023';
    END IF;
    prefilter_receipt := prefilter_receipt_text::jsonb;
    IF jsonb_typeof(prefilter_receipt)<>'object'
       OR pg_column_size(prefilter_receipt)>8192
       OR prefilter_receipt->>'contract_version'
          <>'memory_v1_personal_evidence_disposition_receipt_v2'
       OR prefilter_receipt->>'decision' NOT IN (
         'skip_zero_call','review_context','route_internal','block_local'
       )
       OR prefilter_receipt->>'owner_user_id'<>actor::text
       OR prefilter_receipt->>'job_id'<>exact_job::text
       OR prefilter_receipt->>'evidence_content_sha256'<>exact_content_sha
       OR (prefilter_receipt->>'external_model_calls')::integer<>0
       OR prefilter_receipt->>'evidence_id'
          !~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
       OR prefilter_receipt->>'exchange_id'
          !~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
       OR prefilter_receipt->>'gate_contract_version'
          !~ '^[a-z][a-z0-9_.-]{1,127}$'
       OR prefilter_receipt->>'policy_version'
          !~ '^[a-z][a-z0-9_.-]{1,127}$'
       OR prefilter_receipt->>'gate_result_sha256' !~ '^[0-9a-f]{64}$'
       OR prefilter_receipt->>'lineage_sha256' !~ '^[0-9a-f]{64}$'
       OR prefilter_receipt->>'policy_sha256' !~ '^[0-9a-f]{64}$'
       OR prefilter_receipt->>'selected_spans_sha256' !~ '^[0-9a-f]{64}$'
       OR jsonb_typeof(prefilter_receipt->'reason_codes')<>'array'
       OR jsonb_array_length(prefilter_receipt->'reason_codes') NOT BETWEEN 1 AND 16
       OR EXISTS (
         SELECT 1
         FROM jsonb_array_elements_text(
           prefilter_receipt->'reason_codes'
         ) AS reason(value)
         WHERE reason.value !~ '^[a-z][a-z0-9_.-]{1,127}$'
       )
       OR (
       ...
       OR (prefilter_receipt->>'context_turn_count')::integer NOT BETWEEN 0 AND 8
       OR (prefilter_receipt->>'selected_span_count')::integer NOT BETWEEN 0 AND 64
       OR jsonb_typeof(prefilter_receipt->'context_envelope_sha256')
          NOT IN ('null','string')
       OR (
         jsonb_typeof(prefilter_receipt->'context_envelope_sha256')='string'
         AND prefilter_receipt->>'context_envelope_sha256' !~ '^[0-9a-f]{64}$'
       )
       OR jsonb_typeof(prefilter_receipt->'source_gate_sha256')
          NOT IN ('null','string')
       OR (
         jsonb_typeof(prefilter_receipt->'source_gate_sha256')='string'
         AND prefilter_receipt->>'source_gate_sha256' !~ '^[0-9a-f]{64}$'
       )
       OR EXISTS (
         SELECT 1 FROM jsonb_object_keys(prefilter_receipt) AS keys(key_name)
         WHERE key_name<>ALL(ARRAY[
       ...
       OR (SELECT count(*) FROM jsonb_object_keys(prefilter_receipt))<>19 THEN
      RAISE EXCEPTION 'eligibility disposition receipt is invalid'
        USING ERRCODE='22023';
    END IF;

    expected_lineage_sha := encode(public.digest(convert_to(concat_ws('|',
      prefilter_receipt->>'contract_version',
      actor::text,
      prefilter_receipt->>'evidence_id',
      exact_job::text,
      exact_content_sha,
      prefilter_receipt->>'exchange_id',
      prefilter_receipt->>'decision',
      prefilter_receipt->>'gate_result_sha256',
      prefilter_receipt->>'policy_sha256',
      coalesce(prefilter_receipt->>'context_envelope_sha256',''),
      coalesce(prefilter_receipt->>'source_gate_sha256',''),
      (
        SELECT string_agg(reason.value,',' ORDER BY reason.ordinality)
        FROM jsonb_array_elements_text(prefilter_receipt->'reason_codes')
          WITH ORDINALITY AS reason(value,ordinality)
      )
    ),'UTF8'),'sha256'),'hex');
    IF prefilter_receipt->>'lineage_sha256'<>expected_lineage_sha THEN
      RAISE EXCEPTION 'eligibility disposition lineage binding is invalid'
        USING ERRCODE='23514';
    END IF;

    disposition_decision := prefilter_receipt->>'decision';
    disposition_target_status := CASE disposition_decision
      WHEN 'skip_zero_call' THEN 'skipped'::memory.evidence_extraction_job_status
      ELSE 'review_required'::memory.evidence_extraction_job_status
    END;
    disposition_event_type := CASE disposition_decision
      WHEN 'skip_zero_call' THEN 'skipped'
    ...
    SELECT event.* INTO disposition_replay
    FROM memory.evidence_extraction_event AS event
    WHERE event.owner_user_id=actor
      AND event.operation_id=p_operation_id;
    IF FOUND THEN
      IF disposition_replay.job_id<>exact_job
         OR disposition_replay.event_type<>disposition_event_type
         OR disposition_replay.to_status<>disposition_target_status
         OR disposition_replay.details->>'result_sha256'<>request_receipt_sha
         OR disposition_replay.details->>'decision'<>disposition_decision
         OR disposition_replay.details->>'exchange_id'
            <>prefilter_receipt->>'exchange_id'
         OR disposition_replay.details->>'lineage_sha256'
            <>prefilter_receipt->>'lineage_sha256' THEN
        RAISE EXCEPTION 'eligibility disposition replay conflicts'
          USING ERRCODE='23514';
      END IF;
      SELECT extraction_job.* INTO target_job
      FROM memory.evidence_extraction_job AS extraction_job
      WHERE extraction_job.owner_user_id=actor
        AND extraction_job.job_id=exact_job
        AND extraction_job.evidence_id=(prefilter_receipt->>'evidence_id')::uuid
        AND extraction_job.evidence_content_sha256=exact_content_sha
        AND extraction_job.status=disposition_target_status;
      IF NOT FOUND THEN
        RAISE EXCEPTION 'eligibility disposition replay target is invalid'
          USING ERRCODE='23514';
      END IF;
      RETURN QUERY
      SELECT target_job.job_id,target_job.evidence_id,NULL::uuid,
        ...
        evidence.observed_at,evidence.recorded_at,evidence.sensitivity::text,
        target_job.checkpoint_sequence,target_job.checkpoint_sha256,
        target_job.result,NULL::uuid,
        ('eligibility_'||disposition_decision)::text,0,0,'replayed'::text
      FROM memory.evidence AS evidence
      WHERE evidence.owner_user_id=actor
        AND evidence.evidence_id=target_job.evidence_id;
      RETURN;
    ...
    WHERE extraction_job.owner_user_id=actor
      ...
      AND extraction_job.status IN ('pending','error');
    IF NOT FOUND THEN
      RAISE EXCEPTION 'eligibility disposition target is absent or unavailable'
        USING ERRCODE='23514';
    END IF;
    IF NOT EXISTS (
      SELECT 1 FROM memory.evidence AS evidence
      ...
      RAISE EXCEPTION 'eligibility disposition evidence binding is invalid'
        USING ERRCODE='23514';
    END IF;

    prior_status := target_job.status;
    IF disposition_target_status='review_required' THEN
      UPDATE memory.evidence_extraction_job AS extraction_job
      ...
      INSERT INTO memory.evidence_extraction_event(
        ...
        actor,exact_job,gen_random_uuid(),'claimed',prior_status,'processing',
        'worker',p_worker_id,
        jsonb_build_object(
          'attempt',target_job.attempts,
          'decision',disposition_decision,
          'eligibility_disposition_only',true,
          'exchange_id',prefilter_receipt->>'exchange_id',
          'external_model_calls',0,
          'lease_seconds',p_lease_seconds,
          'lease_token',new_lease_token,
          'lineage_sha256',prefilter_receipt->>'lineage_sha256',
          'parent_operation_id',p_operation_id,
          'route',target_job.route
        )
      );
      prior_status := 'processing'::memory.evidence_extraction_job_status;
    END IF;
    UPDATE memory.evidence_extraction_job AS extraction_job
    SET status=disposition_target_status,
      lease_token=NULL,lease_expires_at=NULL,worker_id=p_worker_id,last_error=NULL,
      result=jsonb_set(
        extraction_job.result,'{final}',
        jsonb_build_object(
          'status',disposition_target_status::text,
          'sha256',request_receipt_sha,
          'payload',prefilter_receipt
        ),true
      )
    WHERE extraction_job.owner_user_id=actor
    ...
    RETURNING extraction_job.* INTO target_job;
    IF pg_column_size(target_job.result)>32768 THEN
      RAISE EXCEPTION 'eligibility disposition result exceeds job budget'
        USING ERRCODE='22023';
    END IF;
    ...
    INSERT INTO memory.evidence_extraction_event(
      ...
      actor,exact_job,p_operation_id,disposition_event_type,prior_status,
      disposition_target_status,'worker',p_worker_id,
      jsonb_build_object(
        'decision',disposition_decision,
        'exchange_id',prefilter_receipt->>'exchange_id',
        'external_model_calls',0,
        'gate_result_sha256',prefilter_receipt->>'gate_result_sha256',
        'lineage_sha256',prefilter_receipt->>'lineage_sha256',
        'reason_codes',prefilter_receipt->'reason_codes',
        'result_sha256',request_receipt_sha
      )
    );
    RETURN QUERY
    SELECT target_job.job_id,target_job.evidence_id,NULL::uuid,
      ...
      target_job.checkpoint_sequence,target_job.checkpoint_sha256,
      target_job.result,NULL::uuid,
      ('eligibility_'||disposition_decision)::text,0,0,'applied'::text
    FROM memory.evidence AS evidence
    WHERE evidence.owner_user_id=actor
      AND evidence.evidence_id=target_job.evidence_id;
    RETURN;
  ...
  IF reservation_mode='memory_v1_openai_request_reservation_v1' THEN
    ...
    IF jsonb_typeof(request_receipt)<>'object'
       ...
       OR EXISTS (
         ...
         WHERE key_name<>ALL(ARRAY[
           ...
           'eligibility_disposition','evidence_content_sha256',
           'gate_policy_sha256',
       ...
       OR (SELECT count(*) FROM jsonb_object_keys(request_receipt))<>38
       OR EXISTS (
      ...
      RAISE EXCEPTION 'provider request receipt is invalid'
        USING ERRCODE='22023';
    END IF;
    prefilter_receipt := request_receipt->'eligibility_disposition';
    IF jsonb_typeof(prefilter_receipt)<>'object'
       OR pg_column_size(prefilter_receipt)>8192
       OR prefilter_receipt->>'contract_version'
          <>'memory_v1_personal_evidence_disposition_receipt_v2'
       OR prefilter_receipt->>'decision'<>'send_external'
       OR prefilter_receipt->>'owner_user_id'<>actor::text
       OR prefilter_receipt->>'job_id'<>exact_job::text
       OR prefilter_receipt->>'evidence_content_sha256'<>exact_content_sha
       OR (prefilter_receipt->>'external_model_calls')::integer<>0
       ...
       OR prefilter_receipt->>'gate_result_sha256' !~ '^[0-9a-f]{64}$'
       OR prefilter_receipt->>'lineage_sha256' !~ '^[0-9a-f]{64}$'
       OR prefilter_receipt->>'policy_sha256'
          <>request_receipt->>'gate_policy_sha256'
       OR prefilter_receipt->>'selected_spans_sha256' !~ '^[0-9a-f]{64}$'
       OR jsonb_typeof(prefilter_receipt->'reason_codes')<>'array'
       OR jsonb_array_length(prefilter_receipt->'reason_codes') NOT BETWEEN 1 AND 16
       OR EXISTS (
       ...
       OR (prefilter_receipt->>'context_turn_count')::integer NOT BETWEEN 0 AND 8
       OR (prefilter_receipt->>'selected_span_count')::integer NOT BETWEEN 1 AND 64
       OR jsonb_typeof(prefilter_receipt->'context_envelope_sha256')
          NOT IN ('null','string')
       OR (
         jsonb_typeof(prefilter_receipt->'context_envelope_sha256')='string'
       ...
       OR (SELECT count(*) FROM jsonb_object_keys(prefilter_receipt))<>19 THEN
      RAISE EXCEPTION 'provider eligibility disposition is invalid'
        USING ERRCODE='22023';
    END IF;
    expected_lineage_sha := encode(public.digest(convert_to(concat_ws('|',
      prefilter_receipt->>'contract_version',actor::text,
      prefilter_receipt->>'evidence_id',exact_job::text,exact_content_sha,
      prefilter_receipt->>'exchange_id',prefilter_receipt->>'decision',
      prefilter_receipt->>'gate_result_sha256',
      prefilter_receipt->>'policy_sha256',
      coalesce(prefilter_receipt->>'context_envelope_sha256',''),
      coalesce(prefilter_receipt->>'source_gate_sha256',''),
    ...
    IF prefilter_receipt->>'lineage_sha256'<>expected_lineage_sha THEN
      RAISE EXCEPTION 'provider eligibility lineage binding is invalid'
        USING ERRCODE='23514';
    END IF;
    IF (
      (
  ...
  IF reservation_mode='memory_v1_openai_request_reservation_v1' THEN
    ...
    INSERT INTO memory.openai_provider_request_receipt_v1(
      ...
      request_receipt_sha,request_receipt,p_provider_id,p_provider_version,
