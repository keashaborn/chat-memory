...
AS $function$
  ...
  external_leaves AS (
    ...
    JOIN memory.evidence AS evidence
     ...
     AND evidence.evidence_id=packet.evidence_id
    WHERE actor.owner_user_id IS NOT NULL
      AND packet.evidence_id=(
        SELECT selected.evidence_id
        FROM memory.evidence_extraction_packet_v5 AS selected
        WHERE selected.owner_user_id=actor.owner_user_id
          AND selected.packet_id=p_packet_id
      )
      AND packet.normalized_packet->>'contract_version'
            ='memory_v1_relational_extraction_v5_2'
      AND packet.normalized_packet->>'predicate_registry_version'
  ...
  external_eligible AS (
    SELECT
      ...
      false AS terminal_eligible
    FROM external_leaves AS packet
    WHERE packet.packet_id=p_packet_id
      AND (SELECT count(*) FROM external_leaves)=1
      AND NOT EXISTS (
        SELECT 1
        FROM memory.v5_2_openai_packet_route_event AS prior
        WHERE prior.owner_user_id=memory.current_actor_user_id()
          AND prior.packet_id=packet.packet_id
      )
      AND NOT EXISTS (
        SELECT 1
        FROM memory.relational_stage_batch AS stage
        WHERE stage.owner_user_id=memory.current_actor_user_id()
          AND stage.evidence_id=packet.evidence_id
      )
  ),
  eligible AS (
    SELECT * FROM local_eligible
    UNION ALL
    SELECT * FROM external_eligible
  ...
  WHERE value.selected_route IS NOT NULL
    AND (SELECT count(*) FROM eligible)=1;
$function$;
