...
BEGIN
  IF current_user<>'sage' THEN
    RAISE EXCEPTION 'exact-job retry6 migration requires sage';
  END IF;
  ...
  IF before_sha<>'e67b5bbd08e9eb40283b29ee436f8aec6e3633439d65fb5897d2bc9ef9938b09'
     OR (length(function_definition)-length(replace(
       function_definition,
       'p_max_attempts NOT BETWEEN 1 AND 5',''
     )))<>length('p_max_attempts NOT BETWEEN 1 AND 5') THEN
    RAISE EXCEPTION 'exact-job claim function drifted before retry6 migration';
  END IF;
  ...
  IF after_sha<>'9768f3cf0d96dcc926a94a3f4d4260e6ada33eb45714851757e0604ce961ef6a'
    ...
    RAISE EXCEPTION 'exact-job retry6 migration postcondition failed';
  END IF;
END
$migration$;
