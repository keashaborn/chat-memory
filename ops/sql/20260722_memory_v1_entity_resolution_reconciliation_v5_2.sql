...
BEGIN
  IF current_user <> 'sage'
    ...
    RAISE EXCEPTION 'V5.2 entity reconciliation prerequisites are absent';
  END IF;
END
$preflight$;

CREATE TABLE IF NOT EXISTS memory.entity_resolution_reconciliation_v5_2 (
...
CREATE INDEX IF NOT EXISTS entity_resolution_reconciliation_v5_2_target_idx
  ON memory.entity_resolution_reconciliation_v5_2(
    owner_user_id, target_entity_id, created_at DESC
  );

CREATE OR REPLACE FUNCTION memory.preflight_entity_resolution_reconciliation_v5_2(
...
BEGIN
  ...
  IF NOT FOUND
     OR source_plan.predicate_registry_version <> 'memory_predicate_registry_v5_2'
    ...
    RAISE EXCEPTION 'owner-scoped V5.2 create-new review plan not found'
      USING ERRCODE = 'P0002';
  END IF;
  IF EXISTS (
    SELECT 1
  ...
  candidate_hash := memory.v5_digest_text(concat_ws('|',
    'memory_v1_entity_reconciliation_candidate_v5_2', actor::text,
  ...
  decision_hash := memory.v5_digest_text(concat_ws('|',
    'memory_v1_entity_reconciliation_decision_v5_2', actor::text,
  ...
  manifest := memory.v5_digest_text(concat_ws('|',
    'memory_v1_entity_reconciliation_v5_2', actor::text,
    source_plan.resolution_id::text, source_plan.decision_sha256,
...
CREATE OR REPLACE FUNCTION memory.reconcile_entity_resolution_v5_2(
  p_request_id uuid,
  p_source_resolution_id uuid,
  p_successor_resolution_id uuid,
  p_target_entity_id uuid,
...
DECLARE
  ...
  source_plan memory.entity_resolution_plan%ROWTYPE;
  mention memory.entity_mention%ROWTYPE;
  existing memory.entity_resolution_reconciliation_v5_2%ROWTYPE;
...
BEGIN
  ...
  FROM memory.entity_resolution_reconciliation_v5_2 AS prior
  ...
  INSERT INTO memory.entity_resolution_plan(
    ...
    'memory_v1_entity_resolution_reconciliation_v5_2',
  ...
  INSERT INTO memory.entity_resolution_reconciliation_v5_2(
...
CREATE OR REPLACE FUNCTION memory.preflight_entity_resolution_review_v5_2(
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'owner-scoped V5.2 resolution not found'
      USING ERRCODE = 'P0002';
  END IF;
  RETURN QUERY SELECT *
...
CREATE OR REPLACE FUNCTION memory.review_entity_resolution_v5_2(
  p_request_id uuid,
  p_resolution_id uuid,
  p_decision memory.entity_review_decision,
  p_reason text,
...
CREATE OR REPLACE FUNCTION memory.preflight_entity_resolution_apply_v5_2(
  p_resolution_id uuid,
...
CREATE OR REPLACE FUNCTION memory.apply_entity_resolution_v5_2(
  p_request_id uuid,
  p_resolution_id uuid,
  p_review_id uuid,
...
ALTER TABLE memory.entity_resolution_reconciliation_v5_2
  OWNER TO memory_v5_writer;
ALTER FUNCTION memory.preflight_entity_resolution_reconciliation_v5_2(
  uuid, uuid, uuid, text
) OWNER TO memory_v5_writer;
ALTER FUNCTION memory.reconcile_entity_resolution_v5_2(
  uuid, uuid, uuid, uuid, text, text
) OWNER TO memory_v5_writer;
ALTER FUNCTION memory.preflight_entity_resolution_review_v5_2(
  uuid, memory.entity_review_decision, text
) OWNER TO memory_v5_writer;
ALTER FUNCTION memory.review_entity_resolution_v5_2(
  uuid, uuid, memory.entity_review_decision, text, text
) OWNER TO memory_v5_writer;
ALTER FUNCTION memory.preflight_entity_resolution_apply_v5_2(uuid, uuid)
  OWNER TO memory_v5_writer;
ALTER FUNCTION memory.apply_entity_resolution_v5_2(uuid, uuid, uuid, text)
  OWNER TO memory_v5_writer;
...
DROP TRIGGER IF EXISTS entity_resolution_reconciliation_v5_2_append_only_guard
  ON memory.entity_resolution_reconciliation_v5_2;
CREATE TRIGGER entity_resolution_reconciliation_v5_2_append_only_guard
BEFORE UPDATE OR DELETE ON memory.entity_resolution_reconciliation_v5_2
FOR EACH ROW EXECUTE FUNCTION memory.guard_v5_append_only();

REVOKE ALL ON memory.entity_resolution_reconciliation_v5_2
  FROM PUBLIC, brains_app;
GRANT SELECT, INSERT ON memory.entity_resolution_reconciliation_v5_2
  TO memory_v5_writer;

REVOKE ALL ON FUNCTION memory.preflight_entity_resolution_reconciliation_v5_2(
  uuid, uuid, uuid, text
) FROM PUBLIC, brains_app;
REVOKE ALL ON FUNCTION memory.reconcile_entity_resolution_v5_2(
  uuid, uuid, uuid, uuid, text, text
) FROM PUBLIC, brains_app;
REVOKE ALL ON FUNCTION memory.preflight_entity_resolution_review_v5_2(
  uuid, memory.entity_review_decision, text
) FROM PUBLIC, brains_app;
REVOKE ALL ON FUNCTION memory.review_entity_resolution_v5_2(
  uuid, uuid, memory.entity_review_decision, text, text
) FROM PUBLIC, brains_app;
REVOKE ALL ON FUNCTION memory.preflight_entity_resolution_apply_v5_2(uuid, uuid)
  FROM PUBLIC, brains_app;
REVOKE ALL ON FUNCTION memory.apply_entity_resolution_v5_2(
  uuid, uuid, uuid, text
) FROM PUBLIC, brains_app;

GRANT EXECUTE ON FUNCTION memory.preflight_entity_resolution_reconciliation_v5_2(
  uuid, uuid, uuid, text
) TO brains_app;
GRANT EXECUTE ON FUNCTION memory.reconcile_entity_resolution_v5_2(
  uuid, uuid, uuid, uuid, text, text
) TO brains_app;
GRANT EXECUTE ON FUNCTION memory.preflight_entity_resolution_review_v5_2(
  uuid, memory.entity_review_decision, text
) TO brains_app;
GRANT EXECUTE ON FUNCTION memory.review_entity_resolution_v5_2(
  uuid, uuid, memory.entity_review_decision, text, text
) TO brains_app;
GRANT EXECUTE ON FUNCTION memory.preflight_entity_resolution_apply_v5_2(uuid, uuid)
  TO brains_app;
GRANT EXECUTE ON FUNCTION memory.apply_entity_resolution_v5_2(
  uuid, uuid, uuid, text
) TO brains_app;
