...
BEGIN
  IF session_user<>'sage' THEN
    RAISE EXCEPTION 'local guard reextract install requires sage';
  END IF;
  IF to_regrole('brains_app') IS NULL
    ...
    RAISE EXCEPTION 'local guard reextract prerequisites are absent';
  END IF;
  IF NOT EXISTS (
...
CREATE OR REPLACE FUNCTION memory.plan_owner_v5_local_guard_reextract_v1(
...
DECLARE
  ...
  prior_compiler_sha text := encode(public.digest(convert_to(
    to_jsonb('memory_v1_local_policy_compiler_v4'::text)::text,'UTF8'
  ),'sha256'),'hex');
BEGIN
  IF session_user<>'brains_app' THEN
    RAISE EXCEPTION 'local guard reextract plan requires brains_app session'
      USING ERRCODE='42501';
  END IF;
  ...
  SELECT
    ...
    '20260719_v5_compound_guard_reextract_v1'::text,
    'memory_v1_local_policy_compiler_v5'::text
  ...
  WHERE packet.owner_user_id=actor
    ...
    AND packet.normalized_packet#>>'{deferrals,0,reason_code}'='transient_state'
...
CREATE OR REPLACE FUNCTION memory.enqueue_owner_v5_local_guard_reextract_v1(
  p_operation_id uuid,
  p_new_job_id uuid,
...
BEGIN
  IF session_user<>'brains_app' THEN
    RAISE EXCEPTION 'local guard reextract apply requires brains_app session'
      USING ERRCODE='42501';
  END IF;
  actor := memory.current_actor_user_id();
  IF actor IS NULL THEN
  ...
  IF p_operation_id IS NULL OR p_new_job_id IS NULL
     ...
     OR p_selector_version<>'20260719_v5_compound_guard_reextract_v1'
     OR p_next_policy_compiler_version<>
       'memory_v1_local_policy_compiler_v5' THEN
    RAISE EXCEPTION 'local guard reextract inputs are invalid'
      USING ERRCODE='22023';
  END IF;

  PERFORM pg_advisory_xact_lock(hashtextextended(concat_ws('|',
    'memory_v1_v5_local_guard_reextract_v1',actor::text,
  ...
  IF FOUND THEN
    ...
    IF replay_job.job_id IS NULL OR replay_terminal.terminal_id IS NULL
      ...
      RAISE EXCEPTION 'local guard reextract replay conflicts'
        USING ERRCODE='23514';
    END IF;
  ...
  SELECT * INTO planned
  FROM memory.plan_owner_v5_local_guard_reextract_v1(p_prior_packet_id);
  IF NOT FOUND
     ...
     OR planned.reason_code<>'transient_state'
    ...
    RAISE EXCEPTION 'local guard reextract plan changed'
      USING ERRCODE='23514';
  END IF;
  IF EXISTS (
    ...
    RAISE EXCEPTION 'local guard reextract target identifiers conflict'
      USING ERRCODE='23514';
  END IF;
  ...
  fingerprint := encode(public.digest(convert_to(jsonb_build_object(
    ...
    'reason_code','compiler_guard_false_positive',
  ...
  INSERT INTO memory.evidence_intake_terminal(
    ...
    fingerprint,actor,session_user,jsonb_build_object(
      ...
      'reextract_reason','compiler_guard_false_positive',
      'prior_packet_id',p_prior_packet_id,
      'prior_packet_storage_sha256',p_expected_packet_storage_sha256,
  ...
  INSERT INTO memory.evidence_extraction_event(
    ...
    'admin','local_guard_reextract_v1',jsonb_build_object(
      'prior_packet_id',p_prior_packet_id,
...
ALTER FUNCTION memory.plan_owner_v5_local_guard_reextract_v1(uuid)
  OWNER TO memory_v5_local_reextract_maintainer;
ALTER FUNCTION memory.enqueue_owner_v5_local_guard_reextract_v1(
  uuid,uuid,uuid,uuid,text,text,text,text
) OWNER TO memory_v5_local_reextract_maintainer;

REVOKE ALL ON FUNCTION memory.plan_owner_v5_local_guard_reextract_v1(uuid)
  FROM PUBLIC,brains_app,memory_v5_local_reextract_maintainer;
REVOKE ALL ON FUNCTION memory.enqueue_owner_v5_local_guard_reextract_v1(
  uuid,uuid,uuid,uuid,text,text,text,text
) FROM PUBLIC,brains_app,memory_v5_local_reextract_maintainer;
GRANT EXECUTE ON FUNCTION memory.plan_owner_v5_local_guard_reextract_v1(uuid)
  TO brains_app,memory_v5_local_reextract_maintainer;
GRANT EXECUTE ON FUNCTION memory.enqueue_owner_v5_local_guard_reextract_v1(
  uuid,uuid,uuid,uuid,text,text,text,text
) TO brains_app;
