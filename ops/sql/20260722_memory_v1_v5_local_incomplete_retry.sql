...
CREATE OR REPLACE FUNCTION memory.requeue_owner_local_incomplete_failure_v1(
  p_operation_id uuid,
  p_job_id uuid,
...
DECLARE
  ...
  rejection_code constant text := 'local_incomplete_response';
BEGIN
  IF session_user<>'brains_app' THEN
    RAISE EXCEPTION 'local incomplete retry requires brains_app session'
      USING ERRCODE='42501';
  END IF;
  ...
  IF p_operation_id IS NULL OR p_job_id IS NULL
     ...
     OR p_reason_code<>'bounded_output_ceiling_increase' THEN
    RAISE EXCEPTION 'local incomplete retry inputs are invalid'
      USING ERRCODE='22023';
  END IF;

  PERFORM pg_advisory_xact_lock(hashtextextended(
    concat_ws('|','memory_v1_v5_local_incomplete_retry',actor::text,
      p_job_id::text),0
  ));
  ...
  IF FOUND THEN
    IF replayed.job_id<>p_job_id OR replayed.event_type<>'queued'
       ...
       OR replayed.actor_ref IS DISTINCT FROM 'local_incomplete_retry'
      ...
      RAISE EXCEPTION 'local incomplete retry replay conflicts'
        USING ERRCODE='23514';
    END IF;
  ...
  IF NOT FOUND OR current_job.status<>'skipped'
    ...
    RAISE EXCEPTION 'local incomplete retry job binding is invalid'
      USING ERRCODE='23514';
  END IF;
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'local incomplete retry evidence or packets changed'
      USING ERRCODE='23514';
  END IF;
  ...
  IF NOT FOUND OR prior_failure.event_type<>'skipped'
    ...
    RAISE EXCEPTION 'local incomplete retry failure binding is invalid'
      USING ERRCODE='23514';
  END IF;
  ...
  IF NOT FOUND OR completion.action<>'completed'
    ...
    RAISE EXCEPTION 'local incomplete retry ledger binding is invalid'
      USING ERRCODE='23514';
  END IF;
  ...
  INSERT INTO memory.evidence_extraction_event(
    ...
    'admin','local_incomplete_retry',jsonb_build_object(
...
ALTER FUNCTION memory.requeue_owner_local_incomplete_failure_v1(
  uuid,uuid,text,uuid,uuid,integer,text
) OWNER TO memory_v5_local_inference_maintainer;
REVOKE ALL ON FUNCTION memory.requeue_owner_local_incomplete_failure_v1(
  uuid,uuid,text,uuid,uuid,integer,text
) FROM PUBLIC,brains_app,memory_v5_local_inference_maintainer;
GRANT EXECUTE ON FUNCTION memory.requeue_owner_local_incomplete_failure_v1(
  uuid,uuid,text,uuid,uuid,integer,text
) TO brains_app;
