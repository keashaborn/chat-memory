...
BEGIN
  IF session_user<>'sage' THEN
    RAISE EXCEPTION 'zero-call validation retry install requires sage';
  END IF;
  IF to_regrole('memory_v5_local_inference_maintainer') IS NULL
    ...
    RAISE EXCEPTION 'zero-call validation retry prerequisites are absent';
  END IF;
END
$preflight$;
...
DECLARE
  ...
  rejection_code constant text := 'local_validation_rejected';
BEGIN
  IF session_user<>'brains_app' THEN
    RAISE EXCEPTION 'zero-call validation retry requires brains_app session'
      USING ERRCODE='42501';
  END IF;
  ...
  IF p_operation_id IS NULL OR p_job_id IS NULL
     ...
     OR p_reason_code<>'evidence_context_window_compiler_repair' THEN
    RAISE EXCEPTION 'zero-call validation retry inputs are invalid'
      USING ERRCODE='22023';
  END IF;

  PERFORM pg_advisory_xact_lock(hashtextextended(
    concat_ws('|','memory_v1_zero_call_validation_retry',actor::text,
      p_job_id::text),0
  ));
  ...
  IF FOUND THEN
    IF replayed.job_id<>p_job_id OR replayed.event_type<>'queued'
       ...
       OR replayed.actor_ref IS DISTINCT FROM
          'local_zero_call_validation_retry'
      ...
      RAISE EXCEPTION 'zero-call validation retry replay conflicts'
        USING ERRCODE='23514';
    END IF;
  ...
  IF NOT FOUND OR current_job.status<>'skipped'
    ...
    RAISE EXCEPTION 'zero-call validation retry job binding is invalid'
      USING ERRCODE='23514';
  END IF;
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'zero-call validation retry evidence or packets changed'
      USING ERRCODE='23514';
  END IF;
  ...
  IF NOT FOUND OR prior_failure.event_type<>'skipped'
    ...
    RAISE EXCEPTION 'zero-call validation retry failure binding is invalid'
      USING ERRCODE='23514';
  END IF;
  ...
  IF NOT FOUND OR completion.action<>'completed'
     ...
     OR completion.local_model_calls<>0 OR completion.external_model_calls<>0
     OR NOT EXISTS (
       ...
       WHERE reservation.owner_user_id=actor
         ...
         AND reservation.local_model_calls=0
         AND reservation.external_model_calls=0
     ) THEN
    RAISE EXCEPTION 'zero-call validation retry ledger binding is invalid'
      USING ERRCODE='23514';
  END IF;
  ...
  INSERT INTO memory.evidence_extraction_event(
    ...
    'admin','local_zero_call_validation_retry',jsonb_build_object(
