...
BEGIN
  IF to_regprocedure(
    ...
    RAISE EXCEPTION 'required V5.2 route authority functions are absent';
  END IF;
  IF NOT EXISTS (
    SELECT 1
    FROM pg_roles
    WHERE rolname='memory_v5_2_local_router_maintainer'
  ) THEN
    RAISE EXCEPTION 'required V5.2 router maintainer role is absent';
  END IF;
END
$block$;

CREATE OR REPLACE FUNCTION
  memory.plan_owner_v5_2_exact_packet_route_v1(p_packet_id uuid)
RETURNS TABLE(
  packet_id uuid,
  job_id uuid,
  evidence_id uuid,
...
COMMENT ON FUNCTION memory.plan_owner_v5_2_exact_packet_route_v1(uuid)
IS 'Owner-scoped exact-packet V5.2 route planner. Avoids global bounded-list pagination and performs no writes.';
