...
BEGIN
  IF session_user<>'sage' THEN
    RAISE EXCEPTION 'V5 local validation retry install requires sage';
  END IF;
...
CREATE OR REPLACE FUNCTION memory.requeue_owner_local_validation_failure_v1(
  p_operation_id uuid,
  p_job_id uuid,
...
BEGIN
  IF session_user<>'brains_app' THEN
    RAISE EXCEPTION 'local validation retry requires brains_app session'
      USING ERRCODE='42501';
  END IF;
  ...
  IF p_operation_id IS NULL OR p_job_id IS NULL
     ...
     OR p_rejection_code<>'local_validation_rejected'
     OR p_reason_code<>'registry_enum_compiler_repair' THEN
    RAISE EXCEPTION 'local validation retry inputs are invalid'
      USING ERRCODE='22023';
  END IF;

  PERFORM pg_advisory_xact_lock(hashtextextended(
    concat_ws('|','memory_v1_v5_local_validation_retry',actor::text,
      p_job_id::text),0
  ));
  ...
  IF FOUND THEN
    IF replayed.job_id<>p_job_id OR replayed.event_type<>'queued'
       ...
       OR replayed.actor_ref IS DISTINCT FROM 'local_validation_retry'
      ...
      RAISE EXCEPTION 'local validation retry replay conflicts'
        USING ERRCODE='23514';
    END IF;
  ...
  IF NOT FOUND OR current_job.status<>'skipped'
    ...
    RAISE EXCEPTION 'local validation retry job binding is invalid'
      USING ERRCODE='23514';
  END IF;
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'local validation retry evidence or packets changed'
      USING ERRCODE='23514';
  END IF;
  ...
  IF NOT FOUND OR prior_failure.event_type<>'skipped'
    ...
    RAISE EXCEPTION 'local validation retry failure binding is invalid'
      USING ERRCODE='23514';
  END IF;
  ...
  IF NOT FOUND OR completion.action<>'completed'
    ...
    RAISE EXCEPTION 'local validation retry ledger binding is invalid'
      USING ERRCODE='23514';
  END IF;
  ...
  INSERT INTO memory.evidence_extraction_event(
    ...
    'admin','local_validation_retry',jsonb_build_object(
...
ALTER FUNCTION memory.requeue_owner_local_validation_failure_v1(
  uuid,uuid,text,uuid,uuid,integer,text,text
) OWNER TO memory_v5_local_inference_maintainer;

REVOKE ALL ON FUNCTION memory.requeue_owner_local_validation_failure_v1(
  uuid,uuid,text,uuid,uuid,integer,text,text
) FROM PUBLIC,brains_app,memory_v5_local_inference_maintainer;
GRANT EXECUTE ON FUNCTION memory.requeue_owner_local_validation_failure_v1(
  uuid,uuid,text,uuid,uuid,integer,text,text
) TO brains_app;
