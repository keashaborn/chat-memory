...
BEGIN
  IF session_user <> 'sage' THEN
    RAISE EXCEPTION 'V5 local compiler retry v2 install requires sage';
  END IF;
  IF to_regrole('memory_v5_local_inference_maintainer') IS NULL
     ...
     OR to_regclass('memory.evidence_extraction_packet_v5_local') IS NULL THEN
    RAISE EXCEPTION 'V5 local compiler retry v2 prerequisites are absent';
  END IF;
END
$preflight$;

CREATE OR REPLACE FUNCTION memory.requeue_owner_local_compiler_failure_v2(
  p_operation_id uuid,
  p_job_id uuid,
...
BEGIN
  IF session_user <> 'brains_app' THEN
    RAISE EXCEPTION 'local compiler retry v2 requires brains_app session'
      USING ERRCODE='42501';
  END IF;
  ...
  IF p_operation_id IS NULL OR p_job_id IS NULL
     ...
     OR p_retry_contract_version <> 'memory_v1_local_policy_compiler_v4' THEN
    RAISE EXCEPTION 'local compiler retry v2 inputs are invalid'
      USING ERRCODE='22023';
  END IF;

  PERFORM pg_advisory_xact_lock(hashtextextended(
    concat_ws('|','memory_v1_v5_local_compiler_retry_v2',actor::text,
      p_job_id::text),0
  ));
  ...
  IF FOUND THEN
    IF replayed.job_id<>p_job_id OR replayed.event_type<>'queued'
       ...
       OR replayed.actor_ref IS DISTINCT FROM 'local_compiler_retry_v2'
       ...
       OR replayed.details->>'rejection_code'
          IS DISTINCT FROM p_rejection_code
       ...
       OR replayed.details->>'retry_contract_version'
          IS DISTINCT FROM p_retry_contract_version THEN
      RAISE EXCEPTION 'local compiler retry v2 replay conflicts'
        USING ERRCODE='23514';
    END IF;
  ...
  IF NOT FOUND OR current_job.status<>'skipped'
     ...
     OR current_job.last_error IS DISTINCT FROM
        'local_inference_rejected: '||p_rejection_code THEN
    RAISE EXCEPTION 'local compiler retry v2 job binding is invalid'
      USING ERRCODE='23514';
  END IF;
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'local compiler retry v2 evidence or packets changed'
      USING ERRCODE='23514';
  END IF;
  ...
  IF NOT FOUND OR prior_failure.event_type<>'skipped'
    ...
    RAISE EXCEPTION 'local compiler retry v2 failure binding is invalid'
      USING ERRCODE='23514';
  END IF;
  ...
  IF NOT FOUND OR completion.action<>'completed'
     ...
     OR completion.rejection_code<>p_rejection_code
     OR completion.target_content_sha256<>p_expected_content_sha256
     OR completion.policy_compiler_sha256<>p_prior_policy_compiler_sha256
     ...
     OR NOT EXISTS (
       ...
       WHERE reservation.owner_user_id=actor
         ...
         AND reservation.target_content_sha256=p_expected_content_sha256
         AND reservation.policy_compiler_sha256=
             p_prior_policy_compiler_sha256
     ) THEN
    RAISE EXCEPTION 'local compiler retry v2 ledger binding is invalid'
      USING ERRCODE='23514';
  END IF;
  ...
  INSERT INTO memory.evidence_extraction_event(
    ...
    'admin','local_compiler_retry_v2',jsonb_build_object(
      ...
      'attempts',p_expected_attempts,'rejection_code',p_rejection_code,
      'prior_policy_compiler_sha256',p_prior_policy_compiler_sha256,
      'retry_contract_version',p_retry_contract_version
    )
  );
  RETURN QUERY SELECT p_job_id,'pending'::text,p_expected_attempts,
    'applied'::text;
...
ALTER FUNCTION memory.requeue_owner_local_compiler_failure_v2(
  uuid,uuid,text,uuid,uuid,integer,text,text,text
) OWNER TO memory_v5_local_inference_maintainer;

REVOKE ALL ON FUNCTION memory.requeue_owner_local_compiler_failure_v2(
  uuid,uuid,text,uuid,uuid,integer,text,text,text
) FROM PUBLIC,brains_app,memory_v5_local_inference_maintainer;
GRANT EXECUTE ON FUNCTION memory.requeue_owner_local_compiler_failure_v2(
  uuid,uuid,text,uuid,uuid,integer,text,text,text
) TO brains_app;
