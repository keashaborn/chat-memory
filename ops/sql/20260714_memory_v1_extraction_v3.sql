...
BEGIN
  IF current_user <> 'sage' THEN
    RAISE EXCEPTION
      'memory V1 extraction v3 migration must run as sage, current_user=%',
      current_user;
  END IF;
  IF NOT EXISTS (
    SELECT 1
    ...
    RAISE EXCEPTION 'memory V1 extraction v2 migration is required';
  END IF;
END
$block$;

ALTER TABLE memory.consolidation_job
  ALTER COLUMN pipeline_version SET DEFAULT '20260714_v3';
...
BEGIN
  ...
  IF queued_job_id IS NOT NULL THEN
    INSERT INTO memory.consolidation_event(
      ...
      jsonb_build_object('pipeline_version', '20260714_v3')
    );
  END IF;
  RETURN NEW;
END
...
BEGIN
  ...
  IF p_pipeline_version <> '20260714_v3' THEN
