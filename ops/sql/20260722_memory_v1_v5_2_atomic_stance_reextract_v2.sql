...
BEGIN
  IF session_user<>'sage' THEN
    RAISE EXCEPTION 'local V5.2 atomic stance reextract install requires sage';
  END IF;
  IF to_regrole('brains_app') IS NULL
    ...
    RAISE EXCEPTION 'local V5.2 atomic stance reextract prerequisites are absent';
  END IF;
  IF NOT EXISTS (
...
BEGIN
  ...
  IF source_sha=expected_after THEN
    IF (length(source)-length(replace(source,new_fragment,'')))
      ...
      RAISE EXCEPTION 'V5.2 persistence v5 installed body changed'
        USING ERRCODE='23514';
    END IF;
    RETURN;
  END IF;
  IF source_sha<>expected_before
    ...
    RAISE EXCEPTION 'V5.2 persistence v5 baseline changed: %',source_sha
      USING ERRCODE='23514';
  END IF;
  source:=replace(source,old_fragment,new_fragment);
  source_sha:=encode(public.digest(convert_to(source,'UTF8'),'sha256'),'hex');
  IF source_sha<>expected_after THEN
    RAISE EXCEPTION 'V5.2 persistence v5 replacement changed: %',source_sha
      USING ERRCODE='23514';
  END IF;
  EXECUTE source;
END
...
CREATE OR REPLACE FUNCTION memory.plan_owner_v5_2_atomic_stance_reextract_v2(
...
BEGIN
  IF session_user<>'brains_app' THEN
    RAISE EXCEPTION 'local V5.2 atomic stance reextract plan requires brains_app session'
      USING ERRCODE='42501';
  END IF;
  ...
  SELECT
    ...
    'semantic_stance_atomic_split_required'::text,
    '20260722_v5_2_atomic_stance_reextract_v2'::text,
    'memory_v1_semantic_policy_compiler_v5'::text
  ...
  WHERE packet.owner_user_id=actor
    ...
    AND jsonb_array_length(packet.normalized_packet->'observations')=1
    AND jsonb_array_length(packet.normalized_packet->'comparison_hints')=0
    AND jsonb_array_length(packet.normalized_packet->'deferrals')=0
    AND packet.normalized_packet#>>'{entity_mentions,0,entity_type}'='self'
    AND packet.normalized_packet#>>'{observations,0,predicate}'=
        'stance.reported'
    AND packet.normalized_packet#>>'{observations,0,modality}'=
        'reported_belief'
    AND packet.normalized_packet#>>'{observations,0,projection_class}'=
        'reported_stance'
    AND EXISTS (
      SELECT 1
    ...
    AND NOT EXISTS (
      ...
      WHERE next_job.owner_user_id=actor
        ...
        AND next_job.selector_version=
          '20260722_v5_2_atomic_stance_reextract_v2'
    );
END
$function$;

CREATE OR REPLACE FUNCTION memory.enqueue_owner_v5_2_atomic_stance_reextract_v2(
  p_operation_id uuid,
  p_new_job_id uuid,
...
BEGIN
  IF session_user<>'brains_app' THEN
    RAISE EXCEPTION 'local V5.2 atomic stance reextract apply requires brains_app session'
      USING ERRCODE='42501';
  END IF;
  actor := memory.current_actor_user_id();
  IF actor IS NULL THEN
  ...
  IF p_operation_id IS NULL OR p_new_job_id IS NULL
     ...
     OR p_selector_version<>'20260722_v5_2_atomic_stance_reextract_v2'
     OR p_next_policy_compiler_version<>
       'memory_v1_semantic_policy_compiler_v5' THEN
    RAISE EXCEPTION 'local V5.2 atomic stance reextract inputs are invalid'
      USING ERRCODE='22023';
  END IF;

  PERFORM pg_advisory_xact_lock(hashtextextended(concat_ws('|',
    'memory_v1_v5_2_atomic_stance_reextract_v2',actor::text,
  ...
  IF FOUND THEN
    ...
    IF replay_job.job_id IS NULL OR replay_terminal.terminal_id IS NULL
       ...
       OR replay_event.actor_ref IS DISTINCT FROM
         'local_v5_2_atomic_stance_reextract_v2'
      ...
      RAISE EXCEPTION 'local V5.2 atomic stance reextract replay conflicts'
        USING ERRCODE='23514';
    END IF;
  ...
  SELECT * INTO planned
  FROM memory.plan_owner_v5_2_atomic_stance_reextract_v2(p_prior_packet_id);
  IF NOT FOUND
     ...
     OR planned.reason_code<>'semantic_stance_atomic_split_required'
    ...
    RAISE EXCEPTION 'local V5.2 atomic stance reextract plan changed'
      USING ERRCODE='23514';
  END IF;
  IF EXISTS (
    ...
    RAISE EXCEPTION 'local V5.2 atomic stance reextract target identifiers conflict'
      USING ERRCODE='23514';
  END IF;
  ...
  fingerprint := encode(public.digest(convert_to(jsonb_build_object(
    ...
    'reason_code','semantic_stance_atomic_split_required',
  ...
  INSERT INTO memory.evidence_intake_terminal(
    ...
    fingerprint,actor,session_user,jsonb_build_object(
      ...
      'reextract_reason','semantic_stance_atomic_split_required',
      'prior_packet_id',p_prior_packet_id,
      'prior_packet_storage_sha256',p_expected_packet_storage_sha256,
  ...
  INSERT INTO memory.evidence_extraction_event(
    ...
    'admin','local_v5_2_atomic_stance_reextract_v2',jsonb_build_object(
      'prior_packet_id',p_prior_packet_id,
...
ALTER FUNCTION memory.plan_owner_v5_2_atomic_stance_reextract_v2(uuid)
  OWNER TO memory_v5_local_reextract_maintainer;
ALTER FUNCTION memory.enqueue_owner_v5_2_atomic_stance_reextract_v2(
  uuid,uuid,uuid,uuid,text,text,text,text
) OWNER TO memory_v5_local_reextract_maintainer;

REVOKE ALL ON FUNCTION memory.plan_owner_v5_2_atomic_stance_reextract_v2(uuid)
  FROM PUBLIC,brains_app,memory_v5_local_reextract_maintainer;
REVOKE ALL ON FUNCTION memory.enqueue_owner_v5_2_atomic_stance_reextract_v2(
  uuid,uuid,uuid,uuid,text,text,text,text
) FROM PUBLIC,brains_app,memory_v5_local_reextract_maintainer;
GRANT EXECUTE ON FUNCTION memory.plan_owner_v5_2_atomic_stance_reextract_v2(uuid)
  TO brains_app,memory_v5_local_reextract_maintainer;
GRANT EXECUTE ON FUNCTION memory.enqueue_owner_v5_2_atomic_stance_reextract_v2(
  uuid,uuid,uuid,uuid,text,text,text,text
) TO brains_app;
