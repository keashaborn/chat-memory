...
BEGIN
  ...
  IF source_sha<>expected_before THEN
    RAISE EXCEPTION 'local circuit function baseline changed: %',source_sha
      USING ERRCODE='23514';
  END IF;
  IF (length(source)-length(replace(source,old_fragment,'')))
       / length(old_fragment)<>1 THEN
    RAISE EXCEPTION 'local circuit replacement anchor is not unique'
      USING ERRCODE='23514';
  END IF;
  source := replace(source,old_fragment,new_fragment);
  source_sha := encode(public.digest(convert_to(source,'UTF8'),'sha256'),'hex');
  IF source_sha<>expected_after THEN
    RAISE EXCEPTION 'local circuit replacement hash changed: %',source_sha
      USING ERRCODE='23514';
  END IF;
  EXECUTE source;
END
