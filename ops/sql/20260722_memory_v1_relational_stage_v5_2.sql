-- PostgreSQL enum values are additive and cannot be removed transactionally.
-- They are installed before the transactional function/constraint changes.
ALTER TYPE memory.observation_modality
  ADD VALUE IF NOT EXISTS 'reported_belief';
ALTER TYPE memory.observation_projection_class
  ADD VALUE IF NOT EXISTS 'reported_stance';
ALTER TYPE memory.observation_surface_policy
  ADD VALUE IF NOT EXISTS 'relevant_recall_or_explicit_recall';
...
BEGIN
  ...
  END IF;
  IF position('memory_predicate_registry_v5_2' IN constraint_definition)=0 THEN
...
CREATE OR REPLACE FUNCTION memory.stage_relational_packet_v5_2(
  p_request_id uuid,
  p_evidence_id uuid,
  p_extractor text,
...
BEGIN
  ...
  IF octet_length(p_extraction_packet_text) > 1048576
    ...
    RAISE EXCEPTION 'V5.2 packets exceed the 1 MiB limit'
      USING ERRCODE = '54000';
  END IF;
  ...
  IF NOT memory.v5_jsonb_exact_keys(extraction_packet, ARRAY[
    ...
    RAISE EXCEPTION 'V5.2 packet top-level fields mismatch'
      USING ERRCODE = '23514';
  END IF;
  IF extraction_packet->>'contract_version' <> 'memory_v1_relational_extraction_v5_2'
     OR extraction_packet->>'predicate_registry_version' <> 'memory_predicate_registry_v5_2'
     OR resolution_packet->>'contract_version' <> 'memory_v1_entity_resolution_review_v5_2'
     OR resolution_packet->>'predicate_registry_version' <> 'memory_predicate_registry_v5_2'
    ...
    RAISE EXCEPTION 'V5.2 packet version or resolver packet hash mismatch'
      USING ERRCODE = '23514';
  END IF;
  IF jsonb_typeof(extraction_packet->'entity_mentions') <> 'array'
    ...
    RAISE EXCEPTION 'V5.2 packet arrays exceed contract limits'
      USING ERRCODE = '23514';
  END IF;
  ...
  IF NOT memory.v5_jsonb_exact_keys(extraction_source, ARRAY[
    ...
    RAISE EXCEPTION 'source envelopes do not match the V5.2 contract'
      USING ERRCODE = '23514';
  END IF;
  ...
  stage_manifest := memory.v5_digest_text(concat_ws('|',
    'memory_v1_stage_packet_v5_2', actor::text, p_evidence_id::text,
  ...
  LOOP
    ...
    INSERT INTO memory.entity_resolution_plan(
      ...
      'memory_predicate_registry_v5_2', 'memory_entity_normalization_v5',
  ...
  LOOP
    ...
    INSERT INTO memory.observation(
      ...
      observation->>'predicate', 'memory_predicate_registry_v5_2',
...
ALTER FUNCTION memory.stage_relational_packet_v5_2(
  uuid,uuid,text,text,text,text,text,text
) OWNER TO memory_v5_writer;
REVOKE ALL ON FUNCTION memory.stage_relational_packet_v5_2(
  uuid,uuid,text,text,text,text,text,text
) FROM PUBLIC,brains_app;
GRANT EXECUTE ON FUNCTION memory.stage_relational_packet_v5_2(
  uuid,uuid,text,text,text,text,text,text
) TO brains_app;

CREATE OR REPLACE FUNCTION memory.preflight_relational_stage_bundle_v5_2(
  p_evidence_id uuid,
...
BEGIN
  ...
  IF p_evidence_id IS NULL
    ...
    RAISE EXCEPTION 'V5.2 stage preflight inputs are invalid'
      USING ERRCODE='22023';
  END IF;
...
ALTER FUNCTION memory.preflight_relational_stage_bundle_v5_2(
  uuid,text,text,timestamptz
) OWNER TO memory_v5_writer;

REVOKE ALL ON FUNCTION memory.preflight_relational_stage_bundle_v5_2(
  uuid,text,text,timestamptz
) FROM PUBLIC,brains_app;
GRANT EXECUTE ON FUNCTION memory.preflight_relational_stage_bundle_v5_2(
  uuid,text,text,timestamptz
) TO brains_app;
