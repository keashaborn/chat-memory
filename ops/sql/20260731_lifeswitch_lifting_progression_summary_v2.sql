...
RETURNS TABLE(
  ...
  first_day date,last_day date,first_set_count integer,latest_set_count integer,
  first_max_load double precision,latest_max_load double precision,
  first_total_reps integer,latest_total_reps integer,
  first_total_volume double precision,latest_total_volume double precision,
  first_average_load double precision,latest_average_load double precision,
  first_load_unit text,latest_load_unit text,resolution_sources text[]
)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  per_exposure AS (
    SELECT base.exercise_id,
      ...
      CASE
        WHEN COALESCE(pg_catalog.sum(base.reps),0)>0
          THEN COALESCE(pg_catalog.sum(base.volume),0)::double precision
               / pg_catalog.sum(base.reps)::double precision
  ...
  aggregate_summary AS (
    SELECT ranked.exercise_id,pg_catalog.max(ranked.exercise_name)::text AS exercise_name,
      ...
      pg_catalog.max(ranked.set_count) FILTER (WHERE ranked.first_rank=1)::integer AS first_set_count,
      pg_catalog.max(ranked.set_count) FILTER (WHERE ranked.latest_rank=1)::integer AS latest_set_count,
      ...
      pg_catalog.max(ranked.average_load) FILTER (WHERE ranked.first_rank=1) AS first_average_load,
      pg_catalog.max(ranked.average_load) FILTER (WHERE ranked.latest_rank=1) AS latest_average_load,
  ...
  SELECT aggregate_summary.exercise_id,aggregate_summary.exercise_name,
    ...
    aggregate_summary.first_set_count,aggregate_summary.latest_set_count,
    ...
    aggregate_summary.first_average_load,aggregate_summary.latest_average_load,
...
ALTER FUNCTION lifeswitch_chat.read_lifting_progression_summary_v2(uuid,date,date)
  OWNER TO sage;
REVOKE ALL ON FUNCTION lifeswitch_chat.read_lifting_progression_summary_v2(uuid,date,date)
  FROM PUBLIC,brains_app;
GRANT EXECUTE ON FUNCTION lifeswitch_chat.read_lifting_progression_summary_v2(uuid,date,date)
  TO lifeswitch_chat_reader_v1;

COMMENT ON FUNCTION lifeswitch_chat.read_lifting_progression_summary_v2(uuid,date,date) IS
  'Owner-bound bounded strength-progression comparison with per-exposure set counts and average load.';
