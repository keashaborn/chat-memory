...
-- V5-native intake compatibility:
--   * evidence-ingest audit proves capture, not semantic processing;
--   * frozen V4 consolidation jobs must not suppress V5 extraction.
-- Legacy selector versions retain their original behavior. Versions beginning
-- with 20260717_v2 use the V5-native rules below.
...
BEGIN
  ...
  WITH unaccounted AS (
    ...
    WHERE evidence.owner_user_id=actor
      ...
      AND (
        p_selector_version LIKE '20260717_v2%'
        OR NOT EXISTS (
  ...
  with_job AS (
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE p_selector_version NOT LIKE '20260717_v2%'
        AND source_job.owner_user_id=actor
