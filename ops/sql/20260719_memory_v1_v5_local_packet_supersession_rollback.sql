BEGIN;
DROP FUNCTION IF EXISTS memory.finalize_owner_v5_local_packet_supersession_v1(
...
DROP TABLE IF EXISTS memory.v5_local_packet_supersession;
...
GRANT EXECUTE ON FUNCTION
  memory.plan_owner_v5_local_packet_disposition_v1(integer)
  TO brains_app,memory_v5_local_disposition_maintainer;

DO $cleanup$
BEGIN
  IF to_regrole('memory_v5_local_supersession_maintainer') IS NOT NULL THEN
    ...
    REVOKE EXECUTE ON FUNCTION public.digest(bytea,text)
      FROM memory_v5_local_supersession_maintainer;
    DROP ROLE memory_v5_local_supersession_maintainer;
  END IF;
END
$cleanup$;
COMMIT;
