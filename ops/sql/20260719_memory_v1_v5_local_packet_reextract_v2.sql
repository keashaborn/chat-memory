...
BEGIN
  IF session_user<>'sage' THEN
    RAISE EXCEPTION 'local packet reextract v2 install requires sage';
  END IF;
  IF to_regrole('brains_app') IS NULL
    ...
    RAISE EXCEPTION 'local packet reextract v2 prerequisites are absent';
  END IF;
  IF NOT EXISTS (
...
CREATE OR REPLACE FUNCTION memory.plan_owner_v5_local_packet_reextract_v2(
...
BEGIN
  IF session_user<>'brains_app' THEN
    RAISE EXCEPTION 'local packet reextract v2 plan requires brains_app session'
      USING ERRCODE='42501';
  END IF;
  ...
  SELECT
    ...
    'temporal_persistence_matrix_mismatch'::text,
    '20260719_v5_temporal_contract_reextract_v1'::text,
...
CREATE OR REPLACE FUNCTION memory.enqueue_owner_v5_local_packet_reextract_v2(
  p_operation_id uuid,
  p_new_job_id uuid,
...
BEGIN
  IF session_user<>'brains_app' THEN
    RAISE EXCEPTION 'local packet reextract v2 apply requires brains_app session'
      USING ERRCODE='42501';
  END IF;
  actor := memory.current_actor_user_id();
  IF actor IS NULL THEN
  ...
  IF p_operation_id IS NULL OR p_new_job_id IS NULL
     ...
     OR p_selector_version<>'20260719_v5_temporal_contract_reextract_v1'
     OR p_next_policy_compiler_version<>
       'memory_v1_local_policy_compiler_v6' THEN
    RAISE EXCEPTION 'local packet reextract v2 inputs are invalid'
      USING ERRCODE='22023';
  END IF;

  PERFORM pg_advisory_xact_lock(hashtextextended(concat_ws('|',
    'memory_v1_v5_local_packet_reextract_v2',actor::text,
  ...
  IF FOUND THEN
    ...
    IF replay_job.job_id IS NULL OR replay_terminal.terminal_id IS NULL
      ...
      RAISE EXCEPTION 'local packet reextract v2 replay conflicts'
        USING ERRCODE='23514';
    END IF;
  ...
  SELECT * INTO planned
  FROM memory.plan_owner_v5_local_packet_reextract_v2(p_prior_packet_id);
  IF NOT FOUND
     ...
     OR planned.reason_code<>'temporal_persistence_matrix_mismatch'
    ...
    RAISE EXCEPTION 'local packet reextract v2 plan changed'
      USING ERRCODE='23514';
  END IF;
  IF EXISTS (
    ...
    RAISE EXCEPTION 'local packet reextract v2 target identifiers conflict'
      USING ERRCODE='23514';
  END IF;
  ...
  fingerprint := encode(public.digest(convert_to(jsonb_build_object(
    ...
    'reason_code','temporal_persistence_matrix_mismatch',
  ...
  INSERT INTO memory.evidence_intake_terminal(
    ...
    fingerprint,actor,session_user,jsonb_build_object(
      ...
      'reextract_reason','temporal_persistence_matrix_mismatch',
      'prior_packet_id',p_prior_packet_id,
      'prior_packet_storage_sha256',p_expected_packet_storage_sha256,
  ...
  INSERT INTO memory.evidence_extraction_event(
    ...
    'admin','local_packet_reextract_v2',jsonb_build_object(
      'prior_packet_id',p_prior_packet_id,
...
ALTER FUNCTION memory.plan_owner_v5_local_packet_reextract_v2(uuid)
  OWNER TO memory_v5_local_reextract_maintainer;
ALTER FUNCTION memory.enqueue_owner_v5_local_packet_reextract_v2(
  uuid,uuid,uuid,uuid,text,text,text,text
) OWNER TO memory_v5_local_reextract_maintainer;

REVOKE ALL ON FUNCTION memory.plan_owner_v5_local_packet_reextract_v2(uuid)
  FROM PUBLIC,brains_app,memory_v5_local_reextract_maintainer;
REVOKE ALL ON FUNCTION memory.enqueue_owner_v5_local_packet_reextract_v2(
  uuid,uuid,uuid,uuid,text,text,text,text
) FROM PUBLIC,brains_app,memory_v5_local_reextract_maintainer;
GRANT EXECUTE ON FUNCTION memory.plan_owner_v5_local_packet_reextract_v2(uuid)
  TO brains_app,memory_v5_local_reextract_maintainer;
GRANT EXECUTE ON FUNCTION memory.enqueue_owner_v5_local_packet_reextract_v2(
  uuid,uuid,uuid,uuid,text,text,text,text
) TO brains_app;
