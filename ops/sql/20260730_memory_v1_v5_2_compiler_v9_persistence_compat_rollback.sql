...
BEGIN
  ...
  IF definition_sha=new_definition_sha THEN
    IF (length(definition)-length(replace(definition,new_fragment,'')))
      ...
      RAISE EXCEPTION 'V5.2 compiler-v9 rollback guard is not replaceable';
    END IF;
    ...
    IF encode(public.digest(convert_to(
      ...
      RAISE EXCEPTION 'V5.2 compiler-v9 rollback changed';
    END IF;
    EXECUTE definition;
  ELSIF definition_sha=old_definition_sha THEN
    NULL;
  ELSE
    RAISE EXCEPTION 'V5.2 compiler-v9 rollback baseline changed: %',
      definition_sha;
  END IF;
END
$rollback$;
