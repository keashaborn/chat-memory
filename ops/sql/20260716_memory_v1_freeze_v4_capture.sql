...
BEGIN
  IF current_user <> 'sage' THEN
    RAISE EXCEPTION
      'memory V1 V4 capture freeze must run as sage, current_user=%',
      current_user;
  END IF;
  IF NOT EXISTS (
    SELECT 1
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'chat-log owner or immutability guard is not enabled';
  END IF;
END
$block$;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'V4 chat-log consolidation trigger was not disabled';
  END IF;
END
$block$;
