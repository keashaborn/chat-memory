...
BEGIN
  IF to_regrole('brains_app') IS NULL
     ...
     OR to_regprocedure('memory.enqueue_owner_evidence_extraction_v1(uuid,text,text,text,text)') IS NULL
    ...
    RAISE EXCEPTION 'legacy V5 reintake prerequisites are absent';
  END IF;
END
$preflight$;

CREATE OR REPLACE FUNCTION memory.plan_owner_v5_legacy_claim_reintake_v1(
...
BEGIN
  IF session_user<>'brains_app' THEN
    RAISE EXCEPTION 'legacy V5 reintake plan requires brains_app session'
      USING ERRCODE='42501';
  END IF;
  ...
  IF actor IS NULL THEN
    RAISE EXCEPTION 'app.user_id actor context is required'
      USING ERRCODE='42501';
  END IF;
  IF p_selector_version<>'20260719_v5_legacy_claim_reintake_v1'
     OR p_limit IS NULL OR p_limit<1 OR p_limit>100 THEN
    RAISE EXCEPTION 'legacy V5 reintake plan inputs are invalid'
      USING ERRCODE='22023';
  END IF;
  ...
  WHERE evidence.owner_user_id=actor
    ...
    AND EXISTS (
      ...
      WHERE link.owner_user_id=actor
        ...
        AND contract.lifecycle='legacy_read_only'
    )
    AND NOT EXISTS (
      SELECT 1
      FROM memory.claim_evidence AS link
...
CREATE OR REPLACE FUNCTION memory.enqueue_owner_v5_legacy_claim_reintake_v1(
  p_evidence_id uuid,
  p_selector_version text,
...
BEGIN
  IF session_user<>'brains_app' THEN
    RAISE EXCEPTION 'legacy V5 reintake enqueue requires brains_app session'
      USING ERRCODE='42501';
  END IF;
  actor := memory.current_actor_user_id();
  IF actor IS NULL THEN
  ...
  IF p_evidence_id IS NULL
     OR p_selector_version<>'20260719_v5_legacy_claim_reintake_v1'
    ...
    RAISE EXCEPTION 'legacy V5 reintake enqueue inputs are invalid'
      USING ERRCODE='22023';
  END IF;

  PERFORM pg_advisory_xact_lock(hashtextextended(
    concat_ws('|','legacy_v5_reintake',actor::text,p_evidence_id::text,
  ...
  IF job_found OR terminal_found THEN
    IF NOT job_found OR NOT terminal_found
      ...
      RAISE EXCEPTION 'legacy V5 reintake replay conflicts'
        USING ERRCODE='23514';
    END IF;
  ...
  IF NOT FOUND OR planned.outcome<>'eligible'
     ...
     OR planned.reason_code<>p_expected_reason_code THEN
    RAISE EXCEPTION 'legacy V5 reintake plan changed'
      USING ERRCODE='23514';
  END IF;
  ...
  INSERT INTO memory.evidence_extraction_event(
    ...
    'v5_legacy_claim_reintake',jsonb_build_object(
...
GRANT SELECT ON memory.evidence,memory.claim,memory.claim_evidence,
  memory.predicate_contract,memory.evidence_extraction_job,
  memory.relational_stage_batch,memory.observation_entailment_v5,
  memory.preference_revision_evidence,memory.project_knowledge_revision_evidence
TO memory_intake_maintainer;

ALTER FUNCTION memory.plan_owner_v5_legacy_claim_reintake_v1(text,integer,uuid)
  OWNER TO memory_intake_maintainer;
ALTER FUNCTION memory.enqueue_owner_v5_legacy_claim_reintake_v1(
  uuid,text,text,text,text
) OWNER TO memory_extraction_queue_maintainer;

GRANT EXECUTE ON FUNCTION
  memory.plan_owner_v5_legacy_claim_reintake_v1(text,integer,uuid)
...
REVOKE ALL ON FUNCTION
  memory.enqueue_owner_v5_legacy_claim_reintake_v1(uuid,text,text,text,text)
