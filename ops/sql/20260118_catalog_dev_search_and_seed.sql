...
-- Escapes %, _ and backslash for LIKE patterns.
...
-- Exercise search across canonical names + aliases (trigram + substring).
...
-- Food search across canonical names + aliases.
...
-- Seed: brands (minimal, for alias labeling)
...
-- Seed: canonical exercises/activities (small smoke set)
...
VALUES
  ...
  ('barbell_bench_press','Barbell Bench Press','strength','free_weight','horizontal_push',
    ARRAY['chest'], ARRAY['triceps','anterior_deltoid'], ARRAY['shoulder','elbow'], ARRAY['barbell','bench'], false,
    '{"type":"strength","fields":["sets","reps","load"]}'::jsonb,'seed'),

  ('deadlift_conventional','Deadlift (Conventional)','strength','free_weight','hinge',
    ARRAY['glutes','hamstrings'], ARRAY['erectors','lats','traps'], ARRAY['hip'], ARRAY['barbell'], false,
    '{"type":"strength","fields":["sets","reps","load"]}'::jsonb,'seed'),

  ('dumbbell_bench_press','Dumbbell Bench Press','strength','free_weight','horizontal_push',
    ARRAY['chest'], ARRAY['triceps','anterior_deltoid'], ARRAY['shoulder','elbow'], ARRAY['dumbbells','bench'], false,
    '{"type":"strength","fields":["sets","reps","load"]}'::jsonb,'seed'),

  ('pullup','Pull-Up','strength','bodyweight','vertical_pull',
    ARRAY['lats'], ARRAY['biceps','mid_back'], ARRAY['shoulder','elbow'], ARRAY['pullup_bar'], false,
    '{"type":"strength","fields":["sets","reps","added_load"]}'::jsonb,'seed'),
  ...
  ('leg_press_machine','Leg Press','strength','machine_selectorized','squat',
    ARRAY['quadriceps','glutes'], ARRAY['hamstrings'], ARRAY['knee','hip'], ARRAY['leg_press_machine'], false,
    '{"type":"strength","fields":["sets","reps","stack_load"]}'::jsonb,'seed'),
...
-- Seed: exercise aliases (includes brand-tagged machine names)
WITH
  b_hs AS (SELECT brand_id FROM catalog_dev.equipment_brand WHERE brand_name_norm = catalog_dev.norm_text('Hammer Strength')),
  b_tg AS (SELECT brand_id FROM catalog_dev.equipment_brand WHERE brand_name_norm = catalog_dev.norm_text('Technogym')),
  b_cy AS (SELECT brand_id FROM catalog_dev.equipment_brand WHERE brand_name_norm = catalog_dev.norm_text('Cybex')),
  b_pr AS (SELECT brand_id FROM catalog_dev.equipment_brand WHERE brand_name_norm = catalog_dev.norm_text('Precor')),
  b_mx AS (SELECT brand_id FROM catalog_dev.equipment_brand WHERE brand_name_norm = catalog_dev.norm_text('Matrix')),

  ex_pl_cp AS (SELECT exercise_id FROM catalog_dev.exercise WHERE slug = 'plate_loaded_chest_press'),
  ex_sel_cp AS (SELECT exercise_id FROM catalog_dev.exercise WHERE slug = 'selectorized_chest_press'),
  ex_lp AS (SELECT exercise_id FROM catalog_dev.exercise WHERE slug = 'selectorized_lat_pulldown'),
  ex_row AS (SELECT exercise_id FROM catalog_dev.exercise WHERE slug = 'seated_row_machine'),
  ex_tri AS (SELECT exercise_id FROM catalog_dev.exercise WHERE slug = 'cable_triceps_pushdown'),
  ex_bb_sq AS (SELECT exercise_id FROM catalog_dev.exercise WHERE slug = 'barbell_back_squat')

INSERT INTO catalog_dev.exercise_alias (exercise_id, alias, locale, brand_id, model_name, source, confidence)
SELECT ex_bb_sq.exercise_id, v.alias, 'en', NULL, NULL, 'seed', 0.90
FROM ex_bb_sq, (VALUES
  ('Back Squat'),
  ('BB Back Squat'),
  ('High Bar Squat')
) v(alias)
...
WITH ex AS (SELECT exercise_id FROM catalog_dev.exercise WHERE slug = 'plate_loaded_chest_press')
INSERT INTO catalog_dev.exercise_alias (exercise_id, alias, locale, brand_id, model_name, source, confidence)
SELECT ex.exercise_id, v.alias, 'en', b_hs.brand_id, NULL, 'seed', 0.95
FROM ex, b_hs, (VALUES
...
WITH ex AS (SELECT exercise_id FROM catalog_dev.exercise WHERE slug = 'selectorized_chest_press')
INSERT INTO catalog_dev.exercise_alias (exercise_id, alias, locale, brand_id, model_name, source, confidence)
SELECT ex.exercise_id, v.alias, 'en', b_tg.brand_id, NULL, 'seed', 0.85
FROM ex, b_tg, (VALUES
...
WITH ex AS (SELECT exercise_id FROM catalog_dev.exercise WHERE slug = 'selectorized_chest_press')
INSERT INTO catalog_dev.exercise_alias (exercise_id, alias, locale, brand_id, model_name, source, confidence)
SELECT ex.exercise_id, v.alias, 'en', b_cy.brand_id, NULL, 'seed', 0.80
FROM ex, b_cy, (VALUES
  ('Cybex Chest Press')
) v(alias)
ON CONFLICT (exercise_id, alias_norm, locale) DO NOTHING;
...
WITH ex AS (SELECT exercise_id FROM catalog_dev.exercise WHERE slug = 'selectorized_chest_press')
INSERT INTO catalog_dev.exercise_alias (exercise_id, alias, locale, brand_id, model_name, source, confidence)
SELECT ex.exercise_id, v.alias, 'en', b_pr.brand_id, NULL, 'seed', 0.80
FROM ex, b_pr, (VALUES
  ('Precor Chest Press')
) v(alias)
ON CONFLICT (exercise_id, alias_norm, locale) DO NOTHING;
...
WITH ex AS (SELECT exercise_id FROM catalog_dev.exercise WHERE slug = 'selectorized_chest_press')
INSERT INTO catalog_dev.exercise_alias (exercise_id, alias, locale, brand_id, model_name, source, confidence)
SELECT ex.exercise_id, v.alias, 'en', b_mx.brand_id, NULL, 'seed', 0.80
FROM ex, b_mx, (VALUES
  ('Matrix Chest Press')
) v(alias)
ON CONFLICT (exercise_id, alias_norm, locale) DO NOTHING;

WITH ex AS (SELECT exercise_id FROM catalog_dev.exercise WHERE slug = 'selectorized_lat_pulldown')
INSERT INTO catalog_dev.exercise_alias (exercise_id, alias, locale, brand_id, model_name, source, confidence)
SELECT ex.exercise_id, v.alias, 'en', NULL, NULL, 'seed', 0.90
FROM ex, (VALUES
  ('Lat Pulldown'),
  ('Lat Pull Down'),
  ('Pulldown Machine')
...
WITH ex AS (SELECT exercise_id FROM catalog_dev.exercise WHERE slug = 'seated_row_machine')
INSERT INTO catalog_dev.exercise_alias (exercise_id, alias, locale, brand_id, model_name, source, confidence)
SELECT ex.exercise_id, v.alias, 'en', NULL, NULL, 'seed', 0.85
FROM ex, (VALUES
  ('Seated Row'),
  ('Low Row Machine')
) v(alias)
...
WITH ex AS (SELECT exercise_id FROM catalog_dev.exercise WHERE slug = 'cable_triceps_pushdown')
INSERT INTO catalog_dev.exercise_alias (exercise_id, alias, locale, brand_id, model_name, source, confidence)
SELECT ex.exercise_id, v.alias, 'en', NULL, NULL, 'seed', 0.90
FROM ex, (VALUES
  ('Triceps Pushdown'),
...
-- Seed: a couple foods (no macros yet; just to test search plumbing)
