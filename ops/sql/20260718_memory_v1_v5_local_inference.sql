...
CREATE OR REPLACE FUNCTION memory.requeue_owner_local_transport_failure_v1(
  p_operation_id uuid,
  p_job_id uuid,
  p_expected_content_sha256 text,
...
BEGIN
  IF session_user<>'brains_app' THEN
    RAISE EXCEPTION 'local transport retry requires brains_app session'
      USING ERRCODE='42501';
  END IF;
  actor := memory.current_actor_user_id();
  IF actor IS NULL THEN
  ...
  IF p_operation_id IS NULL OR p_job_id IS NULL
     ...
     OR p_rejection_code NOT IN (
       'local_transport_timeout','local_transport_unavailable',
       'local_transport_rate_limited','local_transport_server_error'
     )
     OR p_reason_code<>'private_gpu_route_recovery' THEN
    RAISE EXCEPTION 'local transport retry inputs are invalid'
      USING ERRCODE='22023';
  END IF;

  PERFORM pg_advisory_xact_lock(hashtextextended(
    concat_ws('|','memory_v1_v5_local_transport_retry',actor::text,
      p_job_id::text),0
  ));
  ...
  IF FOUND THEN
    IF replayed.job_id<>p_job_id OR replayed.event_type<>'queued'
       ...
       OR replayed.actor_ref IS DISTINCT FROM 'local_transport_retry'
      ...
      RAISE EXCEPTION 'local transport retry replay conflicts'
        USING ERRCODE='23514';
    END IF;
  ...
  IF NOT FOUND OR current_job.status<>'skipped'
    ...
    RAISE EXCEPTION 'local transport retry job binding is invalid'
      USING ERRCODE='23514';
  END IF;
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'local transport retry evidence or packets changed'
      USING ERRCODE='23514';
  END IF;
  ...
  IF NOT FOUND OR prior_failure.event_type<>'skipped'
    ...
    RAISE EXCEPTION 'local transport retry failure binding is invalid'
      USING ERRCODE='23514';
  END IF;
  ...
  IF NOT FOUND OR completion.action<>'completed'
    ...
    RAISE EXCEPTION 'local transport retry ledger binding is invalid'
      USING ERRCODE='23514';
  END IF;
  ...
  INSERT INTO memory.evidence_extraction_event(
    ...
    'admin','local_transport_retry',jsonb_build_object(
...
ALTER FUNCTION memory.requeue_owner_local_transport_failure_v1(
...
REVOKE ALL ON FUNCTION memory.requeue_owner_local_transport_failure_v1(
  uuid,uuid,text,uuid,uuid,integer,text,text
) FROM PUBLIC,brains_app,memory_v5_local_inference_maintainer;
...
GRANT EXECUTE ON FUNCTION memory.requeue_owner_local_transport_failure_v1(
  uuid,uuid,text,uuid,uuid,integer,text,text
) TO brains_app;
