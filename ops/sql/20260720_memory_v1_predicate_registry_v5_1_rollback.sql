...
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION 'refusing to roll back an active predicate registry V5.1'
      USING ERRCODE = '55000';
  END IF;
  IF EXISTS (
    ...
    RAISE EXCEPTION 'refusing to roll back a used predicate registry V5.1'
      USING ERRCODE = '55000';
  END IF;
END
$guard$;

CREATE TEMP TABLE v5_1_created_predicate ON COMMIT DROP AS
SELECT predicate
FROM memory.predicate_registry_seed
WHERE registry_version = 'memory_predicate_registry_v5_1'
  AND base_predicate_created;

DROP TABLE IF EXISTS memory.relationship_predicate_contract_v5_1;
DROP TABLE IF EXISTS memory.predicate_registry_source_binding_v5_1;
DROP FUNCTION IF EXISTS memory.reject_predicate_registry_v5_1_mutation();
...
USING v5_1_created_predicate AS created
