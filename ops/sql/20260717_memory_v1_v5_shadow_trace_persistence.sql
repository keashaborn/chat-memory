...
BEGIN
  IF current_user <> 'sage' THEN
    RAISE EXCEPTION 'V5 shadow trace persistence migration requires sage';
  END IF;
  IF to_regrole('brains_app') IS NULL
    ...
    RAISE EXCEPTION 'V5 shadow trace persistence prerequisites are missing';
  END IF;
END
$preflight$;
...
BEGIN
  ...
  IF NOT FOUND THEN
    CREATE ROLE memory_v5_trace_writer
  ...
  ELSIF role_state.rolcanlogin
    ...
    RAISE EXCEPTION 'existing memory_v5_trace_writer role is not restricted';
  END IF;
END
$trace_role$;

CREATE OR REPLACE FUNCTION memory.v5_shadow_trace_sha256_valid(value text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
STRICT
...
CREATE OR REPLACE FUNCTION memory.v5_shadow_rejection_counts_valid(p_value jsonb)
RETURNS boolean
LANGUAGE plpgsql
IMMUTABLE
STRICT
...
BEGIN
  IF pg_catalog.jsonb_typeof(p_value) <> 'object'
     OR pg_catalog.pg_column_size(p_value) > 2048 THEN
    RETURN false;
  END IF;
  FOR item IN
  ...
  LOOP
    ...
    IF item_count > 32
       OR item.key !~ '^[a-z0-9_:-]{1,80}$'
       OR pg_catalog.jsonb_typeof(item.value) <> 'number'
       OR item.value::text !~ '^[0-9]+$'
       OR (item.value::text)::integer NOT BETWEEN 0 AND 100 THEN
      RETURN false;
    END IF;
  END LOOP;
  RETURN true;
...
CREATE OR REPLACE FUNCTION memory.guard_v5_shadow_trace_append_only()
RETURNS trigger
LANGUAGE plpgsql
SECURITY INVOKER
...
AS $function$
BEGIN
  RAISE EXCEPTION 'memory.v5_shadow_trace_event is append-only'
    USING ERRCODE='42501';
END
$function$;

CREATE TABLE IF NOT EXISTS memory.v5_shadow_trace_event (
  ...
  embedding_provider_calls integer NOT NULL,
  ...
  CHECK (trace_version='memory_v1_v5_shadow_trace_v1'),
  ...
  CHECK (intent IS NULL OR intent ~ '^[a-z0-9_.:-]{1,64}$'),
  CHECK (domain IS NULL OR domain ~ '^[a-z0-9_.:-]{1,64}$'),
  CHECK (status<>'ok' OR (intent IS NOT NULL AND domain IS NOT NULL)),
  CHECK (candidate_limit BETWEEN 1 AND 100),
  CHECK (max_claims BETWEEN 1 AND 32),
  CHECK (max_tokens BETWEEN 1 AND 10000),
  CHECK (candidate_count BETWEEN 0 AND candidate_limit),
  CHECK (visible_candidate_count BETWEEN 0 AND candidate_count),
  CHECK (selected_count BETWEEN 0 AND visible_candidate_count),
  CHECK (selected_count<=max_claims),
  CHECK (token_estimate BETWEEN 0 AND max_tokens),
  CHECK (memory.v5_shadow_rejection_counts_valid(rejected_counts)),
  CHECK (max_sensitivity IN ('low','medium','high','restricted')),
  CHECK (embedding_provider_calls BETWEEN 0 AND 1),
  CHECK (database_writes=0 AND qdrant_writes=0),
  CHECK (
...
CREATE INDEX IF NOT EXISTS v5_shadow_trace_owner_created_idx
  ON memory.v5_shadow_trace_event(owner_user_id,created_at DESC,trace_event_id);

CREATE OR REPLACE FUNCTION memory.require_v5_shadow_trace_writer_context()
RETURNS uuid
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  IF session_user<>'brains_app'
     OR current_user<>'memory_v5_trace_writer' THEN
    RAISE EXCEPTION 'V5 shadow trace writer requires the brains_app session'
      USING ERRCODE='42501';
  END IF;
  actor := memory.current_actor_user_id();
  IF actor IS NULL THEN
    RAISE EXCEPTION 'V5 shadow trace writer requires a transaction-local actor'
      USING ERRCODE='42501';
  END IF;
  RETURN actor;
END
...
CREATE OR REPLACE FUNCTION memory.record_v5_shadow_trace_v1(
  ...
  p_visible_candidate_count integer,
  ...
  p_embedding_provider_calls integer
)
RETURNS TABLE(
  trace_event_id uuid,
  outcome text,
...
DECLARE
  actor uuid;
  stored memory.v5_shadow_trace_event%ROWTYPE;
  event_id uuid;
...
BEGIN
  ...
  IF p_trace_version<>'memory_v1_v5_shadow_trace_v1'
     ...
     OR (p_intent IS NOT NULL AND p_intent !~ '^[a-z0-9_.:-]{1,64}$')
     OR (p_domain IS NOT NULL AND p_domain !~ '^[a-z0-9_.:-]{1,64}$')
     OR (p_status='ok' AND (p_intent IS NULL OR p_domain IS NULL))
     OR p_candidate_limit NOT BETWEEN 1 AND 100
     OR p_max_claims NOT BETWEEN 1 AND 32
     OR p_max_tokens NOT BETWEEN 1 AND 10000
     OR p_candidate_count NOT BETWEEN 0 AND p_candidate_limit
     OR p_visible_candidate_count NOT BETWEEN 0 AND p_candidate_count
     OR p_selected_count NOT BETWEEN 0 AND p_visible_candidate_count
     OR p_selected_count>p_max_claims
     OR p_token_estimate NOT BETWEEN 0 AND p_max_tokens
     OR NOT memory.v5_shadow_rejection_counts_valid(p_rejected_counts)
     OR p_max_sensitivity NOT IN ('low','medium','high','restricted')
     OR p_embedding_provider_calls NOT BETWEEN 0 AND 1 THEN
    RAISE EXCEPTION 'V5 shadow trace payload is invalid'
      USING ERRCODE='22023';
  END IF;

  SELECT COALESCE(sum((entry.value::text)::integer),0)::integer
    INTO rejected_total
    FROM pg_catalog.jsonb_each(p_rejected_counts) AS entry;
  IF rejected_total<>p_candidate_count-p_selected_count THEN
    RAISE EXCEPTION 'V5 shadow trace rejection counts do not reconcile'
      USING ERRCODE='23514';
  END IF;

  manifest_value := pg_catalog.jsonb_build_object(
    ...
    'visible_candidate_count',p_visible_candidate_count,
    ...
    'max_sensitivity',p_max_sensitivity,
    'embedding_provider_calls',p_embedding_provider_calls,
    'database_writes',0,
    'qdrant_writes',0,
  ...
  manifest_sha := pg_catalog.encode(
    public.digest(
      pg_catalog.convert_to(manifest_value::text,'UTF8'),
      'sha256'
    ),
    'hex'
  );
  ...
  PERFORM pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended(
    actor::text||'|v5_shadow_trace|'||p_request_id_sha256,0
  ));
  SELECT existing.* INTO stored
    FROM memory.v5_shadow_trace_event AS existing
  ...
  IF FOUND THEN
    IF stored.trace_manifest_sha256<>manifest_sha THEN
      RAISE EXCEPTION 'V5 shadow trace replay payload mismatch'
        USING ERRCODE='23514';
    END IF;
    RETURN QUERY SELECT
  ...
  INSERT INTO memory.v5_shadow_trace_event(
    ...
    candidate_set_sha256,selection_set_sha256,candidate_count,
    visible_candidate_count,selected_count,token_estimate,rejected_counts,
    candidate_limit,max_claims,max_tokens,max_sensitivity,
    embedding_provider_calls,trace_manifest_sha256,invoked_by_session
  ) VALUES (
    ...
    p_intent,p_domain,p_candidate_set_sha256,p_selection_set_sha256,
    p_candidate_count,p_visible_candidate_count,p_selected_count,
    p_token_estimate,p_rejected_counts,p_candidate_limit,p_max_claims,
    p_max_tokens,p_max_sensitivity,p_embedding_provider_calls,
    manifest_sha,session_user
  ) RETURNING v5_shadow_trace_event.trace_event_id INTO event_id;
...
ALTER FUNCTION memory.v5_shadow_rejection_counts_valid(jsonb)
  OWNER TO memory_v5_trace_writer;
ALTER FUNCTION memory.guard_v5_shadow_trace_append_only()
  OWNER TO memory_v5_trace_writer;
ALTER TABLE memory.v5_shadow_trace_event OWNER TO memory_v5_trace_writer;
ALTER FUNCTION memory.require_v5_shadow_trace_writer_context()
  OWNER TO memory_v5_trace_writer;
ALTER FUNCTION memory.record_v5_shadow_trace_v1(
  text,text,text,text,text,text,text,text,text,text,text,
  integer,integer,integer,integer,jsonb,integer,integer,integer,text,integer
) OWNER TO memory_v5_trace_writer;

ALTER TABLE memory.v5_shadow_trace_event ENABLE ROW LEVEL SECURITY;
ALTER TABLE memory.v5_shadow_trace_event FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS owner_isolation ON memory.v5_shadow_trace_event;
CREATE POLICY owner_isolation ON memory.v5_shadow_trace_event
...
DROP TRIGGER IF EXISTS v5_shadow_trace_event_append_only_guard
  ON memory.v5_shadow_trace_event;
CREATE TRIGGER v5_shadow_trace_event_append_only_guard
BEFORE UPDATE OR DELETE ON memory.v5_shadow_trace_event
FOR EACH ROW EXECUTE FUNCTION memory.guard_v5_shadow_trace_append_only();
...
GRANT SELECT,INSERT ON memory.v5_shadow_trace_event
  TO memory_v5_trace_writer;

REVOKE ALL ON memory.v5_shadow_trace_event FROM PUBLIC,brains_app;
REVOKE ALL ON FUNCTION memory.v5_shadow_trace_sha256_valid(text)
  FROM PUBLIC,brains_app;
REVOKE ALL ON FUNCTION memory.v5_shadow_rejection_counts_valid(jsonb)
  FROM PUBLIC,brains_app;
REVOKE ALL ON FUNCTION memory.guard_v5_shadow_trace_append_only()
  FROM PUBLIC,brains_app;
REVOKE ALL ON FUNCTION memory.require_v5_shadow_trace_writer_context()
  FROM PUBLIC,brains_app;
REVOKE ALL ON FUNCTION memory.record_v5_shadow_trace_v1(
  text,text,text,text,text,text,text,text,text,text,text,
  integer,integer,integer,integer,jsonb,integer,integer,integer,text,integer
) FROM PUBLIC,brains_app;
GRANT EXECUTE ON FUNCTION memory.record_v5_shadow_trace_v1(
  text,text,text,text,text,text,text,text,text,text,text,
  integer,integer,integer,integer,jsonb,integer,integer,integer,text,integer
) TO brains_app;
