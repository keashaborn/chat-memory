...
BEGIN
  IF current_user<>'sage' OR target IS NULL THEN
    RAISE EXCEPTION 'V5.2 compiler-v6 rollback prerequisites are absent';
  END IF;
  ...
  IF definition_sha<>old_definition_sha
    ...
    RAISE EXCEPTION 'V5.2 compiler-v6 rollback baseline changed: %',
  ...
  IF definition_sha<>new_definition_sha THEN
    RAISE EXCEPTION 'V5.2 compiler-v6 rollback changed: %',
      definition_sha USING ERRCODE='23514';
  END IF;
  EXECUTE definition;
END
