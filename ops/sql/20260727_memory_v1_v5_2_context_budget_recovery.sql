...
CREATE OR REPLACE FUNCTION memory.requeue_owner_v5_2_context_budget_failure_v1(
  p_operation_id uuid,
  p_job_id uuid,
...
BEGIN
  IF session_user<>'brains_app' THEN
    RAISE EXCEPTION 'context budget recovery requires brains_app session'
      USING ERRCODE='42501';
  END IF;
  ...
  IF p_operation_id IS NULL OR p_job_id IS NULL
     ...
     OR p_rejection_code<>'local_transport_http_rejected'
     OR p_reason_code<>'private_gpu_context_budget_recovery_v1' THEN
    RAISE EXCEPTION 'context budget recovery inputs are invalid'
      USING ERRCODE='22023';
  END IF;

  PERFORM pg_advisory_xact_lock(hashtextextended(
    concat_ws('|','memory_v1_v5_2_context_budget_recovery',actor::text,
      p_job_id::text),0
  ));
  ...
  IF FOUND THEN
    IF replayed.job_id<>p_job_id OR replayed.event_type<>'queued'
      ...
      RAISE EXCEPTION 'context budget recovery replay conflicts'
        USING ERRCODE='23514';
    END IF;
  ...
  IF NOT FOUND OR current_job.status<>'skipped'
    ...
    RAISE EXCEPTION 'context budget recovery job binding is invalid'
      USING ERRCODE='23514';
  END IF;
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'context budget recovery evidence or packets changed'
      USING ERRCODE='23514';
  END IF;
  ...
  IF NOT FOUND OR prior_failure.event_type<>'skipped'
    ...
    RAISE EXCEPTION 'context budget recovery failure binding is invalid'
      USING ERRCODE='23514';
  END IF;
  ...
  IF NOT FOUND OR completion.action<>'completed'
    ...
    RAISE EXCEPTION 'context budget recovery ledger binding is invalid'
      USING ERRCODE='23514';
  END IF;
  ...
  INSERT INTO memory.evidence_extraction_event(
    ...
    'admin','v5_2_context_budget_recovery',jsonb_build_object(
...
ALTER FUNCTION memory.requeue_owner_v5_2_context_budget_failure_v1(
...
COMMENT ON FUNCTION memory.requeue_owner_v5_2_context_budget_failure_v1(
  uuid,uuid,text,uuid,uuid,integer,text,text
) IS
  'Owner-scoped, replay-safe recovery for historical V5.2 llama.cpp context-budget failures misclassified as generic HTTP rejection.';
