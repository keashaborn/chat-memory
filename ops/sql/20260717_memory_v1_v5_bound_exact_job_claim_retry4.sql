...
BEGIN
  IF current_user<>'sage' THEN
    RAISE EXCEPTION 'exact-job retry4 migration requires sage';
  END IF;
  ...
  IF before_sha<>'e680f5972b124b96df1a2af20347c0182409a8ec385e50f59883dff32445ba3f'
     OR (length(function_definition)-length(replace(
       function_definition,
       'p_max_attempts NOT BETWEEN 1 AND 3',''
     )))<>length('p_max_attempts NOT BETWEEN 1 AND 3') THEN
    RAISE EXCEPTION 'exact-job claim function drifted before retry4 migration';
  END IF;
  ...
  IF after_sha<>'a694fc38a54422dea43f184f98a3576253b6a0083e47043d85d63c964813c312'
    ...
    RAISE EXCEPTION 'exact-job retry4 migration postcondition failed';
  END IF;
END
$migration$;
