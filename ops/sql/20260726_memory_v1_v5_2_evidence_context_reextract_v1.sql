...
BEGIN
  IF session_user<>'sage' THEN
    RAISE EXCEPTION 'local V5.2 evidence context reextract install requires sage';
  END IF;
  IF to_regrole('brains_app') IS NULL
    ...
    RAISE EXCEPTION 'local V5.2 evidence context reextract prerequisites are absent';
  END IF;
  IF NOT EXISTS (
...
BEGIN
  ...
  IF source_sha<>
     ...
     OR strpos(source,'memory_v1_semantic_policy_compiler_v8')=0 THEN
    RAISE EXCEPTION 'V5.2 persistence contract changed: %',source_sha
      USING ERRCODE='23514';
  END IF;
END
$persistence_contract$;

CREATE OR REPLACE FUNCTION memory.plan_owner_v5_2_evidence_context_reextract_v1(
...
DECLARE
  ...
  prior_compiler_sha constant text :=
    'b9c3a3e5b95ba16064e79b202928bb6357763af8b8bc74ff0efb5dba2c1d2d76';
BEGIN
  IF session_user<>'brains_app' THEN
    RAISE EXCEPTION 'local V5.2 evidence context reextract plan requires brains_app session'
      USING ERRCODE='42501';
  END IF;
  ...
  SELECT
    ...
    'evidence_context_coreference_available'::text,
    '20260726_v5_2_evidence_context_reextract_v1'::text,
  ...
  WHERE packet.owner_user_id=actor
    ...
    AND (
      SELECT count(*)
      FROM memory.evidence AS sibling
      WHERE sibling.owner_user_id=actor
        AND sibling.source_system='public.chat_log'
        AND sibling.status='active'
        AND sibling.metadata->>'source_id'=
            evidence.metadata->>'source_id'
        AND sibling.metadata->>'source_content_sha256'=
    ...
    AND NOT EXISTS (
      ...
      WHERE next_job.owner_user_id=actor
        ...
        AND next_job.selector_version=
          '20260726_v5_2_evidence_context_reextract_v1'
    );
END
$function$;

CREATE OR REPLACE FUNCTION memory.enqueue_owner_v5_2_evidence_context_reextract_v1(
  p_operation_id uuid,
  p_new_job_id uuid,
...
BEGIN
  IF session_user<>'brains_app' THEN
    RAISE EXCEPTION 'local V5.2 evidence context reextract apply requires brains_app session'
      USING ERRCODE='42501';
  END IF;
  actor := memory.current_actor_user_id();
  IF actor IS NULL THEN
  ...
  IF p_operation_id IS NULL OR p_new_job_id IS NULL
     ...
     OR p_selector_version<>'20260726_v5_2_evidence_context_reextract_v1'
     OR p_next_policy_compiler_version<>
       'memory_v1_semantic_policy_compiler_v8' THEN
    RAISE EXCEPTION 'local V5.2 evidence context reextract inputs are invalid'
      USING ERRCODE='22023';
  END IF;

  PERFORM pg_advisory_xact_lock(hashtextextended(concat_ws('|',
    'memory_v1_v5_2_evidence_context_reextract_v1',actor::text,
  ...
  IF FOUND THEN
    ...
    IF replay_job.job_id IS NULL OR replay_terminal.terminal_id IS NULL
       ...
       OR replay_event.actor_ref IS DISTINCT FROM
         'local_v5_2_evidence_context_reextract_v1'
      ...
      RAISE EXCEPTION 'local V5.2 evidence context reextract replay conflicts'
        USING ERRCODE='23514';
    END IF;
  ...
  SELECT * INTO planned
  FROM memory.plan_owner_v5_2_evidence_context_reextract_v1(p_prior_packet_id);
  IF NOT FOUND
     ...
     OR planned.reason_code<>'evidence_context_coreference_available'
    ...
    RAISE EXCEPTION 'local V5.2 evidence context reextract plan changed'
      USING ERRCODE='23514';
  END IF;
  IF EXISTS (
    ...
    RAISE EXCEPTION 'local V5.2 evidence context reextract target identifiers conflict'
      USING ERRCODE='23514';
  END IF;
  ...
  INSERT INTO memory.evidence_intake_terminal(
    ...
    fingerprint,actor,session_user,jsonb_build_object(
      ...
      'reextract_reason','evidence_context_coreference_available',
      'prior_packet_id',p_prior_packet_id,
      'prior_packet_storage_sha256',p_expected_packet_storage_sha256,
  ...
  INSERT INTO memory.evidence_extraction_event(
    ...
    'admin','local_v5_2_evidence_context_reextract_v1',jsonb_build_object(
      'prior_packet_id',p_prior_packet_id,
...
ALTER FUNCTION memory.plan_owner_v5_2_evidence_context_reextract_v1(uuid)
  OWNER TO memory_v5_local_reextract_maintainer;
ALTER FUNCTION memory.enqueue_owner_v5_2_evidence_context_reextract_v1(
  uuid,uuid,uuid,uuid,text,text,text,text
) OWNER TO memory_v5_local_reextract_maintainer;

REVOKE ALL ON FUNCTION memory.plan_owner_v5_2_evidence_context_reextract_v1(uuid)
  FROM PUBLIC,brains_app,memory_v5_local_reextract_maintainer;
REVOKE ALL ON FUNCTION memory.enqueue_owner_v5_2_evidence_context_reextract_v1(
  uuid,uuid,uuid,uuid,text,text,text,text
) FROM PUBLIC,brains_app,memory_v5_local_reextract_maintainer;
GRANT EXECUTE ON FUNCTION memory.plan_owner_v5_2_evidence_context_reextract_v1(uuid)
  TO brains_app,memory_v5_local_reextract_maintainer;
GRANT EXECUTE ON FUNCTION memory.enqueue_owner_v5_2_evidence_context_reextract_v1(
  uuid,uuid,uuid,uuid,text,text,text,text
) TO brains_app;
