...
BEGIN
  IF session_user<>'sage' THEN
    RAISE EXCEPTION 'V5.2 atom review replay compatibility requires sage';
  END IF;
  IF to_regrole('memory_v5_2_atom_admission_maintainer') IS NULL
     ...
     OR to_regclass('memory.v5_2_atom_admission_operation') IS NULL THEN
    RAISE EXCEPTION 'V5.2 atom review replay prerequisites are absent';
  END IF;
END
$preflight$;
...
BEGIN
  ...
  IF p_operation_id IS NULL OR p_review_id IS NULL
     OR NOT memory.v5_sha256_valid(p_authorization_manifest_sha256)
     OR p_proposal_id IS NULL OR p_decision IS NULL
  ...
  operation_manifest:=memory.v5_digest_text(concat_ws('|',
    'memory_v1_v5_2_atom_review_record_v2',actor::text,
    p_review_id::text,p_proposal_id::text,p_decision::text,
    p_reviewer_type,p_reviewer_ref,
...
COMMENT ON FUNCTION memory.review_owner_v5_2_atom_proposal_v1(
  uuid,uuid,uuid,memory.v5_2_atom_review_decision,text,text,jsonb,text
) IS 'Records one current authorized review. Exact replay is bound to the stable semantic request rather than the one-time preflight manifest; first write still requires the current manifest.';
