...
BEGIN
  ...
  LOOP
    IF EXISTS (
      ...
      EXECUTE format(
        'REVOKE ALL ON trusted_web.response_transcript_v1 FROM %I',
        role_name
      );
    END IF;
  END LOOP;
...
COMMENT ON POLICY response_transcript_owner_policy
  ON trusted_web.response_transcript_v1 IS
  'brains_app only; owner UUID is supplied by verified server session state.';
