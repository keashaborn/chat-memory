...
BEGIN
  IF current_user<>'sage'
    ...
    RAISE EXCEPTION 'V5.2 local persistence prerequisites are absent';
  END IF;
  IF NOT EXISTS (
    SELECT 1
    ...
    RAISE EXCEPTION 'V5.2 predicate registry is not installed';
  END IF;
END
$preflight$;

CREATE OR REPLACE FUNCTION memory.persist_owner_v5_2_local_packet_v1(
  p_operation_id uuid,
  p_packet_id uuid,
  p_job_id uuid,
  p_lease_token uuid,
...
BEGIN
  IF session_user<>'brains_app' THEN
    RAISE EXCEPTION 'V5.2 local packet persistence requires brains_app session'
      USING ERRCODE='42501';
  END IF;
  ...
  IF p_operation_id IS NULL OR p_packet_id IS NULL OR p_job_id IS NULL
    ...
    RAISE EXCEPTION 'V5.2 local packet persistence inputs are invalid'
      USING ERRCODE='22023';
  END IF;
  ...
  IF p_normalized_packet->>'contract_version'
       IS DISTINCT FROM 'memory_v1_relational_extraction_v5_2'
     OR p_normalized_packet->>'predicate_registry_version'
       IS DISTINCT FROM 'memory_predicate_registry_v5_2'
    ...
    RAISE EXCEPTION 'normalized local V5.2 packet contract is invalid'
      USING ERRCODE='23514';
  END IF;
  ...
  IF entity_count>24 OR observation_count>32
    ...
    RAISE EXCEPTION 'normalized local V5.2 packet exceeds bounded counts'
      USING ERRCODE='23514';
  END IF;
  IF (
    ...
    RAISE EXCEPTION 'normalized local V5.2 packet references are invalid'
      USING ERRCODE='23514';
  END IF;
  IF EXISTS (
    SELECT 1
    ...
    RAISE EXCEPTION 'local V5.2 project knowledge must remain unresolved'
      USING ERRCODE='23514';
  END IF;
  ...
  IF FOUND THEN
    IF replayed.packet_id<>p_packet_id OR replayed.job_id<>p_job_id
      ...
      RAISE EXCEPTION 'V5.2 local packet replay conflicts'
        USING ERRCODE='23514';
    END IF;
  ...
  IF NOT FOUND OR current_job.status<>'processing'
    ...
    RAISE EXCEPTION 'V5.2 local packet lease or content binding is invalid'
      USING ERRCODE='23514';
  END IF;
  ...
  IF NOT FOUND OR evidence_record.source_system<>'public.chat_log'
    ...
    RAISE EXCEPTION 'normalized local V5.2 packet source binding is invalid'
      USING ERRCODE='23514';
  END IF;

  final_summary := jsonb_build_object(
    'contract_version','memory_v1_evidence_extraction_packet_summary_v5_2_local',
  ...
  IF pg_column_size(next_result)>32768 THEN
    RAISE EXCEPTION 'sanitized local V5.2 job summary exceeds queue budget'
      USING ERRCODE='22023';
  END IF;
