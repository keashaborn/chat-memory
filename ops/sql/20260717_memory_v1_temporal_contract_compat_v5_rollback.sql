...
BEGIN
  IF EXISTS (
    SELECT 1
      FROM memory.observation_temporal
     WHERE
       (basis = 'none' AND semantic <> 'none')
       OR (basis = 'calendar' AND anchored_to_source_time)
       OR (basis = 'relative' AND shape <> 'instant')
  ) THEN
    RAISE EXCEPTION
      'rollback refused: V5.1 temporal rows require the compatibility matrix';
  END IF;
END
$guard$;
