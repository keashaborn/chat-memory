...
BEGIN
  IF EXISTS (
    ...
    WHERE source='backend/seebx:assistant:v2'
  ) THEN
    RAISE EXCEPTION 'canonical assistant rows exist; expanded historical reader must remain installed';
  END IF;
END
$migration$;
...
COMMENT ON VIEW lifeswitch_chat.read_prior_answer_lifeswitch_provenance_v1 IS
  'Owner-scoped prior LifeSwitch answer provenance; legacy source only before canonical writer activation.';
COMMIT;
