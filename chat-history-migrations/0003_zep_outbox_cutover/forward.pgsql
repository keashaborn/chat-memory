...
DO $preflight$
BEGIN
  IF to_regclass('conversation_sync_private.zep_turn_outbox') IS NULL THEN
    RAISE EXCEPTION 'canonical Zep turn outbox is required';
  END IF;
  IF to_regprocedure(
    ...
    RAISE EXCEPTION 'chat history clear functions are required';
  END IF;
END;
$preflight$;
...
BEGIN
  ...
  LOCK TABLE conversation_sync_private.zep_turn_outbox
    IN SHARE ROW EXCLUSIVE MODE;
  ...
  IF EXISTS (
    SELECT 1
    FROM conversation_sync_private.zep_turn_outbox AS outbox
    JOIN pg_temp.chat_history_target_message AS target
      ON target.message_id IN (
        outbox.user_message_id, outbox.assistant_message_id
      )
  ...
  DELETE FROM conversation_sync_private.zep_turn_outbox AS outbox
...
BEGIN
  IF pg_get_functiondef(
     ...
     ) ILIKE '%memory_ingest_private%' THEN
    RAISE EXCEPTION 'legacy memory ingest dependency remains';
  END IF;
  IF pg_get_functiondef(
       to_regprocedure(
         'chat_history_private.clear_history(uuid,text,uuid,integer)'
       )
     ) NOT ILIKE '%conversation_sync_private.zep_turn_outbox%'
     OR pg_get_functiondef(
       to_regprocedure('chat_history_private.clear_message_tail(uuid,uuid)')
     ) NOT ILIKE '%conversation_sync_private.zep_turn_outbox%' THEN
    RAISE EXCEPTION 'canonical Zep outbox dependency is missing';
  END IF;
  IF EXISTS (
    ...
    RAISE EXCEPTION 'legacy source-erasure trigger remains';
  END IF;
END;
$postflight$;
