...
WORKER_VERSION = "memory_v1_v5_local_packet_reextract_v2"
APPLY_ENABLE_TOKEN = "memory_v1_v5_local_packet_reextract_apply_v2"
SELECTOR_VERSION = "20260719_v5_temporal_contract_reextract_v1"
NEXT_COMPILER_VERSION = "memory_v1_local_policy_compiler_v6"
REASON_CODE = "temporal_persistence_matrix_mismatch"
IDENTITY_NAMESPACE = uuid.UUID("ea62f0ba-599c-5cda-a242-39161534ac7e")
...
def loopback_dsn(value: str) -> str:
    ...
    if parsed.scheme not in {"postgres", "postgresql"}:
        raise RuntimeError("packet reextract database DSN scheme is invalid")
    if parsed.hostname not in {"127.0.0.1", "::1", "localhost"}:
        raise RuntimeError("packet reextract database DSN must be loopback-only")
    return value
...
def validated_arguments(args: argparse.Namespace) -> tuple[uuid.UUID, uuid.UUID]:
    ...
    if args.apply and os.getenv("MEMORY_V1_V5_LOCAL_PACKET_REEXTRACT_APPLY") != (
        APPLY_ENABLE_TOKEN
    ):
        raise RuntimeError("packet reextract apply capability is absent")
    return owner, packet
...
async def read_plan(
    ...
    async with conn.transaction(readonly=True):
        ...
        row = await conn.fetchrow(
            "SELECT * FROM memory.plan_owner_v5_local_packet_reextract_v2($1)",
            packet,
        )
...
async def invoke(
    ...
    *,
    ...
    async with conn.transaction():
        ...
        row = await conn.fetchrow(
            """
            SELECT * FROM memory.enqueue_owner_v5_local_packet_reextract_v2(
              $1,$2,$3,$4,$5,$6,$7,$8
            )
            """,
            operation_id,
    ...
    if row is None:
        raise RuntimeError("packet reextract apply returned no row")
    return dict(row)
...
async def run() -> int:
    ...
    try:
        if await conn.fetchval("SELECT session_user") != "brains_app":
            raise RuntimeError("packet reextract requires brains_app session")
        plan = await read_plan(conn, owner, packet)
        if plan is None:
            raise RuntimeError("exact packet reextract plan is absent")
        if (
            ...
            raise RuntimeError("exact packet reextract plan changed")
        ...
        if args.expected_plan_sha256 is not None and plan_sha256 != (
            ...
            raise RuntimeError("packet reextract plan hash changed")
        report = {
        ...
        if applied["apply_outcome"] not in {"applied", "replayed"}:
            raise RuntimeError("packet reextract apply outcome is invalid")
        if replayed["apply_outcome"] != "replayed":
            raise RuntimeError("packet reextract replay wrote again")
        report.update(
            {
                "outcome": "queued",
