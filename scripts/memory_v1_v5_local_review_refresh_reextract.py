...
WORKER_VERSION = "memory_v1_v5_local_review_refresh_reextract_v1"
APPLY_ENABLE_TOKEN = "memory_v1_v5_local_review_refresh_reextract_apply_v1"
SELECTOR_VERSION = "20260719_v5_self_bootstrap_review_refresh_v1"
NEXT_COMPILER_VERSION = "memory_v1_local_policy_compiler_v6"
REASON_CODE = "trusted_owner_self_became_available"
IDENTITY_NAMESPACE = uuid.UUID("481d4ad1-c4d2-5ca6-91e4-7773a8d7eff2")
...
def loopback_dsn(value: str) -> str:
    ...
    if parsed.scheme not in {"postgres", "postgresql"}:
        raise RuntimeError("review refresh database DSN scheme is invalid")
    if parsed.hostname not in {"127.0.0.1", "::1", "localhost"}:
        raise RuntimeError("review refresh database DSN must be loopback-only")
    return value
...
def arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plan or append one owner-scoped review-state refresh job. "
    ...
    parser.add_argument("--expected-review-report-sha256", required=True)
    parser.add_argument("--expected-stage-bundle-sha256", required=True)
...
def validated_arguments(args: argparse.Namespace) -> tuple[uuid.UUID, uuid.UUID]:
    ...
    for value in (
        ...
        args.expected_review_report_sha256,
        args.expected_stage_bundle_sha256,
    ):
        if not SHA256_RE.fullmatch(value):
            raise RuntimeError("an expected review refresh hash is invalid")
    ...
    if args.apply and os.getenv(
        "MEMORY_V1_V5_LOCAL_REVIEW_REFRESH_REEXTRACT_APPLY"
    ) != APPLY_ENABLE_TOKEN:
        raise RuntimeError("review refresh apply capability is absent")
    return owner, packet
...
def operation_ids(
    ...
    identity = (
        ...
        f"{report_sha256}|{bundle_sha256}|{SELECTOR_VERSION}|"
        f"{NEXT_COMPILER_VERSION}"
    )
    return (
...
def plan_contract(row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stage_bundle_sha256": row["stage_bundle_sha256"],
        "active_self_entity_count": row["active_self_entity_count"],
...
async def read_plan(
    ...
    async with conn.transaction(readonly=True):
        ...
        row = await conn.fetchrow(
            "SELECT * FROM "
            "memory.plan_owner_v5_local_review_refresh_reextract_v1($1,$2,$3)",
            packet,
            report_sha256,
            bundle_sha256,
        )
...
async def invoke(
    ...
    *,
    ...
    if row is None:
        raise RuntimeError("review refresh apply returned no row")
    return dict(row)
...
async def run() -> int:
    ...
    try:
        if await conn.fetchval("SELECT session_user") != "brains_app":
            raise RuntimeError("review refresh requires brains_app session")
        ...
        if plan is None:
            raise RuntimeError("exact review refresh plan is absent")
        if (
            ...
            != args.expected_packet_storage_sha256
            or plan["review_report_sha256"]
            != args.expected_review_report_sha256
            or plan["stage_bundle_sha256"]
            != args.expected_stage_bundle_sha256
            or plan["active_self_entity_count"] != 1
            ...
            raise RuntimeError("exact review refresh plan changed")
        ...
        if args.expected_plan_sha256 is not None and plan_sha256 != (
            ...
            raise RuntimeError("review refresh plan hash changed")
        report = {
            "contract_version": WORKER_VERSION,
            "apply": args.apply,
            "owner_user_id_sha256": sha256_text(str(owner)),
            "prior_packet_id_sha256": sha256_text(str(packet)),
            "artifact_id_sha256": sha256_text(str(plan["artifact_id"])),
            "plan_sha256": plan_sha256,
            "reason_code": REASON_CODE,
            "active_self_entity_count": plan["active_self_entity_count"],
        ...
        parameters = {
            ...
            "report_sha256": args.expected_review_report_sha256,
            "bundle_sha256": args.expected_stage_bundle_sha256,
        }
        applied = await invoke(conn, **parameters)
        replayed = await invoke(conn, **parameters)
        if applied["apply_outcome"] not in {"applied", "replayed"}:
            raise RuntimeError("review refresh apply outcome is invalid")
        if replayed["apply_outcome"] != "replayed":
            raise RuntimeError("review refresh replay wrote again")
        report.update(
            {
                "outcome": "queued",
