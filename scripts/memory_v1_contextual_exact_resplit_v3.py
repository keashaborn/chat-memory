...
CONTRACT_VERSION = "memory_v1_contextual_exact_resplit_v3"
SELECTOR_VERSION = "20260729_v4_contextual_resplit"
APPLY_CAPABILITY = "memory_v1_contextual_exact_resplit_apply_v3"
...
async def preflight_split(
    ...
    planned = await conn.fetchrow(
        """
        SELECT *
          FROM memory.preflight_owner_contextual_split_v3($1,$2,$3,$4::jsonb)
        """,
        evidence_id,
        row["evidence_content_sha256"],
        SPLITTER_VERSION,
...
async def apply_contextual_split(
    ...
    first = await conn.fetch(
        """
        SELECT *
          FROM memory.apply_owner_contextual_split_v3(
            $1,$2,$3,$4::jsonb,$5
          )
        """,
        *values,
    ...
    first_terminal = await conn.fetchrow(
        """
        SELECT *
          FROM memory.finalize_owner_contextual_split_v3($1,$2,$3,$4,$5)
        """,
        *terminal_values,
    )
...
async def dispatch_owner(
    ...
    actual_plan_sha256 = digest(before)
    if (
        expected_plan_sha256 is not None
        and expected_plan_sha256 != actual_plan_sha256
    ):
        raise RuntimeError("exact V3 resplit plan changed")
...
async def main() -> int:
    ...
    if not args.evidence_id:
        raise RuntimeError(
            "exact V3 resplit requires --evidence-id targets"
        )
    ...
    if args.expected_plan_sha256 is not None and (
        ...
        or any(
            ...
            for char in args.expected_plan_sha256
        )
    ):
        raise RuntimeError("expected plan SHA-256 is invalid")
    if args.apply and args.expected_plan_sha256 is None:
        raise RuntimeError("apply requires an expected plan SHA-256")
    if (
        args.apply
        and os.getenv("MEMORY_V1_CONTEXTUAL_EXACT_RESPLIT_APPLY")
        != APPLY_CAPABILITY
    ):
        raise RuntimeError("contextual resplit apply capability is absent")
