#!/usr/bin/env python3
"""Apply five exact V5.2 entity resolutions and bind four observations."""
...
MANIFEST_CONTRACT = "memory_v1_v5_2_compiler_v8_entity_apply_manifest_v1"
PLAN_CONTRACT = "memory_v1_v5_2_compiler_v8_entity_apply_plan_v1"
AUTHORIZATION_CONTRACT = "memory_v1_v5_2_compiler_v8_entity_apply_authorization_v1"
REPORT_CONTRACT = "memory_v1_v5_2_compiler_v8_entity_apply_report_v1"
...
REQUEST_NAMESPACE = uuid.UUID("d321224d-4690-553e-b8db-b6875f639042")
EXPECTED: dict[str, list[dict[str, Any]]] = {
    "dcf5ece1-0e22-574f-8ac9-f3d0acc4e8f5": [
        {
            "entity_ref": "e00",
            "resolution_id": "37e5a24b-2862-46df-8ce6-7fc6cfbc61bb",
            "review_id": None,
            "entity_type": "self",
        ...
        {
            "entity_ref": "e01",
            "resolution_id": "35fe3e20-e02d-4304-ba4b-2e85066034be",
            "review_id": "1b0c5e22-fbf3-4511-89d9-6607117bd63b",
            "entity_type": "concept",
            "mention_kind": "named",
        ...
        {
            "entity_ref": "e02",
            "resolution_id": "065f0bde-dd18-4630-8840-4ba49655bc55",
            "review_id": "4a97cb73-da53-407c-8f19-53badc5f689b",
            "entity_type": "concept",
            "mention_kind": "named",
            "name_text": "BCBA",
            "action": "create_new",
    ...
    "fea59e7e-30f5-4139-b634-97b291c88e14": [
        {
            "entity_ref": "e00",
            "resolution_id": "f7f8b81b-4fa5-4f36-a0e4-4fb2cfd571ae",
            "review_id": None,
            "entity_type": "self",
            "mention_kind": "self_reference",
            "name_text": None,
        ...
        {
            "entity_ref": "e01",
            "resolution_id": "8ed545c8-da8b-4db8-92fa-519ee62a8185",
            "review_id": "200e0507-1f1e-4068-8f32-113b8d1c62fb",
            "entity_type": "person",
            "mention_kind": "named",
            "name_text": "Monika",
            "action": "create_new",
...
async def apply_preflight(
    ...
    if (
        ...
        or (
            ...
            and result["prospective_entity_id"] != SELF_ENTITY
        )
        or (
...
async def build_manifest(args: argparse.Namespace) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "expected_new_entities": 3,
        "expected_total_bindings": 4,
        "expected_new_rows": 20,
        "items": items,
    }
    path, digest = secure_write(args.output, manifest)
    return {
        "contract_version": "memory_v1_v5_2_compiler_v8_entity_apply_manifest_report_v1",
...
def load_manifest(path_value: str, root: Path, head: str) -> tuple[dict[str, Any], str]:
    ...
    if (
        ...
        or manifest["expected_item_count"] != 5
        or manifest["expected_new_entities"] != 3
        or manifest["expected_total_bindings"] != 4
        or manifest["expected_new_rows"] != 20
        or not isinstance(manifest["items"], list)
        or len(manifest["items"]) != 5
    ):
    ...
    if sum(item["expected_bindings"] for item in manifest["items"]) != 4:
        raise EntityApplyError("entity apply binding budget is invalid")
...
def load_plan(path_value: str, root: Path, head: str) -> tuple[dict[str, Any], str]:
    ...
    if (
        ...
        or plan["item_count"] != 5
        or plan["expected_new_entities"] != 3
        or plan["expected_total_bindings"] != 4
        or plan["expected_new_rows"] != 20
...
def load_authorization(
    ...
    if (
        ...
        or authorization["scope"] != "apply_compiler_v8_entities_and_bind_only"
        or authorization["owner_user_id"] != OWNER
        or authorization["plan_sha256"] != plan_sha
        or authorization["expected_item_count"] != 5
        or authorization["expected_new_entities"] != 3
        or authorization["expected_total_bindings"] != 4
        or authorization["expected_new_rows"] != 20
...
async def apply_plan(args: argparse.Namespace) -> dict[str, Any]:
    if os.getenv("MEMORY_V1_V5_2_COMPILER_V8_ENTITY_APPLY") != "authorized":
        raise EntityApplyError("entity apply environment gate is closed")
    ...
    try:
        async with conn.transaction(isolation="serializable"):
            ...
            await conn.execute(
                ...
                f"{OWNER}|compiler_v8_entity_apply",
            )
            for entry in plan["items"]:
                ...
                if (
                    ...
                    or (
                        ...
                        and result["applied_entity_id"] != SELF_ENTITY
                    )
                ):
        ...
        await conn.close()
    if sum(item["bindings_created"] for item in first) != 4:
        raise EntityApplyError("entity apply binding total is not four")
    report = {
        ...
        "item_count": 5,
        "new_entities": 3,
        "new_entity_resolution_applies": 5,
        "new_alias_observations": 3,
        "new_observation_bindings": 4,
        "new_operation_requests": 5,
        "new_rows": 20,
        "first_results": first,
