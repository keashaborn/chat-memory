...
CONTRACT = "memory_v1_v5_2_compiler_v8_claim_review_manifest_v1"
DECISION_CONTRACT = "memory_v1_v5_2_compiler_v8_claim_review_decisions_v1"
STAGE_CONTRACT = "memory_v1_v5_2_compiler_v8_claim_stage_manifest_v1"
...
def validate_decisions(
    value: dict[str, Any], owner: str, evidence_ids: list[str]
    ...
    if (
        ...
        or sorted(value["evidence_ids"]) != sorted(evidence_ids)
    ):
        raise ManifestError("decision owner or evidence set mismatch")
...
async def run() -> int:
    ...
    evidence_ids = [str(uuid.UUID(value)) for value in stage["evidence_ids"]]
    decisions_value = load_hashed_json(
        decisions_path, DECISION_CONTRACT, "decisions_sha256"
    )
    decisions = validate_decisions(decisions_value, owner, evidence_ids)
