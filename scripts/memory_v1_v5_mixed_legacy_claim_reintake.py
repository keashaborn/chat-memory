...
def arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plan or enqueue legacy relational facts from evidence that also "
            "backs an active, already-separated preference."
        )
    )
...
def secure_write(path: Path, value: dict[str, Any]) -> str:
    ...
    if path.exists():
        raise RuntimeError("mixed reintake report already exists")
...
async def plan(
    ...
    rows = await conn.fetch(
        ...
        SELECT evidence_id,evidence_content_sha256,outcome,route,reason_code,
               legacy_claim_count,active_preference_count
        FROM memory.plan_owner_v5_mixed_legacy_claim_reintake_v1($1,$2,NULL)
        """,
        SELECTOR_VERSION,
        limit,
    )
...
async def run_owner(
    ...
    if any(
        ...
        or int(row["legacy_claim_count"]) < 1
        or int(row["active_preference_count"]) < 1
        for row in before
    ):
        raise RuntimeError("mixed legacy reintake plan contract changed")
    applied = 0
    replayed = 0
    if apply:
        for row in before:
            first = await conn.fetchrow(
                """
                SELECT * FROM memory.enqueue_owner_v5_mixed_legacy_claim_reintake_v1(
                  $1,$2,$3,$4,$5
                )
                """,
                row["evidence_id"],
            ...
            if first["apply_outcome"] != "applied" or second["apply_outcome"] != "replayed":
                raise RuntimeError("mixed legacy reintake enqueue replay failed")
            applied += 1
            replayed += 1
    after = await plan(conn, owner, limit)
    if apply and after:
        raise RuntimeError("mixed legacy reintake plan was not exhausted after apply")
    safe_before = [
        {
            "evidence_id_sha256": sha256_text(str(row["evidence_id"])),
            "evidence_content_sha256": row["evidence_content_sha256"],
            "route": row["route"],
            "legacy_claim_count": int(row["legacy_claim_count"]),
            "active_preference_count": int(row["active_preference_count"]),
        }
        for row in before
    ]
    return {
        ...
        "legacy_claim_count": sum(
            int(row["legacy_claim_count"]) for row in before
        ),
        "active_preference_count": sum(
            int(row["active_preference_count"]) for row in before
        ),
        "applied": applied,
        "replayed": replayed,
...
async def run() -> int:
    ...
    if args.apply and os.getenv("MEMORY_V1_V5_MIXED_REINTAKE_APPLY") != APPLY_TOKEN:
        raise RuntimeError("mixed legacy V5 reintake apply capability is absent")
    ...
    try:
        if await conn.fetchval("SELECT session_user") != "brains_app":
            raise RuntimeError("mixed legacy V5 reintake requires brains_app session")
    ...
    report = {
        ...
        "totals": {
            ...
            "legacy_claims": sum(item["legacy_claim_count"] for item in reports),
            "active_preferences": sum(
                item["active_preference_count"] for item in reports
            ),
