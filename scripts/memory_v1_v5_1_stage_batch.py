#!/usr/bin/env python3
"""Generic owner-scoped runner for reviewed Memory V1 V5.1 stage bundles."""
...
MANIFEST_CONTRACT = "memory_v1_v5_1_stage_batch_manifest_v1"
PLAN_CONTRACT = "memory_v1_v5_1_stage_batch_plan_v1"
AUTHORIZATION_CONTRACT = "memory_v1_v5_1_stage_batch_authorization_v1"
REPORT_CONTRACT = "memory_v1_v5_1_stage_batch_apply_report_v1"
BUNDLE_CONTRACT = "memory_v1_v5_1_stage_preflight_v1"
CONFIRMATION = "STAGE_REVIEWED_OWNER_V5_1_PACKETS_ONLY"
...
def arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plan or apply one reviewed owner-scoped V5.1 staging batch."
    )
...
def expected_schema_hashes() -> dict[str, str]:
    ...
    return {
        "extraction_sha256": _schema_hash(
            root / "specs/memory_v1_relational_extraction_v5_1.schema.json"
        ),
        "resolution_sha256": _schema_hash(
            root / "specs/memory_v1_entity_resolution_review_v5_1.schema.json"
        ),
    }
...
def load_bundle(
    ...
    if (
        ...
        raise StageBatchError(f"bundle is not a zero-write V5.1 preflight: {path}")
...
def load_authorization(
    ...
    if (
        ...
        or value["target_server"] != "seebx"
        or value["scope"] != "stage_reviewed_owner_v5_1_packets_only"
...
async def call_stage(
    ...
    row = await conn.fetchrow(
        ...
        FROM memory.stage_relational_packet_v5_1(
...
async def apply_plan(args: argparse.Namespace) -> dict[str, Any]:
    if os.getenv("MEMORY_V1_V5_1_STAGE_BATCH_APPLY") != "authorized":
        raise StageBatchError(
            "apply requires MEMORY_V1_V5_1_STAGE_BATCH_APPLY=authorized"
        )
