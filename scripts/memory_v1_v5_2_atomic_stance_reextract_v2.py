...
WORKER_VERSION = "memory_v1_v5_2_atomic_stance_reextract_v2"
APPLY_ENABLE_TOKEN = "memory_v1_v5_2_atomic_stance_reextract_apply_v2"
SELECTOR_VERSION = "20260722_v5_2_atomic_stance_reextract_v2"
NEXT_COMPILER_VERSION = "memory_v1_semantic_policy_compiler_v5"
REASON_CODE = "semantic_stance_atomic_split_required"
IDENTITY_NAMESPACE = uuid.UUID("635b0bce-4aed-42f4-9c1a-260ef9edb46d")
...
def loopback_dsn(value: str) -> str:
    ...
    if parsed.scheme not in {"postgres", "postgresql"}:
        raise RuntimeError("V5.2 atomic stance reextract database DSN scheme is invalid")
    if parsed.hostname not in {"127.0.0.1", "::1", "localhost"}:
        raise RuntimeError("V5.2 atomic stance reextract database DSN must be loopback-only")
    return value
...
def arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plan or append one owner-scoped extraction job after a verified "
            "local packet contract failure. Reports contain hashes and "
            "counts only."
        )
    )
...
def validated_arguments(args: argparse.Namespace) -> tuple[uuid.UUID, uuid.UUID]:
    ...
    if args.apply and os.getenv("MEMORY_V1_V5_2_ATOMIC_STANCE_REEXTRACT_APPLY") != (
        APPLY_ENABLE_TOKEN
    ):
        raise RuntimeError("V5.2 atomic stance reextract apply capability is absent")
    return owner, packet
...
async def read_plan(
    ...
    async with conn.transaction(readonly=True):
        ...
        row = await conn.fetchrow(
            "SELECT * FROM memory.plan_owner_v5_2_atomic_stance_reextract_v2($1)",
            packet,
        )
...
async def invoke(
    ...
    *,
    ...
    async with conn.transaction():
        ...
        row = await conn.fetchrow(
            """
            SELECT * FROM memory.enqueue_owner_v5_2_atomic_stance_reextract_v2(
              $1,$2,$3,$4,$5,$6,$7,$8
            )
            """,
            operation_id,
    ...
    if row is None:
        raise RuntimeError("V5.2 atomic stance reextract apply returned no row")
    return dict(row)
...
async def run() -> int:
    ...
    try:
        if await conn.fetchval("SELECT session_user") != "brains_app":
            raise RuntimeError("V5.2 atomic stance reextract requires brains_app session")
        plan = await read_plan(conn, owner, packet)
        if plan is None:
            raise RuntimeError("exact V5.2 atomic stance reextract plan is absent")
        if (
            ...
            raise RuntimeError("exact V5.2 atomic stance reextract plan changed")
        ...
        if args.expected_plan_sha256 is not None and plan_sha256 != (
            ...
            raise RuntimeError("V5.2 atomic stance reextract plan hash changed")
        report = {
        ...
        if applied["apply_outcome"] not in {"applied", "replayed"}:
            raise RuntimeError("V5.2 atomic stance reextract apply outcome is invalid")
        if replayed["apply_outcome"] != "replayed":
            raise RuntimeError("V5.2 atomic stance reextract replay wrote again")
        report.update(
            {
                "outcome": "queued",
