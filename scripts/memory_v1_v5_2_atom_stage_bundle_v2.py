#!/usr/bin/env python3
"""Build and probe one reviewed V5.2 V2 atom-stage bundle without writes."""
...
BUILD_MANIFEST_CONTRACT = "memory_v1_v5_2_atom_stage_build_manifest_v2"
PLAN_CONTRACT = "memory_v1_v5_2_atom_stage_plan_v2"
SOURCE_REPORT_CONTRACT = "memory_v1_v5_2_atom_stage_source_report_v2"
BUILD_MANIFEST_KEYS = {
    "contract_version",
    "target_server",
    "owner_user_id",
...
def load_build_manifest(path_value: str) -> tuple[Path, dict[str, Any]]:
    ...
    resolutions = value["expected_resolutions"]
    if (
        ...
        or any(
            ...
            for entity_ref, expected in resolutions.items()
        )
    ):
        raise AtomStageError("build manifest expected resolutions are invalid")
    return path, value
...
def validate_plan(plan: dict[str, Any], manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        or plan["policy_version"] != "memory_v1_v5_2_atom_stage_policy_v2"
    ):
...
async def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        async with conn.transaction(isolation="repeatable_read", readonly=True):
            ...
            for mention in projection["entity_mentions"]:
                ...
                expected = manifest["expected_resolutions"].get(
                    mention["entity_ref"]
                )
                actual = {
                    key: resolution[key]
                ...
                if expected is None or actual != expected:
                    raise AtomStageError(
                        f"resolution differs from the reviewed manifest: "
                        f"{mention['entity_ref']}"
                    )
            ...
            if {item["entity_ref"] for item in resolutions} != set(
                manifest["expected_resolutions"]
            ):
                raise AtomStageError("reviewed resolution reference set differs")
    finally:
        await conn.close()
    ...
    resolution_summary = {
        state: sum(
            1 for item in resolutions if item["decision_state"] == state
        )
        for state in (
            "auto_link_eligible",
    ...
    bundle = {
        ...
        "extractor": "memory_v1_v5_2_atom_stage_projection_v2",
    ...
    return {
        "contract_version": "memory_v1_v5_2_atom_stage_build_report_v2",
        "owner_user_id": str(owner),
...
async def probe(args: argparse.Namespace) -> dict[str, Any]:
    ...
    tampered["observations"][0]["reason_codes"].append("tampered_probe")
    dsn = os.environ.get("POSTGRES_DSN", "").strip()
    if not dsn:
        raise AtomStageError("POSTGRES_DSN is required")
    conn = await asyncpg.connect(dsn, command_timeout=30)
    ...
    return {
        "contract_version": "memory_v1_v5_2_atom_stage_guard_probe_v2",
