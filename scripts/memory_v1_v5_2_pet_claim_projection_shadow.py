...
from memory_v1_v5_2_pet_claim_projection_clone import (
    claim_snapshot,
    load_plan,
    set_actor,
)
...
REPORT_CONTRACT = "memory_v1_v5_2_pet_claim_projection_shadow_result_v1"
REVIEW_ROOT = Path("/home/ubuntu/memory-v1-reviews")
COLLECTION_PREFIX = "memory_claim_v1_shadow_pet_"
QUERY_BY_PREDICATE = {
    "identity.name": "What were the names of some of my pets?",
    "pet.breed": "What breeds were my dogs?",
    "pet.species": "Were my pets cats or dogs?",
    "relationship.has_pet": "Which pets have I had?",
}
...
async def shadow_tests(
    ...
    for item in items:
        ...
        trace = await build_v5_shadow_trace(
            ...
            context={
                "eligible": True,
                "reason": "isolated_pet_projection_shadow",
...
async def run() -> int:
    ...
    gate = (
        "MEMORY_V1_V5_2_PET_CLAIM_SHADOW_PROJECTION"
        if args.mode == "apply"
        else "MEMORY_V1_V5_2_PET_CLAIM_SHADOW_REPLAY"
    )
    ...
    try:
        ...
        report = {
            ...
            "qdrant_writes": 11 if args.mode == "apply" else 0,
