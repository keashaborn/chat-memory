#!/usr/bin/env python3
"""Controlled owner-scoped review/apply batch for Memory V1 V5 entities."""
...
MANIFEST_CONTRACT = "memory_v1_v5_entity_resolution_batch_manifest_v1"
PLAN_CONTRACT = "memory_v1_v5_entity_resolution_batch_plan_v1"
AUTHORIZATION_CONTRACT = "memory_v1_v5_entity_resolution_batch_authorization_v1"
REPORT_CONTRACT = "memory_v1_v5_entity_resolution_batch_apply_report_v1"
CONFIRMATION = "REVIEW_AND_APPLY_OWNER_V5_ENTITY_RESOLUTIONS_ONLY"
...
def arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plan or apply one reviewed owner-scoped V5 entity batch."
    )
...
async def _apply_preflight(
    ...
    row = await conn.fetchrow(
        ...
        SELECT resolution_id,action::text,decision_state::text,review_id,
          ...
          FROM memory.preflight_entity_resolution_apply_v5($1::uuid,$2::uuid)
        """,
        uuid.UUID(item["resolution_id"]),
        review_id,
    )
    ...
    try:
        async with conn.transaction(readonly=True, isolation="repeatable_read"):
            ...
            for item in metadata["items"]:
                preflight = (
                    await _apply_preflight(conn, item, None)
                    if item["operation"] == "auto_apply"
                    else await _review_preflight(conn, item)
                )
...
def load_authorization(
    ...
    if (
        ...
        or value["scope"] != "review_and_apply_owner_v5_entity_resolutions_only"
...
async def _review(
    ...
    row = await conn.fetchrow(
        ...
        SELECT review_id,outcome,result
          FROM memory.review_entity_resolution_v5(
...
async def _apply(
    ...
    row = await conn.fetchrow(
        ...
        SELECT applied_entity_id,outcome,bindings_created,result
          FROM memory.apply_entity_resolution_v5($1::uuid,$2::uuid,$3::uuid,$4)
        """,
        request_id,
        uuid.UUID(item["resolution_id"]),
        review_id,
...
async def apply_plan(args: argparse.Namespace) -> dict[str, Any]:
    if os.getenv("MEMORY_V1_V5_ENTITY_RESOLUTION_BATCH_APPLY") != "authorized":
    ...
    try:
        async with conn.transaction():
            ...
            await conn.execute(
                ...
                f"{owner}|entity_resolution_batch_v5",
            )
            for entry in plan["items"]:
                ...
                if item["operation"] != "auto_apply":
                    ...
                    review_request = _request_id(
                        plan_sha, item["resolution_id"], "review"
                    )
                    ...
                    apply_preflight = await _apply_preflight(conn, item, review_id)
                else:
                    apply_preflight = await _apply_preflight(conn, item, None)
                    if apply_preflight != entry["preflight"]:
                        raise EntityResolutionBatchError("apply preflight drifted")
                apply_request = _request_id(plan_sha, item["resolution_id"], "apply")
                ...
                applied.append(
                    {
                        ...
                        "operation": item["operation"],
                        "review_request_id": (
                            str(_request_id(plan_sha, item["resolution_id"], "review"))
                            if review_result
                            else None
                        ),
            ...
            for entry, prior in zip(plan["items"], applied, strict=True):
                ...
                if item["operation"] != "auto_apply":
                    replay_review = await _review(
                        ...
                        entry["preflight"]["authorization_manifest_sha256"],
                    )
                    if (
