#!/usr/bin/env python3
"""Controlled owner-scoped reconciliation/review/apply for V5.1 entities."""
...
MANIFEST_CONTRACT = "memory_v1_v5_1_entity_resolution_batch_manifest_v1"
PLAN_CONTRACT = "memory_v1_v5_1_entity_resolution_batch_plan_v1"
AUTHORIZATION_CONTRACT = "memory_v1_v5_1_entity_resolution_batch_authorization_v1"
REPORT_CONTRACT = "memory_v1_v5_1_entity_resolution_batch_apply_report_v1"
CONFIRMATION = "RECONCILE_REVIEW_AND_APPLY_OWNER_V5_1_ENTITY_RESOLUTIONS_ONLY"
DEFAULT_REVIEW_ROOT = "/home/ubuntu/memory-v1-reviews"
REQUEST_NAMESPACE = uuid.UUID("7eddf4d2-46e6-5ea0-8132-bc678e05473d")
MAX_ITEMS = 32
MAX_NEW_ROWS = 500
...
def arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plan or apply one reviewed owner-scoped V5.1 entity batch."
    )
...
async def _apply_preflight(
    ...
    row = await conn.fetchrow(
        ...
        SELECT resolution_id,action::text,decision_state::text,review_id,
          ...
          FROM memory.preflight_entity_resolution_apply_v5_1($1::uuid,$2::uuid)
        """,
        uuid.UUID(item["resolution_id"]),
        review_id,
    )
...
def load_authorization(
    ...
    if (
        ...
        or value["scope"]
        != "reconcile_review_and_apply_owner_v5_1_entity_resolutions_only"
...
async def _apply(
    ...
    row = await conn.fetchrow(
        ...
        SELECT applied_entity_id,outcome,bindings_created,result
          FROM memory.apply_entity_resolution_v5_1($1::uuid,$2::uuid,$3::uuid,$4)
        """,
        request_id,
        uuid.UUID(item["resolution_id"]),
        review_id,
...
async def _reconcile(
    ...
    row = await conn.fetchrow(
        ...
        SELECT reconciliation_id,successor_resolution_id,target_entity_id,
               outcome,result
          FROM memory.reconcile_entity_resolution_v5_1(
...
async def apply_plan(args: argparse.Namespace) -> dict[str, Any]:
    if os.getenv("MEMORY_V1_V5_1_ENTITY_RESOLUTION_BATCH_APPLY") != "authorized":
    ...
    try:
        async with conn.transaction():
            ...
            await conn.execute(
                ...
                f"{owner}|entity_resolution_batch_v5_1",
            )
