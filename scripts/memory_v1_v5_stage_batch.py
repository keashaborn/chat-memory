#!/usr/bin/env python3
"""Generic owner-scoped runner for reviewed Memory V1 V5 stage bundles."""
...
PLAN_CONTRACT = "memory_v1_v5_stage_batch_plan_v1"
AUTHORIZATION_CONTRACT = "memory_v1_v5_stage_batch_authorization_v1"
REPORT_CONTRACT = "memory_v1_v5_stage_batch_apply_report_v1"
...
def arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plan or apply one reviewed owner-scoped V5 staging batch."
    )
...
def load_bundle(
    ...
    if (
        ...
        raise StageBatchError(f"bundle is not a zero-write V5 preflight: {path}")
    ...
    if (
        ...
        or extraction.get("contract_version") != "memory_v1_relational_extraction_v5"
...
def load_authorization(
    ...
    if (
        ...
        or value["target_server"] != "seebx"
        or value["scope"] != "stage_reviewed_owner_v5_packets_only"
...
async def apply_plan(args: argparse.Namespace) -> dict[str, Any]:
    if os.getenv("MEMORY_V1_V5_STAGE_BATCH_APPLY") != "authorized":
        raise StageBatchError(
            "apply requires MEMORY_V1_V5_STAGE_BATCH_APPLY=authorized"
        )
