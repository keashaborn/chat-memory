...
WORKER_VERSION = "memory_v1_v5_2_stance_schema_reextract_v1"
APPLY_ENABLE_TOKEN = "memory_v1_v5_2_stance_schema_reextract_apply_v1"
SELECTOR_VERSION = "20260722_v5_2_stance_schema_reextract_v1"
NEXT_COMPILER_VERSION = "memory_v1_semantic_policy_compiler_v4"
REASON_CODE = "semantic_stance_object_schema_mismatch"
IDENTITY_NAMESPACE = uuid.UUID("f06ebbcf-a245-4569-a8ad-86fd920af843")
...
def loopback_dsn(value: str) -> str:
    ...
    if parsed.scheme not in {"postgres", "postgresql"}:
        raise RuntimeError("V5.2 stance schema reextract database DSN scheme is invalid")
    if parsed.hostname not in {"127.0.0.1", "::1", "localhost"}:
        raise RuntimeError("V5.2 stance schema reextract database DSN must be loopback-only")
    return value
...
def validated_arguments(args: argparse.Namespace) -> tuple[uuid.UUID, uuid.UUID]:
    ...
    if args.apply and os.getenv("MEMORY_V1_V5_2_STANCE_SCHEMA_REEXTRACT_APPLY") != (
        APPLY_ENABLE_TOKEN
    ):
        raise RuntimeError("V5.2 stance schema reextract apply capability is absent")
    return owner, packet
...
async def read_plan(
    ...
    async with conn.transaction(readonly=True):
        ...
        row = await conn.fetchrow(
            "SELECT * FROM memory.plan_owner_v5_2_stance_schema_reextract_v1($1)",
            packet,
        )
...
async def invoke(
    ...
    *,
    ...
    if row is None:
        raise RuntimeError("V5.2 stance schema reextract apply returned no row")
    return dict(row)
...
async def run() -> int:
    ...
    try:
        if await conn.fetchval("SELECT session_user") != "brains_app":
            raise RuntimeError("V5.2 stance schema reextract requires brains_app session")
        plan = await read_plan(conn, owner, packet)
        if plan is None:
            raise RuntimeError("exact V5.2 stance schema reextract plan is absent")
        if (
            ...
            raise RuntimeError("exact V5.2 stance schema reextract plan changed")
        ...
        if args.expected_plan_sha256 is not None and plan_sha256 != (
            ...
            raise RuntimeError("V5.2 stance schema reextract plan hash changed")
        report = {
        ...
        if applied["apply_outcome"] not in {"applied", "replayed"}:
            raise RuntimeError("V5.2 stance schema reextract apply outcome is invalid")
        if replayed["apply_outcome"] != "replayed":
            raise RuntimeError("V5.2 stance schema reextract replay wrote again")
        report.update(
            {
                "outcome": "queued",
