...
def arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--schema", default="specs/memory_v1_projection_plan_v5_2.schema.json"
    )
    parser.add_argument(
        "--registry", default="specs/memory_v1_predicate_registry_v5_2.json"
    )
    parser.add_argument(
...
def validate_projection_schema(schema: dict[str, Any]) -> None:
    ...
    if schema.get("properties", {}).get("predicate_registry_version", {}).get(
        "const"
    ) != "memory_predicate_registry_v5_2":
        raise AssertionError("projection registry profile mismatch")
...
def validate_registry(registry: dict[str, Any]) -> dict[str, dict[str, Any]]:
    if registry.get("registry_version") != "memory_predicate_registry_v5_2":
        raise AssertionError("predicate registry version mismatch")
    predicates = registry.get("predicates")
    if not isinstance(predicates, list) or len(predicates) != 66:
        raise AssertionError("projection contract requires exactly 66 V5.2 predicates")
...
def validate_packet(
    ...
    if packet["predicate_registry_version"] != "memory_predicate_registry_v5_2":
