...
WORKER_VERSION = "memory_v1_v5_local_semantic_reextract_v3"
APPLY_ENABLE_TOKEN = "memory_v1_v5_local_semantic_reextract_apply_v3"
SELECTOR_VERSION = "20260720_v5_semantic_guard_reextract_v1"
NEXT_COMPILER_VERSION = "memory_v1_local_policy_compiler_v7"
REASON_CODE = "semantic_relationship_role_mismatch"
IDENTITY_NAMESPACE = uuid.UUID("d92b6a24-ff4d-5d1c-a137-3bb0d6f8fb41")
...
def loopback_dsn(value: str) -> str:
    ...
    if parsed.scheme not in {"postgres", "postgresql"}:
        raise RuntimeError("semantic reextract database DSN scheme is invalid")
    if parsed.hostname not in {"127.0.0.1", "::1", "localhost"}:
        raise RuntimeError("semantic reextract database DSN must be loopback-only")
    return value
...
def validated_arguments(args: argparse.Namespace) -> tuple[uuid.UUID, uuid.UUID]:
    ...
    if args.apply and os.getenv("MEMORY_V1_V5_LOCAL_SEMANTIC_REEXTRACT_APPLY") != (
        APPLY_ENABLE_TOKEN
    ):
        raise RuntimeError("semantic reextract apply capability is absent")
    return owner, packet
...
async def read_plan(
    ...
    async with conn.transaction(readonly=True):
        ...
        row = await conn.fetchrow(
            "SELECT * FROM memory.plan_owner_v5_local_semantic_reextract_v3($1)",
            packet,
        )
...
async def invoke(
    ...
    *,
    ...
    if row is None:
        raise RuntimeError("semantic reextract apply returned no row")
    return dict(row)
...
async def run() -> int:
    ...
    try:
        if await conn.fetchval("SELECT session_user") != "brains_app":
            raise RuntimeError("semantic reextract requires brains_app session")
        plan = await read_plan(conn, owner, packet)
        if plan is None:
            raise RuntimeError("exact semantic reextract plan is absent")
        if (
            ...
            raise RuntimeError("exact semantic reextract plan changed")
        ...
        if args.expected_plan_sha256 is not None and plan_sha256 != (
            ...
            raise RuntimeError("semantic reextract plan hash changed")
        report = {
        ...
        if applied["apply_outcome"] not in {"applied", "replayed"}:
            raise RuntimeError("semantic reextract apply outcome is invalid")
        if replayed["apply_outcome"] != "replayed":
            raise RuntimeError("semantic reextract replay wrote again")
        report.update(
            {
                "outcome": "queued",
