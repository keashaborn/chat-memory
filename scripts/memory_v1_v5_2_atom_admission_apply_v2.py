#!/usr/bin/env python3
"""Apply or replay a bounded, owner-reviewed V5.2 atom-admission V2 manifest."""
...
RESULT_CONTRACT = "memory_v1_v5_2_atom_admission_apply_result_v2"
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise AdmissionError("atom-admission manifest must contain 1-16 items")
    seen: set[str] = set()
    authorized = 0
    ...
    if authorized < 1:
        raise AdmissionError("manifest must authorize at least one item")
    item_count = len(value["items"])
    expected_rows = {
        "v5_2_atom_admission_proposal": item_count,
        "v5_2_atom_admission_review": item_count,
        "v5_2_atom_admission_apply": authorized,
        "v5_2_atom_admission_operation": (item_count * 2) + authorized,
...
async def run_item(
    ...
    plan = decode_json(
        await connection.fetchval(
            "SELECT memory.plan_owner_v5_2_atom_admission_v2($1)", packet_id
        )
    )
    ...
    proposal = await connection.fetchrow(
        """
        SELECT * FROM memory.record_owner_v5_2_atom_proposal_v2(
          $1,$2,$3,$4,$5
        )
        """,
    ...
    review_preflight = await connection.fetchrow(
        """
        SELECT * FROM memory.preflight_owner_v5_2_atom_review_v2(
    ...
    review = await connection.fetchrow(
        """
        SELECT * FROM memory.review_owner_v5_2_atom_proposal_v2(
    ...
    if item["review_decision"] == "authorized":
        apply_preflight = await connection.fetchrow(
            "SELECT * FROM memory.preflight_owner_v5_2_atom_apply_v2($1)",
            parse_uuid(item["review_id"], "review_id"),
        )
        applied = await connection.fetchrow(
            """
            SELECT * FROM memory.apply_owner_v5_2_atom_review_v2($1,$2,$3,$4)
            """,
...
async def main() -> None:
    ...
    if args.mode == "apply" and os.getenv(
        "MEMORY_V1_V5_2_ATOM_ADMISSION_APPLY_V2"
    ) != "authorized":
