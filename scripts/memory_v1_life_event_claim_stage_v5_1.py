#!/usr/bin/env python3
"""Preflight, stage, or replay one hash-locked V5.1 death-event claim plan."""
...
from memory_v1_life_event_claim_bundle_v5_1 import BUNDLE_CONTRACT
...
RESULT_CONTRACT = "memory_v1_life_event_claim_stage_result_v5_1"
...
def load_bundle(path_value: str) -> dict[str, Any]:
    ...
    if not isinstance(bundle, dict) or set(bundle) != expected:
        raise RuntimeError("death-event claim bundle fields mismatch")
    if bundle["contract_version"] != BUNDLE_CONTRACT:
        raise RuntimeError("death-event claim bundle contract mismatch")
    ...
    if bundle["expected_new_rows"] != 4 or any(
        ...
        raise RuntimeError("death-event claim bundle boundary mismatch")
    if bundle["bundle_sha256"] != sha256(
        {key: value for key, value in bundle.items() if key != "bundle_sha256"}
    ):
        raise RuntimeError("death-event claim bundle hash mismatch")
    if stable_json(bundle["packet"]) != bundle["packet_text"]:
        raise RuntimeError("death-event claim packet text mismatch")
    return bundle
...
async def run() -> int:
    ...
    if os.environ.get("MEMORY_V1_REQUIRED_HEAD") != bundle["required_head_commit"]:
        raise RuntimeError("runtime head does not match death-event claim bundle")
    if args.mode == "apply" and os.environ.get(
        "MEMORY_V1_LIFE_EVENT_CLAIM_STAGE_APPLY"
    ) != "authorized":
        raise RuntimeError("death-event claim stage capability is absent")
    ...
    try:
        ...
        if (
            ...
            raise RuntimeError("death-event claim bundle source drifted")
        ...
        if (
            ...
            raise RuntimeError("death-event claim database preflight is stale")
        ...
        if args.mode == "preflight":
            ...
            if txid is not None:
                raise RuntimeError("death-event claim preflight assigned a txid")
        else:
            ...
            if rows_written != expected_rows or outcome != expected_outcome:
                raise RuntimeError("death-event claim stage row budget failed")
