...
def build_composite(
    ...
    if counts != {
        "active_predicate_count": 67,
        "legacy_predicate_count": 19,
        "total_contract_count": 86,
    }:
...
def install_rows(
    ...
    if len(contracts) != 86 or len(relationships) != 41:
    ...
    return {
        ...
        "source_bindings": [{
            "source_name": "compiler_v12_coverage_registry",
...
BEGIN
  ...
  IF mismatch_count <> 0
     OR (SELECT count(*) FROM memory.predicate_contract
         WHERE registry_version = '{registry['registry_version']}') <> 86
     OR (SELECT count(*) FROM memory.predicate_contract
         WHERE registry_version = '{registry['registry_version']}'
           AND lifecycle = 'active' AND extraction_allowed) <> 67
     OR (SELECT count(*) FROM memory.predicate_contract
         WHERE registry_version = '{registry['registry_version']}'
