...
MANIFEST_CONTRACT = "memory_v1_v5_2_compiler_v8_claim_stage_manifest_v1"
AUTHORIZATION_CONTRACT = "memory_v1_v5_2_compiler_v8_claim_stage_authorization_v1"
RESULT_CONTRACT = "memory_v1_v5_2_compiler_v8_claim_stage_result_v1"
...
TARGETS = {
    "a0ea633d-96df-4ad8-a0c1-b3f4f84e30cc": {
        ...
        "evidence_content_sha256": "fb65fe592059bace88a4daea571ac1ba7df2b5aeb5136233dd01071cb6085182",
        "observation_sha256": "8fcb1a489e67eca9dbd215eae979e85815bd0f667060f677d5b99459e0d634c9",
        "predicate": "occupation.works_as",
        "object_entity_id": "d73cc811-1772-4cba-96d7-0e021c8053a7",
        "object_canonical_name": "BCBA",
    ...
    "c8ce8cd0-e058-4181-ae94-fd6fb1e7c6eb": {
        "evidence_id": "dcf5ece1-0e22-574f-8ac9-f3d0acc4e8f5",
        "evidence_content_sha256": "fb65fe592059bace88a4daea571ac1ba7df2b5aeb5136233dd01071cb6085182",
        "observation_sha256": "e0c27bba436fc245827fb69e03f0b4adaf6684535790dacb5125f6548bd97cfb",
        "predicate": "occupation.works_as",
        "object_entity_id": "98cc43cd-514a-4f3e-9ba9-7caa84175e57",
    ...
    "70d55f38-1e33-418f-8ec6-6bfd2051f4e6": {
        ...
        "evidence_content_sha256": "895146b94431f7e0ec3292e757e30fc4c782af222bd39598e610654adf08ccec",
        "observation_sha256": "3fe4845c86ba471e6a3104799be8ee5f36ad10d666750ffcee82fce8b41abf41",
        "predicate": "relationship.caregiver_for",
        "object_entity_id": "ca0724b2-100d-4b15-80aa-4016b39cd8b5",
    ...
    "bbd94cc7-e9d5-429f-8af1-1a029b119db0": {
        "evidence_id": "fea59e7e-30f5-4139-b634-97b291c88e14",
        "evidence_content_sha256": "895146b94431f7e0ec3292e757e30fc4c782af222bd39598e610654adf08ccec",
        "observation_sha256": "bbf453bde8ccaef4e1d895dca0cfe0209c94c956908e5daecdf33e949cfaf56c",
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if value["owner_user_id"] != TARGET_OWNER:
        raise ProjectionStageError("manifest owner is outside the exact boundary")
    if sorted(value["evidence_ids"]) != sorted(
        {target["evidence_id"] for target in TARGETS.values()}
    ):
        raise ProjectionStageError("manifest evidence set mismatch")
    ...
    for item in value["items"]:
        ...
        if (
            ...
            or item["predicate"] != target["predicate"]
            or item["state_relation"] != target["state_relation"]
            or item["canonical_text"] != target["canonical_text"]
        ):
            raise ProjectionStageError("exact claim candidate semantics mismatch")
    return value
...
async def source_snapshot(
    ...
    if target is None:
        raise ProjectionStageError("observation is outside the exact target set")
    ...
    if (
        ...
        or source["surface_policy"] not in {"direct_or_relevant", "explicit_recall_only"}
        or source["modality"] != "asserted"
        or source["polarity"] != "affirmed"
        or source["object_kind"] != "entity"
        or str(source["subject_entity_id"]) != SELF_ENTITY_ID
        or source["subject_entity_type"] != "self"
        or source["subject_entity_status"] != "active"
        or str(source["object_entity_id"]) != target["object_entity_id"]
        or source["object_canonical_name"] != target["object_canonical_name"]
        or source["object_entity_status"] != "active"
        or source["temporal"]["state_relation"] != target["state_relation"]
        or source["evidence_status"] != "active"
    ):
        raise ProjectionStageError("source is outside the exact compiler-v8 claim boundary")
...
async def prepare_item(
    ...
    return {
        ...
        "canonical_text": packet["projections"][0]["payload"]["canonical_text"],
    }
...
async def build_manifest(args: argparse.Namespace, conn: Any) -> dict[str, Any]:
    ...
    if owner != TARGET_OWNER:
        raise ProjectionStageError("owner is outside the exact target boundary")
        ...
        for observation in observations:
            items.append(
                await prepare_item(
                    ...
                    owner,
                    observation,
                    stable_id("plan", owner, TARGETS[observation]["evidence_id"], observation),
                    stable_id(
                        "entailment",
                        owner,
                        TARGETS[observation]["evidence_id"],
                        observation,
                    ),
                    registry,
                )
...
async def validate_live_item(
    ...
    *,
    ...
    if (
        ...
        or source["temporal"]["state_relation"] != item["state_relation"]
