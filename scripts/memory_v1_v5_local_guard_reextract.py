...
WORKER_VERSION = "memory_v1_v5_local_guard_reextract_v1"
APPLY_ENABLE_TOKEN = "memory_v1_v5_local_guard_reextract_apply_v1"
SELECTOR_VERSION = "20260719_v5_compound_guard_reextract_v1"
NEXT_COMPILER_VERSION = "memory_v1_local_policy_compiler_v5"
IDENTITY_NAMESPACE = uuid.UUID("f08307df-fbf4-5965-b495-e5eb1eb67162")
...
def loopback_dsn(value: str) -> str:
    ...
    if parsed.scheme not in {"postgres", "postgresql"}:
        raise RuntimeError("guard reextract database DSN scheme is invalid")
    if parsed.hostname not in {"127.0.0.1", "::1", "localhost"}:
        raise RuntimeError("guard reextract database DSN must be loopback-only")
    return value
...
def arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "deterministic guard false positive. Reports contain hashes and "
            "counts only."
        )
    )
...
def validated_arguments(args: argparse.Namespace) -> tuple[uuid.UUID, uuid.UUID]:
    ...
    if args.apply and os.getenv("MEMORY_V1_V5_LOCAL_GUARD_REEXTRACT_APPLY") != (
        APPLY_ENABLE_TOKEN
    ):
        raise RuntimeError("guard reextract apply capability is absent")
    return owner, packet
...
async def read_plan(
    ...
    async with conn.transaction(readonly=True):
        ...
        row = await conn.fetchrow(
            "SELECT * FROM memory.plan_owner_v5_local_guard_reextract_v1($1)",
            packet,
        )
...
async def invoke(
    ...
    *,
    ...
    if row is None:
        raise RuntimeError("guard reextract apply returned no row")
    return dict(row)
...
async def run() -> int:
    ...
    try:
        if await conn.fetchval("SELECT session_user") != "brains_app":
            raise RuntimeError("guard reextract requires brains_app session")
        plan = await read_plan(conn, owner, packet)
        if plan is None:
            raise RuntimeError("exact guard reextract plan is absent")
        if (
        ...
        if args.expected_plan_sha256 is not None and plan_sha256 != (
            ...
            raise RuntimeError("guard reextract plan hash changed")
        report = {
        ...
        if applied["apply_outcome"] not in {"applied", "replayed"}:
            raise RuntimeError("guard reextract apply outcome is invalid")
        if replayed["apply_outcome"] != "replayed":
            raise RuntimeError("guard reextract replay wrote again")
        report.update(
            {
                "outcome": "queued",
