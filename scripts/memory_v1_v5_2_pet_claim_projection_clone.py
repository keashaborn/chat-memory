...
PLAN_CONTRACT = "memory_v1_v5_2_pet_claim_projection_plan_v1"
REPORT_CONTRACT = "memory_v1_v5_2_pet_claim_projection_clone_result_v1"
...
def load_plan(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or len(items) != 11
        or len({item.get("claim_id") for item in items}) != 11
        or not isinstance(projection, dict)
        or projection.get("maximum_embedding_requests") != 11
    ...
    for item in items:
        ...
        if (
            ...
            not in {"identity.name", "pet.breed", "pet.species", "relationship.has_pet"}
            or item.get("prior_outbox") not in {"absent", "done_revision_2"}
            or item.get("prior_qdrant") not in {"absent", "revision_2"}
...
async def run() -> int:
    ...
    try:
        ...
        result = await process_owner_projection_outbox(
            ...
            index=index,
            embedder=embed,
            worker_id="pet-claim-projection-clone",
            lease_seconds=600,
            limit=11,
            max_attempts=8,
        )
        ...
        if result != expected_result or calls != 11:
        ...
        replay = await process_owner_projection_outbox(
            ...
            worker_id="pet-claim-projection-clone-replay",
            lease_seconds=600,
            limit=11,
            max_attempts=8,
        )
        ...
        report = {
            ...
            "new_projection_count": 10,
            "replacement_projection_count": 1,
            "embedding_requests": calls,
            "external_model_calls": 0,
            "temporary_qdrant_writes": 12,
