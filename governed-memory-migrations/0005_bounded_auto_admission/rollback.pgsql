...
BEGIN
  IF EXISTS (
    SELECT 1 FROM memory.proposal
    WHERE reviewer_kind = 'system'
      AND review_reason_codes = ARRAY['automatic_low_risk_owner_assertion']::text[]
  ) THEN
    RAISE EXCEPTION 'automatic admission rows prevent schema rollback';
  END IF;
END;
$rollback_guard$;
DROP FUNCTION memory_private.auto_admit_one_ordinary_proposal();
REVOKE EXECUTE ON FUNCTION memory_private.review_proposal(
  uuid,uuid,text,text,text,text,text,text,text[]
) FROM governed_memory_worker;
...
ALTER TABLE memory.proposal
  ...
  ADD CONSTRAINT proposal_review_reason_codes CHECK (
    review_reason_codes IS NULL OR (
      ...
      AND memory_private.is_sorted_unique_allowlist(review_reason_codes,
        ARRAY['duplicate_existing','explicit_owner_review','lifecycle_override',
          'not_durable','proposal_expired','proposal_incorrect']::text[])
    )
  );
...
ALTER TABLE memory.claim_evidence
  ADD CONSTRAINT claim_evidence_reason_codes CHECK (
    ...
    AND memory_private.is_sorted_unique_allowlist(reason_codes,
      ARRAY['duplicate_existing','explicit_owner_review','not_durable',
        'proposal_incorrect']::text[])
  );
