...
ALTER TABLE memory.proposal
  ADD CONSTRAINT proposal_review_shape CHECK (
    ...
    OR (review_state = 'admitted' AND reviewer_kind = 'system'
      ...
      AND review_reason_codes IN (
        ARRAY['automatic_low_risk_owner_assertion']::text[],
        ARRAY['automatic_low_risk_personal_object_location']::text[]
      )
      AND reviewed_at IS NOT NULL)
  ...
  ADD CONSTRAINT proposal_review_reason_codes CHECK (
    review_reason_codes IS NULL OR (
      ...
      AND memory_private.is_sorted_unique_allowlist(review_reason_codes,
        ARRAY['automatic_low_risk_owner_assertion',
          'automatic_low_risk_personal_object_location','duplicate_existing',
...
CREATE FUNCTION memory_private.automatic_admission_reason(
...
AS $function$
  SELECT CASE
    WHEN p_subject_entity_type = 'self'
      AND p_subject_entity_key = 'self'
      AND p_subject_display_name IS NULL
    THEN 'automatic_low_risk_owner_assertion'
    WHEN p_subject_entity_type = 'other'
      AND p_subject_entity_key ~ '^local:[0-9a-f]{64}$'
      AND p_subject_display_name IS NOT NULL
      AND p_predicate = 'location.association'
      AND p_object_kind IN ('entity', 'literal')
      AND pg_catalog.octet_length(p_review_excerpt) <= 600
  ...
  END
$function$;
ALTER FUNCTION memory_private.automatic_admission_reason(
  text,text,text,text,text,text
) OWNER TO governed_memory_owner;
REVOKE ALL ON FUNCTION memory_private.automatic_admission_reason(
  text,text,text,text,text,text
) FROM PUBLIC, governed_memory_api, governed_memory_worker;
...
BEGIN
  ...
  END IF;
  IF automatic_admission THEN
    automatic_reason := memory_private.automatic_admission_reason(
      proposal.subject_entity_type, proposal.subject_entity_key,
      proposal.subject_display_name, proposal.predicate,
      proposal.object_kind, proposal_evidence.review_excerpt
    );
  END IF;
  IF automatic_admission AND (
       ...
       OR automatic_reason IS NULL
       OR p_reason_codes IS DISTINCT FROM ARRAY[automatic_reason]::text[]
...
CREATE OR REPLACE FUNCTION memory_private.auto_admit_one_ordinary_proposal()
RETURNS TABLE(
  outcome text,
  proposal_id uuid,
  claim_id uuid,
...
BEGIN
  ...
  WHERE proposal.review_state = 'pending_review'
    ...
    AND memory_private.automatic_admission_reason(
      proposal.subject_entity_type, proposal.subject_entity_key,
      proposal.subject_display_name, proposal.predicate,
      proposal.object_kind, evidence.review_excerpt
    ) IS NOT NULL
  ...
  FROM memory_private.review_proposal(
    ...
    ARRAY[candidate.automatic_reason]::text[]
  ) AS reviewed;
END;
$function$;
