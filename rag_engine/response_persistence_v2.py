...
class ResponsePersistenceV2Error(RuntimeError):
...
async def persist_finalized_response_v2(
    conn: object,
    *,
    owner_user_id: UUID,
    thread_id: UUID,
    ...
    stage = "validation"
    try:
        value = FinalizedTrustedResponseV2.model_validate_json(
        ...
        async with conn.transaction():
            ...
            await conn.execute(
                ...
                ["assistant", "chat", "server_attested", "response_v2"],
                thread_id,
                request_id,
        ...
        raise ResponsePersistenceV2Error(stage) from None


__all__ = ["ResponsePersistenceV2Error", "persist_finalized_response_v2"]
