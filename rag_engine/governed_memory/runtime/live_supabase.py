...
"""Bounded live Supabase user check for the owner HTTP boundary.

This adapter deliberately consumes only the authoritative response ``id``.
It does not inspect caller-controlled profile or application metadata.  A
successful ``/auth/v1/user`` response proves that Supabase accepted the access
token at request time, but it is not claimed to be a strict immediate signout
revocation check; that decision remains an activation blocker.
"""
...
def _checked_issuer(issuer: object) -> tuple[str, str]:
    if (
    ...
    if normalized != issuer:
        raise HttpAuthError("auth_live_configuration_invalid")
    return normalized, f"{normalized}/user"
...
class LiveSupabaseUserConfig:
    ...
    def __post_init__(self) -> None:
        issuer_url, user_url = _checked_issuer(self.issuer)
...
class LiveSupabaseUserVerifier:
    config: LiveSupabaseUserConfig
    fetcher: UserFetcher = _default_fetch_user

    def __post_init__(self) -> None:
        if not isinstance(self.config, LiveSupabaseUserConfig) or not callable(
            self.fetcher
        ):
            raise HttpAuthError("auth_live_configuration_invalid")
