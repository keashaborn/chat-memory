...
ChatMemoryIngestResolutionV1 = Literal["release", "suppress"]
_RESOLUTION_OUTCOMES = frozenset(
    {"absent", "released", "replayed", "suppressed"}
)
...
async def coordinate_chat_memory_ingest_v1(
    ...
    *,
    ...
    if not isinstance(message_id, UUID) or not isinstance(thread_id, UUID):
        raise ChatMemoryIngestCoordinationErrorV1(
    ...
    if resolution not in {"release", "suppress"}:
        raise ChatMemoryIngestCoordinationErrorV1(
    ...
    try:
        async with connection.transaction():
            ...
            outcome = await connection.fetchval(
                ...
                "resolve_chat_memory_command($1::uuid,$2::uuid,$3::text,$4::text)",
                message_id,
                thread_id,
                content_sha256,
                resolution,
